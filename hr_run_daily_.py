...
def _slugify_filename(value: str) -> str:
    value = re.sub(r"[^A-Za-z0-9]+", "_", str(value or "game")).strip("_")
...
def _wind_impact_label(wind_mult, wind_mph) -> str:
    try:
        wm = float(wind_mult)
    except Exception:
        wm = 1.0
    try:
        ...
        mph = float("nan")
    if np.isfinite(mph) and mph < 5:
        return "Light / neutral"
    if wm >= 1.05:
        return "Blowing out / hitter-friendly"
    if wm <= 0.97:
        return "Blowing in / suppressing"
    return "Cross / mostly neutral"


def _pitcher_hr_bucket(score) -> str:
    try:
        s = float(score)
    except Exception:
        return "Unknown"
    if s >= 70:
        return "Very attackable"
    if s >= 60:
        return "Attackable"
    if s >= 50:
        return "Neutral"
...
def generate_game_sheets(board: pd.DataFrame, date_str: str, top_n: int = 75) -> None:
    ...
    work = board.copy()
    work["_game_key"] = work["venue"].fillna("Unknown Venue")
    if "exp_pa" in work.columns:
        work["_lineup_sort"] = pd.to_numeric(work["exp_pa"], errors="coerce").fillna(0.0)
    else:
        work["_lineup_sort"] = -pd.to_numeric(work.get("rank", 9999), errors="coerce").fillna(9999)

    summary_rows = []
    matchup_rows = []

    for game_key, gdf in work.groupby("_game_key", dropna=False):
        game_df = gdf.copy()
        teams = sorted({str(x) for x in game_df.get("team", pd.Series(dtype=str)).dropna().tolist() if str(x).strip()})
        pitchers = sorted({str(x) for x in game_df.get("probable_pitcher_faced", pd.Series(dtype=str)).dropna().tolist() if str(x).strip()})
        temp_f = pd.to_numeric(game_df.get("weather_temp_f", pd.Series(dtype=float)), errors="coerce").dropna()
        wind_mph = pd.to_numeric(game_df.get("weather_wind_mph", pd.Series(dtype=float)), errors="coerce").dropna()
        wind_dir = pd.to_numeric(game_df.get("weather_wind_dir_deg", pd.Series(dtype=float)), errors="coerce").dropna()
        wind_mult = pd.to_numeric(game_df.get("weather_wind_mult", pd.Series(dtype=float)), errors="coerce").dropna()
        weather_mult = pd.to_numeric(game_df.get("weather_mult", pd.Series(dtype=float)), errors="coerce").dropna()

        avg_temp = round(float(temp_f.mean()), 1) if not temp_f.empty else None
        avg_wind = round(float(wind_mph.mean()), 1) if not wind_mph.empty else None
        avg_wind_dir_deg = round(float(wind_dir.mean()), 1) if not wind_dir.empty else None
        avg_wind_mult = round(float(wind_mult.mean()), 3) if not wind_mult.empty else 1.0
        avg_weather_mult = round(float(weather_mult.mean()), 3) if not weather_mult.empty else 1.0
        wind_compass = _deg_to_compass(avg_wind_dir_deg)
        wind_impact = _wind_impact_label(avg_wind_mult, avg_wind)

        game_top = game_df.sort_values(["rank", "p_hr_1plus_sim"], ascending=[True, False]).head(6)
        top_bats = ", ".join(game_top["player_name"].astype(str).tolist())
        top10 = int((pd.to_numeric(game_df.get("rank", pd.Series(dtype=float)), errors="coerce") <= 10).sum())
        top25 = int((pd.to_numeric(game_df.get("rank", pd.Series(dtype=float)), errors="coerce") <= 25).sum())
        top75_count = int((pd.to_numeric(game_df.get("rank", pd.Series(dtype=float)), errors="coerce") <= top_n).sum())
        mean_matchup = round(float(pd.to_numeric(game_df.get("matchup_score_clean", pd.Series(dtype=float)), errors="coerce").mean()), 2)
        max_prob = round(float(pd.to_numeric(game_df.get("p_hr_1plus_sim", pd.Series(dtype=float)), errors="coerce").max()), 4)

        summary_rows.append({
            "date": date_str,
            "game": " vs ".join(teams) if teams else str(game_key),
            "venue": game_key,
            "teams": " | ".join(teams),
            "pitchers": " | ".join(pitchers),
            "weather_temp_f": avg_temp,
            "weather_wind_mph": avg_wind,
            "weather_wind_dir_deg": avg_wind_dir_deg,
            "weather_wind_direction": wind_compass,
            "weather_wind_impact": wind_impact,
            "weather_mult_avg": avg_weather_mult,
            "avg_matchup_score": mean_matchup,
            "max_hr_prob": max_prob,
            "top10_count": top10,
            "top25_count": top25,
            "top75_count": top75_count,
            "top_bats": top_bats,
        })

        team_views = []
        for team_name, tdf in game_df.groupby("team", dropna=False):
            team_df = tdf.copy().sort_values(["_lineup_sort", "rank", "p_hr_1plus_sim"], ascending=[False, True, False])
            opp_pitcher = team_df["probable_pitcher_faced"].dropna().astype(str).iloc[0] if "probable_pitcher_faced" in team_df.columns and not team_df["probable_pitcher_faced"].dropna().empty else "TBD"
            pitch_score = round(float(pd.to_numeric(team_df.get("pitcher_hr_score", pd.Series(dtype=float)), errors="coerce").mean()), 2)
            pitch_bucket = _pitcher_hr_bucket(pitch_score)
            pitch_mix = round(float(pd.to_numeric(team_df.get("pitch_mix_fit_score", pd.Series(dtype=float)), errors="coerce").mean()), 2)
            bullpen = round(float(pd.to_numeric(team_df.get("bullpen_score", pd.Series(dtype=float)), errors="coerce").mean()), 2)
            top_team = team_df.sort_values(["rank", "p_hr_1plus_sim"], ascending=[True, False]).head(5)
            team_views.append({
                "date": date_str,
                "game": " vs ".join(teams) if teams else str(game_key),
                "venue": game_key,
                ...
                "pitcher_hr_score": pitch_score,
                "pitcher_profile": pitch_bucket,
                "pitch_mix_fit_avg": pitch_mix,
                "bullpen_score": bullpen,
                "weather_temp_f": avg_temp,
                "weather_wind_mph": avg_wind,
                "weather_wind_direction": wind_compass,
                "weather_wind_impact": wind_impact,
                "team_top_bats": ", ".join(top_team["player_name"].astype(str).tolist()),
                "team_top10_count": int((pd.to_numeric(team_df.get("rank", pd.Series(dtype=float)), errors="coerce") <= 10).sum()),
                "team_top25_count": int((pd.to_numeric(team_df.get("rank", pd.Series(dtype=float)), errors="coerce") <= 25).sum()),
            })
            detailed = team_df.sort_values(["rank", "p_hr_1plus_sim"], ascending=[True, False]).copy()
            detailed.insert(0, "game", " vs ".join(teams) if teams else str(game_key))
            detailed.insert(1, "weather_wind_direction", wind_compass)
            detailed.insert(2, "weather_wind_impact", wind_impact)
            keep_cols = [
                "game", "venue", "team", "probable_pitcher_faced", "weather_temp_f", "weather_wind_mph",
                "weather_wind_direction", "weather_wind_impact", "rank", "prob_rank", "player_name",
                "matchup_label", "p_hr_1plus_sim", "p_hr_2plus_sim", "exp_pa", "matchup_score_clean",
                "pitcher_hr_score", "pitch_mix_fit_score", "bullpen_score", "recent_damage_score",
                "dominant_pitch", "dominant_pitch_usage", "batter_vs_pitch_ratio", "bet_flag_tier", "bet_flag_reason"
            ]
            keep_cols = [c for c in keep_cols if c in detailed.columns]
            matchup_rows.append(detailed[keep_cols])
            team_slug = _slugify_filename(f"{date_str}_{game_key}_{team_name}")
            detailed[keep_cols].to_csv(f"outputs/game_sheets/hr_game_sheet_{team_slug}.csv", index=False)

        if team_views:
            pd.DataFrame(team_views).to_csv(
                f"outputs/game_sheets/hr_game_summary_{_slugify_filename(f'{date_str}_{game_key}')}.csv",
                index=False,
            )

    if summary_rows:
        pd.DataFrame(summary_rows).sort_values(["top10_count", "top25_count", "max_hr_prob"], ascending=False).to_csv(
            f"outputs/hr_game_environment_report_{date_str}.csv", index=False
        )
    if matchup_rows:
        pd.concat(matchup_rows, ignore_index=True).to_csv(
            f"outputs/hr_game_sheets_all_{date_str}.csv", index=False
        )
...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    generate_game_sheets(board, date_str=date_str, top_n=top_n)
