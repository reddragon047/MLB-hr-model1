...
def get_confirmed_lineup_slots(game_pk):
    ...
    Notes:
    - We ONLY apply lineup-based exp_pa when the lineup is actually posted (confirmed).
    - Pregame, statsapi.boxscore_data() can be incomplete; we first try the official
      MLB live feed endpoint, then fall back to statsapi.boxscore_data().
    """
    if not game_pk:
    ...
    def parse_players_dict(players_dict):
        slots = {}
        ...
        for p in players_dict.values():
            ...
            try:
                ...
                continue
            slot = _batting_order_to_slot(p.get("battingOrder"))
            if slot is None:
                continue
            if pid not in slots:
                slots[pid] = slot
    ...
    # 1) Preferred: official live feed (often populated pregame once lineups are posted)
    try:
        url = f"https://statsapi.mlb.com/api/v1.1/game/{gpk}/feed/live"
        r = requests.get(url, timeout=20, headers={"User-Agent": "Mozilla/5.0 (HRBoard)"})
        r.raise_for_status()
        j = r.json()
        teams = j.get("liveData", {}).get("boxscore", {}).get("teams", {})
        home_players = teams.get("home", {}).get("players", {})
        away_players = teams.get("away", {}).get("players", {})

        home_slots = parse_players_dict(home_players)
        away_slots = parse_players_dict(away_players)
        ...
        # If either side is confirmed, cache and return immediately
        if out["home_confirmed"] or out["away_confirmed"]:
            LINEUP_CACHE[gpk] = out
            return out
    except Exception:
        pass

    # 2) Fallback: statsapi.boxscore_data (often reliable in-game / postgame)
    try:
