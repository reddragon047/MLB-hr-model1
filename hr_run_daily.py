...
# Safe helper aliases so runtime never dies on missing function names from older patches
_SAFE_SHARPENED_PITCHTYPE_MULTIPLIER = globals().get("sharpened_pitchtype_multiplier")
_SAFE_EXP_BBE_MULTIPLIER = globals().get("exp_bbe_multiplier")
    ...
    for g in games:
        ...
        for side in ["away", "home"]:
            ...
            for hid in hitter_ids:
                ...
                _pt_func = globals().get("sharpened_pitchtype_multiplier") or _SAFE_SHARPENED_PITCHTYPE_MULTIPLIER
                if callable(_pt_func):
                    pt_mult = _pt_func(
                        pt_mult=pt_mult,
                    ...
                    try:
                        pt_mult = float(np.clip(float(pt_mult), 0.93, 1.10))
                    except Exception:
                        pt_mult = 1.0
...
def export_individual_game_sheets(board: pd.DataFrame, date_str: str):
    """Write one top-level CSV per game plus an index. Never raise into the model run."""
    try:
        if board is None or board.empty:
            print("[game_sheets] board empty; skipped.")
            return 0
        ...
        for c in ["venue", "team", "probable_pitcher_faced", "player_name"]:
        ...
        # best-effort game key
        if "game_sheet_key" not in work.columns:
            away = pd.to_numeric(work.get("is_home", 0), errors="coerce").fillna(0)
            if "opponent" in work.columns:
                opp = work["opponent"].astype(str)
            elif "opp" in work.columns:
                opp = work["opp"].astype(str)
            else:
                opp = pd.Series([""] * len(work), index=work.index)
            team = work["team"].astype(str)
            venue = work["venue"].astype(str)
            work["game_sheet_key"] = np.where(away.eq(1), opp + "_at_" + team, team + "_at_" + opp)
            work["game_sheet_key"] = work["game_sheet_key"].str.replace(r"[^A-Za-z0-9_\-]+", "_", regex=True).str.strip("_")
            bad = work["game_sheet_key"].isin(["_at_", "", "nan_at_nan", "None_at_None"])
            work.loc[bad, "game_sheet_key"] = venue.str.replace(r"[^A-Za-z0-9_\-]+", "_", regex=True).str.strip("_")
            work.loc[bad, "game_sheet_key"] = work.loc[bad, "game_sheet_key"].mask(work.loc[bad, "game_sheet_key"].eq(""), "game_unknown")

        preferred = [
            "date", "game_sheet_key", "venue", "team", "opponent", "player_name",
            "rank", "prob_rank", "p_hr_1plus_sim", "matchup_label", "matchup_score_clean",
            "probable_pitcher_faced", "pitcher_hr_score", "pitch_mix_fit_score", "bullpen_score",
            "recent_damage_score", "dominant_pitch", "dominant_pitch_usage", "batter_vs_pitch_ratio",
            "weather_temp", "temp_f", "wind_speed", "wind_dir", "wind_direction",
            "weather_mult", "park_factor", "bet_flag", "bet_flag_reason"
        ]

        created = 0
        index_rows = []
        for gkey, gdf in work.groupby("game_sheet_key", dropna=False):
            gdf = gdf.copy()
            # sort by board rank when available
            if "rank" in gdf.columns:
                gdf = gdf.sort_values(["rank", "p_hr_1plus_sim"], ascending=[True, False])
            elif "p_hr_1plus_sim" in gdf.columns:
                gdf = gdf.sort_values(["p_hr_1plus_sim"], ascending=[False])

            cols = [c for c in preferred if c in gdf.columns] + [c for c in gdf.columns if c not in preferred]
            gdf = gdf[cols]
            safe_key = str(gkey) if str(gkey).strip() else "game_unknown"
            path = os.path.join(out_dir, f"hr_game_sheet_{safe_key}_{date_str}.csv")
            gdf.to_csv(path, index=False)
            created += 1
            index_rows.append({
                "date": date_str,
                "game_sheet_key": safe_key,
                "rows": len(gdf),
                "file": os.path.basename(path),
                "venue": str(gdf["venue"].iloc[0]) if "venue" in gdf.columns and len(gdf) else "",
            })

        pd.DataFrame(index_rows).to_csv(os.path.join(out_dir, f"hr_game_sheet_index_{date_str}.csv"), index=False)
        print(f"[game_sheets] created {created} per-game CSV sheets.")
        return created
    except Exception as e:
...
def main():
    ...
    created = export_individual_game_sheets(board, args.date)
    if created == 0:
        try:
            fallback_path = f"outputs/hr_board_full_slate_{args.date}.csv"
            if os.path.exists(fallback_path):
                fb = pd.read_csv(fallback_path)
                export_individual_game_sheets(fb, args.date)
        except Exception as e:
            print(f"[game_sheets] fallback warning: {e}")
