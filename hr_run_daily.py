...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    for g in games:
        ...
        for side in ["away", "home"]:
            ...
            for hid in hitter_ids:
                ...
                _sharpener = globals().get("sharpened_pitchtype_multiplier")
                if callable(_sharpener):
                    try:
                        pt_mult = _sharpener(
                            pt_mult=pt_mult,
                        ...
                        pt_mult = float(np.clip(pd.to_numeric(pt_mult, errors="coerce") if hasattr(pd, "to_numeric") else pt_mult, 0.85, 1.25))
                else:
                    try:
                        pt_mult = float(np.clip(float(pt_mult), 0.85, 1.25))
                    except Exception:
                        pt_mult = 1.0
    ...
    # Additional per-game environment and hitter views (read-only outputs)
    try:
        ...
        for col, default in {
            ...
            "weather_temp_f": np.nan,
            "weather_wind_mph": np.nan,
            "weather_wind_dir_deg": np.nan,
            "weather_mult": 1.0,
            "pitcher_hr_score": np.nan,
            "pitch_mix_fit_score": np.nan,
            "bullpen_score": np.nan,
            "recent_damage_score": np.nan,
            "dominant_pitch": "",
            "dominant_pitch_usage": np.nan,
            "batter_vs_pitch_ratio": np.nan,
            "bet_flag_reason": "",
            "rank": np.nan,
            "prob_rank": np.nan,
            "p_hr_1plus_sim": np.nan,
            "matchup_label": "",
        }.items():
        ...
        def _wind_label(row):
            wm = pd.to_numeric(row.get("weather_mult"), errors="coerce")
            ws = pd.to_numeric(row.get("weather_wind_mph"), errors="coerce")
            if pd.notna(wm):
                if wm >= 1.05:
                    return "Boost"
                if wm <= 0.97:
                    return "Suppress"
            if pd.notna(ws) and ws >= 10:
                return "Meaningful Wind"
            return "Neutral"

        gs["wind_impact_label"] = gs.apply(_wind_label, axis=1)
        gs["game_key"] = (
            gs["venue"].fillna("").astype(str).str.strip() + " | " +
            gs["team"].fillna("").astype(str).str.strip() + " vs " +
            gs["probable_pitcher_faced"].fillna("").astype(str).str.strip()
        ).str.strip(" |")
        gs.loc[gs["game_key"].eq(""), "game_key"] = gs["venue"].fillna("Unknown Game")

        env_cols = [
            "date", "game_key", "venue", "team", "probable_pitcher_faced",
            "weather_temp_f", "weather_wind_mph", "weather_wind_dir_deg",
            "weather_mult", "wind_impact_label",
            "pitcher_hr_score", "bullpen_score"
        ]
        env_cols = [c for c in env_cols if c in gs.columns]
        env_report = gs[env_cols].drop_duplicates().sort_values([c for c in ["venue", "team"] if c in env_cols])
        env_report.to_csv(f"outputs/hr_game_environment_report_{date_str}.csv", index=False)

        hitter_cols = [
            "date", "game_key", "venue", "team", "player_name", "probable_pitcher_faced",
            "rank", "prob_rank", "p_hr_1plus_sim", "matchup_label",
            "pitcher_hr_score", "pitch_mix_fit_score", "bullpen_score", "recent_damage_score",
            "weather_temp_f", "weather_wind_mph", "weather_wind_dir_deg", "wind_impact_label",
            "dominant_pitch", "dominant_pitch_usage", "batter_vs_pitch_ratio", "bet_flag_reason"
        ]
        hitter_cols = [c for c in hitter_cols if c in gs.columns]
        all_games = gs[hitter_cols].sort_values([c for c in ["game_key", "rank", "prob_rank"] if c in gs.columns])
        all_games.to_csv(f"outputs/hr_game_sheets_all_{date_str}.csv", index=False)

        for game_key, gdf in gs.groupby("game_key", dropna=False):
            safe_name = "_".join(str(game_key).replace("/", "-").replace("\\", "-").split())[:140] or "game"
            gdf[hitter_cols].sort_values([c for c in ["rank", "prob_rank"] if c in gdf.columns]).to_csv(
                f"outputs/game_sheets/{safe_name}_{date_str}.csv", index=False
            )
    except Exception as e:
        print(f"[WARN] game sheet export skipped: {e}")
