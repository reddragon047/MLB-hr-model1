...
def fails_viability_filter(row) -> bool:
    """
    Remove obvious non-bettable fringe names from the final betting board.
    Keeps full-slate generation intact, then filters only at the end.
    """
    import pandas as pd
    ...
    try:
        ...
        p_hr_pa = 0.0
    try:
        power_qualified = int(row.get("power_qualified", 0) or 0)
    except Exception:
        power_qualified = 0

    matchup_tier = str(row.get("matchup_tier", "") or "").lower().strip()

    odds_open = row.get("odds_open_1plus", None)
    odds_close = row.get("odds_close_1plus", None)

    no_open = pd.isna(odds_open) or odds_open in ("", "NA", "N/A", None)
    no_close = pd.isna(odds_close) or odds_close in ("", "NA", "N/A", None)
    no_odds = no_open and no_close

    # Hard remove no-odds fringe players like backup catchers / pinch-hit types.
    if no_odds and exp_pa < 3.8 and power_qualified == 0:
        return True

    # If the matchup itself is weak and player is unqualified with no odds, remove.
    if no_odds and matchup_tier == "thin":
        return True

    # Low PA + weak HR-per-PA + no odds = not a board-worthy pregame play.
    if no_odds and exp_pa < 3.7 and p_hr_pa < 0.03:
        return True
...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    # Keep full slate generation, then filter only obvious non-bettable fringe names.
    ...
    preferred_cols = [
        ...
        "recent_barrel_rate", "recent_hard_hit_rate", "recent_ev", "exp_bbe_mult", "power_spike_mult", "p_hr_1plus_raw", "calibration_method", "batter_id", "date", "odds_open_1plus", "odds_close_1plus"
    ]
