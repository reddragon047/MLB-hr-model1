...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    def _relative_bucket(series: pd.Series, high_q: float, mid_q: float, high_label: str = "HIGH", mid_label: str = "MED", low_label: str = "LOW") -> pd.Series:
        vals = pd.to_numeric(series, errors="coerce").fillna(0.0)
        if len(vals) == 0:
            return pd.Series(dtype="object")
        q_high = float(vals.quantile(high_q))
        q_mid = float(vals.quantile(mid_q))
        if q_high <= q_mid:
            pct = vals.rank(method="average", pct=True)
            out = np.where(pct >= high_q, high_label, np.where(pct >= mid_q, mid_label, low_label))
        else:
            out = np.where(vals >= q_high, high_label, np.where(vals >= q_mid, mid_label, low_label))
        return pd.Series(out, index=vals.index)

    def _power_yes_no(series: pd.Series, yes_q: float = 0.60) -> pd.Series:
        vals = pd.to_numeric(series, errors="coerce").fillna(0.0)
        if len(vals) == 0:
            return pd.Series(dtype="object")
        q_yes = float(vals.quantile(yes_q))
        if q_yes <= 0:
            pct = vals.rank(method="average", pct=True)
            out = np.where(pct >= yes_q, "YES", "NO")
        else:
            out = np.where(vals >= q_yes, "YES", "NO")
        return pd.Series(out, index=vals.index)

    board["pitch_match_bucket"] = _relative_bucket(board.get("pitch_mix_fit_score", 0.0), high_q=0.75, mid_q=0.40)
    board["pitcher_risk_bucket"] = _relative_bucket(board.get("pitcher_vulnerability_score", board.get("pitcher_hr_score", 0.0)), high_q=0.70, mid_q=0.35)
    board["recent_power_bucket"] = _power_yes_no(board.get("recent_damage_score", 0.0), yes_q=0.60)
    board["bullpen_bucket"] = _relative_bucket(board.get("bullpen_score", 0.0), high_q=0.70, mid_q=0.35, high_label="WEAK", mid_label="AVG", low_label="OK")

    pm = pd.to_numeric(board.get("pitch_mix_fit_score", 0.0), errors="coerce").fillna(0.0)
    pr = pd.to_numeric(board.get("pitcher_vulnerability_score", board.get("pitcher_hr_score", 0.0)), errors="coerce").fillna(0.0)
    rd = pd.to_numeric(board.get("recent_damage_score", 0.0), errors="coerce").fillna(0.0)
    env = pd.to_numeric(board.get("environment_score", 0.0), errors="coerce").fillna(0.0)
    opp = pd.to_numeric(board.get("opportunity_score", 0.0), errors="coerce").fillna(0.0)

    pm_hi = float(pm.quantile(0.75))
    pr_hi = float(pr.quantile(0.70))
    rd_hi = float(rd.quantile(0.60))
    env_hi = float(env.quantile(0.70))
    opp_hi = float(opp.quantile(0.70))
    ...
    board["pitch_match_flag"] = "MATCH:" + board["pitch_match_bucket"].astype(str)
    board["pitcher_risk_flag"] = "RISK:" + board["pitcher_risk_bucket"].astype(str)
    board["recent_power_flag"] = "POWER:" + board["recent_power_bucket"].astype(str)
    board["bullpen_flag"] = "BP:" + board["bullpen_bucket"].astype(str)
    board["flags"] = (
        board["pitch_match_flag"].astype(str) + " | " +
        board["pitcher_risk_flag"].astype(str) + " | " +
        board["recent_power_flag"].astype(str) + " | " +
        board["bullpen_flag"].astype(str)
    )
    board["display_line"] = (
        board["player_name"].astype(str) + " | " +
        (pd.to_numeric(board["p_hr_1plus_sim"], errors="coerce").fillna(0.0) * 100).round(1).astype(str) +
        "% | " + board["matchup_label"].astype(str) + " | " + board["flags"].astype(str)
    )
    ...
    cols = [
        ...
        "p_hr_1plus_sim","matchup_label",
        "pitch_match_flag","pitcher_risk_flag","recent_power_flag","bullpen_flag",
        "flags","display_line",
        "matchup_score_clean","matchup_reason_primary",
        "recent_damage_score","pitch_mix_fit_score","pitcher_hr_score","bullpen_score",
        "pitch_match_bucket","pitcher_risk_bucket","recent_power_bucket","bullpen_bucket"
    ]
...
def write_outputs(board: pd.DataFrame, date_str: str, top_n: int):
    if "p_hr_1plus_sim" in board.columns:
        board = board.sort_values("p_hr_1plus_sim", ascending=False).reset_index(drop=True)
