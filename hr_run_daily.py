...
def sharpened_pitchtype_multiplier(
    ...
    """
    Slightly sharpen the stable pitch-mix multiplier without letting it hijack the board.

    Inputs:
      pt_mult                 -> base multiplier from compute_pitch_matchup_multiplier
      dominant_pitch_usage    -> starter's top pitch usage share
      batter_vs_pitch_ratio   -> hitter HR/PA vs dominant pitch relative to league

    Returns a bounded multiplier centered near 1.0.
    """
    try:
        base = float(pt_mult)
    except Exception:
    ...
    # Neutralize when there is not enough dominant-pitch concentration to matter.
    if usage < 0.25:
        return float(np.clip(base, 0.95, 1.08))

    # Scale the pitch-fit signal by how concentrated the starter is on the dominant pitch.
    # This gives more lift/suppression only when the pitch actually drives the matchup.
    ...
    for g in games:
        ...
        for side in ["away", "home"]:
            ...
            for hid in hitter_ids:
                ...
                sharpen_fn = globals().get("sharpened_pitchtype_multiplier")
                if callable(sharpen_fn):
                    pt_mult = sharpen_fn(
                        pt_mult=pt_mult,
                    ...
                    try:
                        ...
                        pt_mult = 1.0
                    pt_mult = float(np.clip(pt_mult, 0.93, 1.12))
