...
def safe_sort_values(self, by=None, *args, **kwargs):
    try:
        return _original_df_sort_values(self, by=by, *args, **kwargs)
    except KeyError as e:
        missing_key = e.args[0] if e.args else None
        if missing_key is None:
            raise
        by_list = list(by) if isinstance(by, (list, tuple)) else [by]
        if missing_key not in by_list:
            raise
        tmp = self.copy()
        # If old code asks for game_date, mirror date when possible.
        if missing_key == "game_date" and "date" in tmp.columns:
            tmp["game_date"] = tmp["date"].astype(str)
        else:
            tmp[missing_key] = np.nan
        print(f"[production_guard] sort_values missing {missing_key!r}; created fallback column and continued.")
...
def refine_daily_context_from_scorecard(
    matchup_escalation_mult: float,
    environment_escalation_mult: float,
    daily_context_mult: float,
    ...
    power_gate_mult: float = 1.0,
) -> tuple[float, float, float, int, int, str, str]:
    """
    Second-pass, scorecard-aware context engine.

    The first-pass multiplier only sees generic multipliers. That misses players
    like Brandon Marsh when the raw model labels him as low pitcher-risk but the
    same-day scorecard is screaming: split edge + usable pitch fit + hot park +
    PA volume. This pass uses the actual same-day components that land in the
    board and upgrades only clustered spots.
    """
    reasons = []
    ...
    m_count = int(matchup_signal_count or 0)
    e_count = int(environment_signal_count or 0)
    m_mult = _f(matchup_escalation_mult, 1.0)
    e_mult = _f(environment_escalation_mult, 1.0)
    ctx_mult = _f(daily_context_mult, 1.0)
    flag = str(context_flag or "NONE").upper()

    split_score = _f(matchup_scores.get("split_advantage_score"), 0.0)
    pitch_fit = _f(matchup_scores.get("pitch_mix_fit_score"), 0.0)
    pitcher_vuln = _f(matchup_scores.get("pitcher_vulnerability_score"), 0.0)
    env_score = _f(matchup_scores.get("environment_score"), 0.0)
    opp_score = _f(matchup_scores.get("opportunity_score"), 0.0)
    daily_score = _f(matchup_scores.get("daily_matchup_hr_score"), 0.0)
    venue_l = str(venue or "").lower()
    temp = _f(weather_temp_f, 0.0)
    wind = _f(weather_wind_mph, 0.0)
    wmult = _f(weather_mult, 1.0)
    pf = _f(park_factor, 1.0)
    pa = _f(exp_pa, 4.2)
    pscore = _f(power_score, 0.0)
    pg = _f(power_gate_mult, 1.0)

    # Matchup scorecard signals. Split advantage is a real batter-vs-pitcher edge
    # even if the generic pitcher HR model is cold on the pitcher.
    if split_score >= 65:
        m_count += 1; m_mult += 0.018; reasons.append("split_edge")
    if split_score >= 75:
        m_count += 1; m_mult += 0.012; reasons.append("major_split_edge")
    if pitch_fit >= 45:
        m_count += 1; m_mult += 0.012; reasons.append("usable_pitch_fit")
    if pitch_fit >= 60:
        m_count += 1; m_mult += 0.018; reasons.append("strong_pitch_fit")
    if pitcher_vuln >= 35:
        m_count += 1; m_mult += 0.014; reasons.append("pitcher_vulnerable")
    if daily_score >= 55:
        m_count += 1; m_mult += 0.016; reasons.append("daily_matchup_score")

    # Environment scorecard signals. This intentionally catches Wrigley/Coors/
    # warm-wind days even when the earlier weather direction calculation is too
    # conservative or labels the wind incorrectly.
    is_spike_park = any(k in venue_l for k in ["wrigley", "coors", "great american", "yankee stadium", "citizens bank", "camden", "guaranteed rate", "american family"])
    warm = temp >= 72
    hot = temp >= 80
    windy = wind >= 10
    very_windy = wind >= 15
    if is_spike_park:
        e_count += 1; e_mult += 0.014; reasons.append("spike_park")
    if warm:
        e_count += 1; e_mult += 0.012; reasons.append("warm")
    if hot:
        e_count += 1; e_mult += 0.010; reasons.append("hot")
    if windy:
        e_count += 1; e_mult += 0.012; reasons.append("windy")
    if very_windy:
        e_count += 1; e_mult += 0.010; reasons.append("very_windy")
    if wmult >= 1.015 or pf >= 1.03 or env_score >= 50:
        e_count += 1; e_mult += 0.010; reasons.append("env_score")
    if pa >= 4.1 or opp_score >= 45:
        e_count += 1; e_mult += 0.008; reasons.append("pa_volume")

    # Marsh-style override: do not let a thin seasonal power gate bury a hitter
    # when split edge + environment + playable power all align on that day.
    marsh_style = (split_score >= 65 and pitch_fit >= 42 and (is_spike_park or (warm and windy)) and pscore >= 25 and pa >= 4.0)
    if marsh_style:
        m_count = max(m_count, 3)
        e_count = max(e_count, 2)
        m_mult += 0.025
        e_mult += 0.025
        reasons.append("marsh_style_cluster")
        # Partially free the hitter from a low power gate when the same-day
        # context is exactly the kind of spot the old model flattened.
        if pg < 0.88:
            ctx_mult *= 1.030
            reasons.append("thin_power_escape")

    # Rebuild flag and context multiplier after the added signals.
    if m_count >= 4 and e_count >= 3:
        flag = "PERFECT_STORM"
        ctx_mult *= 1.075
    elif m_count >= 3 and e_count >= 2:
        flag = "STACKED"
        ctx_mult *= 1.050
    elif m_count >= 3 or e_count >= 3:
        flag = "LIVE"
        ctx_mult *= 1.030

    m_mult = float(np.clip(m_mult, 0.94, 1.24))
    e_mult = float(np.clip(e_mult, 0.94, 1.24))
    ctx_mult = float(np.clip(ctx_mult * max(0.96, m_mult) * max(0.96, e_mult) / max(0.96, float(matchup_escalation_mult or 1.0)) / max(0.96, float(environment_escalation_mult or 1.0)), 0.92, 1.38))
    return m_mult, e_mult, ctx_mult, int(m_count), int(e_count), flag, "+".join(reasons) if reasons else "none"
...
def apply_post_calibration_context_boost(
    calibrated_p1: float,
    raw_p1: float,
    context_flag: str,
    matchup_signal_count: int,
    ...
    """
    Controlled post-calibration escape hatch.

    Goal: keep the Marsh-style daily spike recognition, but prevent the top of
    the board from piling into one identical ceiling number. Boosts are now:
      1) graduated by signal strength,
      2) damped near the top via diminishing returns,
      3) capped dynamically instead of slammed into a shared hard cap.

    Returns:
      boosted_probability,
      absolute_boost_added,
      boost_reason,
    ...
    p = float(np.clip(p, 1e-6, 0.999))
    raw = float(np.clip(raw, 1e-6, 0.999))
    flag = str(context_flag or "NONE").upper()

    try:
        m_count = int(matchup_signal_count)
    except Exception:
        m_count = 0
    try:
        e_count = int(environment_signal_count)
    except Exception:
        e_count = 0
    try:
        m_mult = float(matchup_escalation_mult)
    except Exception:
        m_mult = 1.0
    try:
        e_mult = float(environment_escalation_mult)
    except Exception:
        e_mult = 1.0
    try:
        ctx_mult = float(daily_context_mult)
    except Exception:
        ctx_mult = 1.0
    try:
        pg = float(power_gate_mult)
    except Exception:
        pg = 1.0

    reasons = []

    # Weighted signal strength. Matchup gets more weight because HR props are
    # driven heavily by batter-pitcher fit; environment still matters, but does
    # not auto-max everyone in the same park/weather bucket.
    matchup_component = min(1.0, max(0.0, m_count) / 5.0)
    environment_component = min(1.0, max(0.0, e_count) / 4.0)
    mult_component = min(1.0, max(0.0, (m_mult - 1.0) + (e_mult - 1.0) + 0.50 * (ctx_mult - 1.0)) / 0.55)
    strength_factor = float(np.clip(0.50 * matchup_component + 0.30 * environment_component + 0.20 * mult_component, 0.0, 1.0))

    # Base boost by context flag, but scaled by strength. This replaces the old
    # blunt +5/+3.2/+1.8 flat boosts that caused ceiling pileups.
    if flag == "PERFECT_STORM":
        base_boost = 0.040
        reasons.append("perfect_storm_scaled")
    elif flag == "STACKED":
        base_boost = 0.030
        reasons.append("stacked_scaled")
    elif flag == "LIVE":
        base_boost = 0.020
        reasons.append("live_scaled")
    else:
        base_boost = 0.000

    boost = base_boost * (0.45 + 0.55 * strength_factor) if base_boost > 0 else 0.0

    # True spike override, but graduated. The old fixed +3% sent too many names
    # to the cap. This now depends on how deep the combined signal really is.
    if m_count >= 3 and e_count >= 2:
        spike_strength = float(np.clip(((m_count - 2) / 3.0) * 0.60 + ((e_count - 1) / 3.0) * 0.40, 0.0, 1.0))
        spike_boost = 0.010 + 0.020 * spike_strength
        boost += spike_boost
        reasons.append("true_spike_scaled")
    if m_count >= 4 and e_count >= 3:
        boost += 0.006 * strength_factor
        reasons.append("deep_cluster_scaled")
    if m_count >= 5 and e_count >= 4:
        boost += 0.004 * strength_factor
        reasons.append("slate_breaker_scaled")

    # Preserve some raw pre-calibration day-specific edge without letting a raw
    # inflated number dominate the calibrated output.
    raw_gap = max(0.0, raw - p)
    if raw_gap > 0.005 and (m_count >= 3 or e_count >= 3):
        gap_keep = min(0.018, raw_gap * (0.25 + 0.25 * strength_factor))
        boost += gap_keep
        reasons.append("raw_gap_scaled")

    # Small context multiplier kicker with a stricter limit.
    ctx_strength = max(0.0, (m_mult - 1.0) + (e_mult - 1.0) + 0.50 * (ctx_mult - 1.0))
    if ctx_strength > 0 and (m_count + e_count) >= 5:
        boost += min(0.010, ctx_strength * 0.035 * (0.50 + 0.50 * strength_factor))
        reasons.append("context_strength_scaled")

    # Power gate damping. Thin-power one-day escapes can still move, but not the
    # same way as elite power bats with the same environment.
    if pg < 0.82:
        boost *= 0.45
        reasons.append("low_power_gate_damped")
    elif pg < 0.92:
        boost *= 0.75
        reasons.append("power_gate_damped")

    # Diminishing returns near the top. This is the key anti-pileup control:
    # a 19-20% calibrated hitter does not receive the same absolute lift as an
    # 11-13% hitter in a newly recognized spike spot.
    diminishing = float(np.clip(1.0 - (p / 0.255), 0.25, 1.0))
    boost *= diminishing
    reasons.append("diminishing_returns")

    # Dynamic cap: normal flags live around 21.5-22%, elite deep clusters can
    # approach 23.5-24%, but only with high strength. This keeps a ceiling while
    # avoiding one shared cap number across the whole top tier.
    if flag == "PERFECT_STORM" or (m_count >= 4 and e_count >= 3):
        dynamic_cap = 0.222 + 0.018 * strength_factor
    elif flag == "STACKED" or (m_count >= 3 and e_count >= 2):
        dynamic_cap = 0.215 + 0.015 * strength_factor
    elif flag == "LIVE":
        dynamic_cap = 0.205 + 0.012 * strength_factor
    else:
        dynamic_cap = 0.205
    dynamic_cap = float(np.clip(dynamic_cap, 0.205, 0.240))

    unclipped = p + boost
    boosted = float(np.clip(unclipped, 1e-6, dynamic_cap))
    actual_boost = float(max(0.0, boosted - p))
    cap_pressure = float(max(0.0, unclipped - dynamic_cap))
    reason = "+".join(reasons) if reasons else "none"
    return boosted, actual_boost, reason, strength_factor, dynamic_cap, cap_pressure
...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    for g in games:
        ...
        for side in ["away", "home"]:
            ...
            for hid in hitter_ids:
                ...
                matchup_escalation_mult, environment_escalation_mult, daily_context_mult, matchup_signal_count, environment_signal_count, context_flag, context_signal_reasons = refine_daily_context_from_scorecard(
                    matchup_escalation_mult=matchup_escalation_mult,
                    environment_escalation_mult=environment_escalation_mult,
                    daily_context_mult=daily_context_mult,
                    ...
                    weather_mult=w_mult,
                    park_factor=park_contact_mult,
                    exp_pa=exp_pa,
                    power_score=power_score,
                    power_gate_mult=power_gate_mult,
                )
                p_pa_adj = float(np.clip(p_pa_adj * daily_context_mult, 1e-6, 0.40))
                ...
                raw_p1 = p1
                p1_calibrated, calibration_method = calibrate_final_hr_probability(raw_p1)
                p1, post_calibration_boost, post_calibration_boost_reason, boost_strength_factor, dynamic_prob_cap, cap_pressure = apply_post_calibration_context_boost(
                    calibrated_p1=p1_calibrated,
                    raw_p1=raw_p1,
                    context_flag=context_flag,
                    matchup_signal_count=matchup_signal_count,
                    environment_signal_count=environment_signal_count,
                ...
                if post_calibration_boost > 0:
                    calibration_method = f"{calibration_method}+post_context"

                # keep 2+ probability coherent after 1+ calibration and post-context boost
                ...
                rows.append(
                    {
                        ...
                        "environment_signal_count": environment_signal_count,
                        "context_flag": context_flag,
                        "context_signal_reasons": context_signal_reasons,
                        "p_hr_1plus_calibrated": round(float(p1_calibrated), 6),
                        "post_calibration_boost": round(float(post_calibration_boost), 6),
                        "post_calibration_boost_reason": post_calibration_boost_reason,
                        "boost_strength_factor": round(float(boost_strength_factor), 4),
                        "dynamic_prob_cap": round(float(dynamic_prob_cap), 4),
                        "cap_pressure": round(float(cap_pressure), 6),
                        "p_hr_pa": p_pa_adj,
                        "p_hr_1plus_sim": p1,
                        "p_hr_2plus_sim": p2,
    ...
    # 60/40 blend of matchup context and probability to prevent high-sim bats from being buried
...
def enrich_supercomputer_output(board: pd.DataFrame) -> pd.DataFrame:
    ...
    def _norm(s: pd.Series) -> pd.Series:
        ...
        return (s - s.min()) / rng
    board["super_score"] = 45*_norm(matchup_clean)+30*_norm(prob)+10*_norm(pitcher_hr)+7*_norm(pitch_fit_score)+4*_norm(env_clean)+4*_norm(recent_damage)
    board.loc[board["stack_flag"] == "SUPER", "super_score"] += 10
    board.loc[board["stack_flag"] == "LIVE", "super_score"] += 5
    board.loc[elite_pitch_fit, "super_score"] += 2
    board.loc[si_volume & si_fit, "super_score"] += 2
    board = board.sort_values(["super_score", "p_hr_1plus_sim", "matchup_score_clean"], ascending=False).reset_index(drop=True)
...
def write_supercomputer_exports(full_board: pd.DataFrame, date_str: str, top_n: int):
    ...
    os.makedirs("outputs", exist_ok=True)
    preferred_cols = ["super_rank", "prob_rank", "rank", "player_name", "team", "opponent", "game", "venue", "probable_pitcher_faced", "p_hr_1plus_sim", "p_hr_2plus_sim", "super_score", "stack_flag", "stack_signal_count", "matchup_label", "scan_flags", "phone_view", "daily_matchup_hr_score", "matchup_score_clean", "pitch_mix_fit_score", "pitcher_hr_score", "environment_clean", "recent_damage_score", "power_score", "bet_flag_tier", "bet_flag_score", "bet_flag_reason", "power_tier", "matchup_tier", "flags", "display_line", "exp_pa", "matchup_escalation_mult", "environment_escalation_mult", "daily_context_mult", "context_flag", "context_signal_reasons", "p_hr_1plus_calibrated", "post_calibration_boost", "post_calibration_boost_reason", "boost_strength_factor", "dynamic_prob_cap", "cap_pressure", "pitchtype_mult", "pitcher_hr_mult", "weather_mult", "park_factor", "bullpen_score", "date"]
    ...
    if game_col:
        summary_rows = []
        out_cols_master = [c for c in ["super_rank", "prob_rank", "player_name", "team", "probable_pitcher_faced", "p_hr_1plus_sim", "p_hr_1plus_calibrated", "post_calibration_boost", "post_calibration_boost_reason", "boost_strength_factor", "dynamic_prob_cap", "cap_pressure", "context_flag", "context_signal_reasons", "stack_flag", "stack_signal_count", "matchup_label", "scan_flags", "phone_view", "super_score", "daily_matchup_hr_score", "matchup_score_clean", "pitcher_hr_score", "pitch_mix_fit_score", "environment_clean", "power_score", "bet_flag_tier", "bet_flag_reason"] if c in export.columns]
        ...
        for i, (game_name, g) in enumerate(export.groupby(game_col, dropna=False), start=1):
            ...
            for _, tr in top3.iterrows():
                pct = round(float(pd.to_numeric(tr.get("p_hr_1plus_sim"), errors="coerce") * 100.0), 1) if "p_hr_1plus_sim" in top3.columns else 0.0
                ctx = str(tr.get("context_flag", "NONE"))
                stack = str(tr.get("stack_flag", ""))
                flags = str(tr.get("scan_flags", tr.get("flags", "")))
                top3_lines.append(f"{tr.get('player_name','')} ({pct}% | {ctx} | {stack} | {flags})")
            summary_rows.append({
                "game": game_name,
                "top_1": top3_lines[0] if len(top3_lines) > 0 else "",
                "top_2": top3_lines[1] if len(top3_lines) > 1 else "",
                "top_3": top3_lines[2] if len(top3_lines) > 2 else "",
                "top_3_combined": " | ".join(top3_lines),
                "best_stack": ", ".join(top3.loc[top3["stack_flag"].isin(["SUPER", "LIVE", "PERFECT_STORM", "STACKED"]), "player_name"].astype(str).tolist()) if "stack_flag" in top3.columns else "",
                "best_prob": round(float(pd.to_numeric(g2["p_hr_1plus_sim"], errors="coerce").max() * 100.0), 1) if len(g2) and "p_hr_1plus_sim" in g2.columns else 0.0,
            })
...
def write_outputs(board: pd.DataFrame, date_str: str, top_n: int):
    ...
    display_cols = [c for c in ["super_rank", "prob_rank", "player_name", "team", "probable_pitcher_faced", "p_hr_1plus_sim", "stack_flag", "matchup_label", "scan_flags", "phone_view"] if c in show.columns]
    html = f"""
    <html>
    <head>
