...
def exp_bbe_multiplier(k_rate: float | None, exp_pa: float | None) -> float:
    """Penalize fragile contact paths and modestly reward stable contact paths."""
    try:
        k = float(k_rate)
        pa = float(exp_pa)
    except Exception:
...
def recent_power_spike_multiplier(recent_barrel_rate: float | None,
    ...
    """Reward real short-term power spikes without going crazy."""
    try:
    ...
    return float(np.clip(mult, 0.95, 1.15))
...
def sharpened_pitchtype_multiplier(pt_mult: float | None,
    ...
    """
    Let true pitch-match edges show up more strongly near the top of the board.
    Keeps weak edges from exploding.
    """
    try:
        pt = float(pt_mult)
    except Exception:
    ...
    bonus = 1.0
    if usage >= 0.40 and ratio >= 1.10 and pt > 1.0:
        bonus = 1.06
    if usage >= 0.50 and ratio >= 1.20 and pt > 1.0:
        bonus = 1.10

    return float(np.clip(pt * bonus * 1.05, 0.80, 1.35))
...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    preferred_cols = [
        ...
        "recent_barrel_rate", "recent_hard_hit_rate", "recent_ev", "exp_bbe_mult", "power_spike_mult", "cluster_mult", "batter_id", "date"
    ]
