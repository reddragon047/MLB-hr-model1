...
def _sanitize_game_filename(value: str, max_len: int = 120) -> str:
...
def _ensure_game_key(board: pd.DataFrame) -> pd.DataFrame:
    out = board.copy()
    if out is None or out.empty:
        return out

    if "game_pk" in out.columns and out["game_pk"].notna().any():
        out["game_key"] = "gamepk_" + out["game_pk"].astype(str).fillna("").str.strip()
        return out

    if {"date", "away_team", "home_team"}.issubset(out.columns):
        out["game_key"] = (
            out["date"].astype(str).fillna("").str.strip()
            + "_"
            + out["away_team"].astype(str).fillna("").str.strip()
            + "_at_"
            + out["home_team"].astype(str).fillna("").str.strip()
        )
        return out

    if "game" in out.columns:
        g = out["game"].astype(str).fillna("").str.strip()
        if (g != "").any():
            out["game_key"] = g
            return out

    if "venue" in out.columns:
        date_vals = out["date"].astype(str).fillna("").str.strip() if "date" in out.columns else pd.Series([""] * len(out), index=out.index)
        out["game_key"] = date_vals + "_" + out["venue"].astype(str).fillna("UnknownVenue").str.strip()
        return out

    out["game_key"] = "Unknown_Game"
    return out


def write_game_sheets(board: pd.DataFrame, date_str: str):
    if board is None or board.empty:
        return
...
def write_outputs(board: pd.DataFrame, date_str: str, top_n: int):
    ...
    board.to_csv(f"outputs/hr_board_full_slate_{date_str}.csv", index=False)
    write_game_sheets(board.copy(), date_str=date_str)
...
def main():
    ...
    print("\nTop 25 from full slate (by HR probability):")
