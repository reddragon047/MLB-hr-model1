...
MODEL_VERSION = "v4_power_shape_xgb_2026_04_16"
...
def ensure_recent_feature_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Ensure base and recent feature columns always exist so training/inference never crash."""
    base_defaults = {
        "barrel_rate": 0.0,
        "avg_ev": 0.0,
        "avg_la": 0.0,
...
def batter_pitcher_tables(stat_df: pd.DataFrame, season: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    bbe["is_barrel_proxy"] = ((bbe["launch_speed"] >= 98) & (bbe["launch_angle"].between(26, 30))).astype(int)
    bbe["is_hard_hit"] = (bbe["launch_speed"] >= 95).astype(int)
    bbe["is_fb_proxy"] = ((bbe["launch_angle"] >= 25) & (bbe["launch_angle"] <= 50)).astype(int)
    bbe["is_sweet_spot"] = ((bbe["launch_angle"] >= 8) & (bbe["launch_angle"] <= 32)).astype(int)
    bbe["is_air_power"] = ((bbe["launch_speed"] >= 95) & (bbe["launch_angle"] >= 20) & (bbe["launch_angle"] <= 40)).astype(int)
    ...
    bat_bbe = bbe.groupby(["batter", "season"]).agg(
        ...
        flyball_rate=("is_fb_proxy", "mean"),
        sweet_spot_rate=("is_sweet_spot", "mean"),
        air_power_rate=("is_air_power", "mean"),
    ).reset_index()
    ...
    pit_bbe = bbe.groupby(["pitcher", "season"]).agg(
        ...
        flyball_rate_allowed=("is_fb_proxy", "mean"),
        sweet_spot_rate_allowed=("is_sweet_spot", "mean"),
        air_power_rate_allowed=("is_air_power", "mean"),
    ).reset_index()
...
def get_training_cached(train_seasons: list[int]) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if all(os.path.exists(p) for p in [bat_path, pit_path, mix_path, dmg_path, park_path, bp_path, platoon_path]):
        ...
        missing_bat = [c for c in FEATURE_COLS if c not in bat_cached.columns]
        required_pit = [
            "hr_rate_allowed",
            "barrel_rate_allowed",
            "avg_ev_allowed",
            ...
            "flyball_rate_allowed",
            "sweet_spot_rate_allowed",
            "air_power_rate_allowed",
        ]
        missing_pit = [c for c in required_pit if c not in pit_cached.columns]
        if not missing_bat and not missing_pit:
            return (
                bat_cached,
                pit_cached,
        ...
        print(f"[DEBUG] Rebuilding cached training tables due to missing columns. bat_missing={missing_bat} pit_missing={missing_pit}")
