...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    # Harpoon sharpening layer
    if "recent_damage_score" not in df.columns:
        df["recent_damage_score"] = 0.0
    if "recent_barrel_rate" not in df.columns:
        df["recent_barrel_rate"] = 0.0

    df["final_score"] = (
        pd.to_numeric(df["p_hr_1plus_sim"], errors="coerce").fillna(0.0) * 0.60 +
        pd.to_numeric(df["env_score"], errors="coerce").fillna(0.0) * 0.25 +
        (pd.to_numeric(df["recent_damage_score"], errors="coerce").fillna(0.0) / 100.0) * 0.15
    )

    df["elite_flag"] = (
        (pd.to_numeric(df["p_hr_1plus_sim"], errors="coerce").fillna(0.0) >= 0.23) &
        (pd.to_numeric(df["env_score"], errors="coerce").fillna(0.0) >= 1.00)
    )

    df["spike_flag"] = (
        (pd.to_numeric(df["recent_barrel_rate"], errors="coerce").fillna(0.0) >= 0.12) |
        (pd.to_numeric(df["recent_damage_score"], errors="coerce").fillna(0.0) >= 40.0)
    )

    board = board.sort_values(["final_score", "daily_matchup_hr_score", "p_hr_1plus_sim", "p_hr_pa"], ascending=False).reset_index(drop=True)
    ...
    preferred_cols = [
        ...
        "final_score", "elite_flag", "spike_flag",
...
def force_sequential_ranks(board: pd.DataFrame) -> pd.DataFrame:
    if board is None or board.empty:
        return board
    sort_cols = ["final_score", "p_hr_pa"] if "final_score" in board.columns else ["p_hr_1plus_sim", "p_hr_pa"]
    board = board.sort_values(sort_cols, ascending=False).reset_index(drop=True)
    if "rank" in board.columns:
        board = board.drop(columns=["rank"])
    board.insert(0, "rank", np.arange(1, len(board) + 1))
    return board
...
def build_mobile_views(board: pd.DataFrame) -> dict:
    ...
    if "env_score" not in b.columns:
        if "environment_score" in b.columns:
            env_raw = pd.to_numeric(b["environment_score"], errors="coerce").fillna(100.0)
            b["env_score"] = (env_raw / 100.0).clip(lower=0.85, upper=1.15)
        else:
            b["env_score"] = 1.0

    if "recent_damage_score" not in b.columns:
        b["recent_damage_score"] = 0.0

    if "final_score" not in b.columns:
        b["final_score"] = (
            pd.to_numeric(b.get("p_hr_1plus_sim", 0.0), errors="coerce").fillna(0.0) * 0.60 +
            pd.to_numeric(b["env_score"], errors="coerce").fillna(1.0) * 0.25 +
            (pd.to_numeric(b["recent_damage_score"], errors="coerce").fillna(0.0) / 100.0) * 0.15
        )

    if "elite_flag" not in b.columns:
        b["elite_flag"] = (
            (pd.to_numeric(b.get("p_hr_1plus_sim", 0.0), errors="coerce").fillna(0.0) >= 0.23) &
            (pd.to_numeric(b["env_score"], errors="coerce").fillna(1.0) >= 1.00)
        )

    if "recent_barrel_rate" not in b.columns:
        b["recent_barrel_rate"] = 0.0

    if "spike_flag" not in b.columns:
        b["spike_flag"] = (
            (pd.to_numeric(b["recent_barrel_rate"], errors="coerce").fillna(0.0) >= 0.12) |
            (pd.to_numeric(b["recent_damage_score"], errors="coerce").fillna(0.0) >= 40.0)
        )
    ...
    decision_cols = [
        "rank", "tier_bucket", "player_name", "team", "probable_pitcher_faced",
        "venue", "p_hr_1plus_sim", "p_hr_2plus_sim", "p_hr_pa", "final_score",
        "elite_flag", "spike_flag", "env_score", "env_label", "power_score",
        "daily_matchup_hr_score", "matchup_score_today", "recent_damage_score",
        "bet_flag_tier", "bet_flag_reason"
    ]
    decision_cols = [c for c in decision_cols if c in b.columns]
    decision_board = b[decision_cols].copy()

    compact_cols = [c for c in [
        "rank", "player_name", "team", "p_hr_1plus_sim", "final_score",
        "env_score", "env_label", "elite_flag", "spike_flag"
    ] if c in decision_board.columns]
    compact_top35 = decision_board[decision_board["rank"] <= 35][compact_cols].copy()

    tier1 = decision_board[decision_board["rank"] <= 15].copy()
    tier2 = decision_board[(decision_board["rank"] >= 16) & (decision_board["rank"] <= 35)].copy()
    tier3 = decision_board[(decision_board["rank"] >= 36) & (decision_board["rank"] <= 60)].copy()
...
def write_outputs(board: pd.DataFrame, date_str: str, top_n: int):
    ...
    # legacy full board outputs
    board.to_csv(csv_path, index=False)
    board.to_csv("hr_board.csv", index=False)

    # phone-friendly outputs at artifact root AND inside outputs/
