...
def get_game_weather(lat: float, lon: float, date_str: str, game_hour_local: int | None = None) -> dict:
    ...
    if best_idx is None:
        return {
            ...
            "rain_mm": None,
            "snowfall_cm": None,
            "weather_status": "NO_MATCHING_HOUR",
        }
...
def compute_weather_multiplier(loc: dict | None, home_team: str, game_datetime: str | None, date_str: str) -> tuple[float, dict]:
    ...
    # Strong weather build: combine temp + oriented wind with a wider but still safe cap.
    final_mult = float(np.clip(t_mult * w_mult, 0.58, 1.34))

    # Precip/cold penalties should be strong; bad hitting weather should actually move the board.
    ...
    # Correct directional bonus/penalty with cosine component.
    wind_component = None
    if wind_speed is not None and wind_dir is not None and out_to_cf_deg is not None:
        try:
            diff = abs(((float(wind_dir) - float(out_to_cf_deg) + 180.0) % 360.0) - 180.0)
            wind_component = float(np.cos(np.deg2rad(diff)))
            # Extra nudge when wind is meaningful and direction is clear.
            if float(wind_speed) >= 8:
                if wind_component >= 0.45:
                    final_mult *= 1.0 + min(0.08, 0.006 * float(wind_speed) * wind_component)
                elif wind_component <= -0.45:
                    final_mult *= 1.0 - min(0.09, 0.0065 * float(wind_speed) * abs(wind_component))
        except Exception:
            wind_component = None

    final_mult = float(np.clip(final_mult, 0.52, 1.38))

    # Human-readable weather tag / impact score for the board.
    ...
    weather.update({
        ...
        "weather_status": weather.get("weather_status", "HOURLY_OK"),
    })
