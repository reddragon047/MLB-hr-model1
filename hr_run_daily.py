...
def _first_nonnull(df: pd.DataFrame, cols, default=np.nan):
    for c in cols:
        if c in df.columns:
            ser = df[c].dropna()
            if not ser.empty:
...
def _make_game_key_cols(board: pd.DataFrame) -> pd.DataFrame:
    out = board.copy()
    if 'game_key' in out.columns and out['game_key'].astype(str).str.strip().ne('').any():
        out['_game_key'] = out['game_key'].astype(str).str.strip()
        return out

    venue = out['venue'] if 'venue' in out.columns else pd.Series('Unknown Venue', index=out.index)
    venue = venue.fillna('Unknown Venue').astype(str)

    if 'team' in out.columns:
        team_block = out.groupby(venue)['team'].transform(
            lambda s: ' at '.join(sorted({str(x).strip() for x in s.dropna() if str(x).strip()})) or 'Unknown Teams'
        )
    else:
        team_block = pd.Series('Unknown Teams', index=out.index)

    out['_game_key'] = (venue + ' | ' + team_block.astype(str)).str.replace(r'\s+', ' ', regex=True).str.strip()
    return out
...
def export_game_scouting_sheets(board: pd.DataFrame, date_str: str) -> None:
    ...
    df = _make_game_key_cols(board)
    sort_cols = [c for c in ['_game_key', 'rank', 'prob_rank', 'p_hr_1plus_sim'] if c in df.columns]
    asc = [True] * len(sort_cols)
    if 'p_hr_1plus_sim' in sort_cols:
        asc[sort_cols.index('p_hr_1plus_sim')] = False
    if sort_cols:
    ...
    try:
        ...
        print(f'[game_sheets] workbook disabled: {e}')
        writer = None

    index_rows = []
    for i, (game_key, g) in enumerate(df.groupby('_game_key', dropna=False), start=1):
        g = g.copy().reset_index(drop=True)
        venue = _first_nonnull(g, ['venue'], 'Unknown Venue')
        teams = sorted({str(x).strip() for x in g['team'].dropna().tolist()}) if 'team' in g.columns else []
        away = teams[0] if len(teams) >= 1 else 'Away'
        home = teams[1] if len(teams) >= 2 else 'Home'
        slug = _safe_slug(f'{away}_at_{home}_{date_str}')
        ...
        env_row = {
            ...
            'home_team': home,
            'venue': venue,
            'weather_temp_f': _first_nonnull(g, ['weather_temp_f', 'temp_f', 'temperature_f']),
            'weather_wind_mph': _first_nonnull(g, ['weather_wind_mph', 'wind_speed_mph', 'wind_mph']),
            'weather_wind_dir_deg': _first_nonnull(g, ['weather_wind_dir_deg', 'wind_dir_deg']),
            'wind_impact_label': _first_nonnull(g, ['wind_impact_label', 'wind_direction_label', 'wind_dir_label'], ''),
            'park_factor': _first_nonnull(g, ['park_factor']),
            'weather_mult': _first_nonnull(g, ['weather_mult']),
            'game_time': _first_nonnull(g, ['game_time', 'start_time', 'game_datetime'], ''),
        }

        rows = [env_row]

        if {'team', 'probable_pitcher_faced'}.issubset(g.columns):
            pitch_agg = g.groupby(['team', 'probable_pitcher_faced'], dropna=False).agg(
                hitters_in_sheet=('player_name', 'count'),
                avg_rank=('rank', 'mean') if 'rank' in g.columns else ('player_name', 'count'),
                avg_prob=('p_hr_1plus_sim', 'mean') if 'p_hr_1plus_sim' in g.columns else ('player_name', 'count'),
                max_prob=('p_hr_1plus_sim', 'max') if 'p_hr_1plus_sim' in g.columns else ('player_name', 'count'),
                pitcher_hr_score=('pitcher_hr_score', 'mean') if 'pitcher_hr_score' in g.columns else ('player_name', 'count'),
                bullpen_score=('bullpen_score', 'mean') if 'bullpen_score' in g.columns else ('player_name', 'count'),
            ).reset_index()
            for _, r in pitch_agg.iterrows():
                rows.append({
                    ...
                    'avg_hr_prob_pct': round(float(r.get('avg_prob', np.nan))*100, 2) if pd.notna(r.get('avg_prob', np.nan)) else np.nan,
                    'max_hr_prob_pct': round(float(r.get('max_prob', np.nan))*100, 2) if pd.notna(r.get('max_prob', np.nan)) else np.nan,
                    'pitcher_hr_score': r.get('pitcher_hr_score'),
                    'bullpen_score': r.get('bullpen_score'),
                })

        hitter_cols = [c for c in [
            'team','player_name','probable_pitcher_faced','rank','prob_rank','p_hr_1plus_sim','matchup_label','matchup_score_clean',
            'pitcher_hr_score','pitch_mix_fit_score','bullpen_score','recent_damage_score','dominant_pitch','dominant_pitch_usage',
            'batter_vs_pitch_ratio','bet_flag','bet_flag_reason','venue','weather_temp_f','weather_wind_mph','wind_impact_label'
        ] if c in g.columns]

        hitters = g[hitter_cols].copy()
        if 'p_hr_1plus_sim' in hitters.columns:
            hitters['hr_prob_pct'] = (pd.to_numeric(hitters['p_hr_1plus_sim'], errors='coerce')*100).round(2)
            hitters = hitters.drop(columns=['p_hr_1plus_sim'])
        hitters.insert(0, 'section', 'HITTER')
        hitters.insert(1, 'game_key', game_key)

        sheet_df = pd.concat([pd.DataFrame(rows), hitters], ignore_index=True, sort=False)

        top_csv = os.path.join('outputs', f'hr_game_sheet_{slug}.csv')
        nested_csv = os.path.join('outputs', 'game_sheets', f'hr_game_sheet_{slug}.csv')
        sheet_df.to_csv(top_csv, index=False)
        sheet_df.to_csv(nested_csv, index=False)
        ...
        index_rows.append({
            ...
            'home_team': home,
            'venue': venue,
            'top_level_csv': os.path.basename(top_csv),
            'nested_csv': os.path.relpath(nested_csv, 'outputs'),
            'rows': len(sheet_df),
        })

        if writer is not None:
            sheet_name = _safe_slug(f'{away}_at_{home}')[:31] or f'Game_{i}'
    ...
    if writer is not None:
        try:
            ...
            print(f'[game_sheets] workbook close warning: {e}')

    if index_rows:
        pd.DataFrame(index_rows).to_csv(os.path.join('outputs', f'hr_game_sheet_index_{date_str}.csv'), index=False)


def export_game_scouting_sheets_from_outputs(date_str: str) -> int:
    """Fallback exporter: rebuild per-game sheets from the written full-slate CSV."""
    candidates = [
        os.path.join('outputs', f'hr_board_full_slate_{date_str}.csv'),
        os.path.join('outputs', f'hr_board_{date_str}.csv'),
    ]
    src = next((fp for fp in candidates if os.path.exists(fp)), None)
    if src is None:
        print(f"[game_sheets] fallback source missing for {date_str}")
        return 0

    try:
        df = safe_read_csv(src)
    except Exception as e:
        print(f"[game_sheets] fallback read failed: {e}")
        return 0

    if df is None or df.empty:
        print(f"[game_sheets] fallback source empty: {src}")
        return 0

    export_game_scouting_sheets(df, date_str)
    created = 0
    try:
        created = len([x for x in os.listdir('outputs') if x.startswith('hr_game_sheet_') and x.endswith(f'_{date_str}.csv')])
    except Exception:
        created = 0
    return created
...
def main():
    ...
    created_game_sheets = 0
    try:
        export_game_scouting_sheets(board, args.date)
        created_game_sheets = len([x for x in os.listdir('outputs') if x.startswith('hr_game_sheet_') and x.endswith(f'_{args.date}.csv')])
        if created_game_sheets:
            print(f"[game_sheets] created {created_game_sheets} per-game CSV sheets.")
    except Exception as e:
        print(f"[game_sheets] primary export warning: {e}")

    if created_game_sheets == 0:
        try:
            created_game_sheets = export_game_scouting_sheets_from_outputs(args.date)
            if created_game_sheets:
                print(f"[game_sheets] fallback created {created_game_sheets} per-game CSV sheets.")
            else:
                print(f"[game_sheets] no per-game sheets were created for {args.date}.")
        except Exception as e:
            print(f"[game_sheets] fallback export warning: {e}")
