...
# Production hardening: some helper modules / old output blocks still sort on legacy
# column names (especially game_date). Do not let an optional export/helper kill the
# entire HR board just because one legacy alias is missing.
_original_df_sort_values = pd.DataFrame.sort_values

def safe_sort_values(self, by=None, *args, **kwargs):
    """Sort defensively: legacy/optional exports may ask for columns not present.
    Missing sort keys are added as neutral NaN columns instead of killing the run.
    """
    by_list = list(by) if isinstance(by, (list, tuple, pd.Index)) else [by]
    tmp = self
    added = []
    for key in by_list:
        if key is None:
        ...
        if key not in tmp.columns:
            if tmp is self:
                tmp = self.copy()
            if key == "game_date" and "date" in tmp.columns:
                tmp[key] = tmp["date"].astype(str)
            elif key == "game_pk":
                tmp[key] = np.nan
            else:
                tmp[key] = np.nan
    ...
    if added:
        print(f"[production_guard] sort_values missing {added}; created fallback columns and continued.")
    return _original_df_sort_values(tmp, by=by, *args, **kwargs)

pd.DataFrame.sort_values = safe_sort_values


def ensure_board_schema(df: pd.DataFrame | None, run_date: str | None = None) -> pd.DataFrame:
    """Guarantee legacy aliases and output-critical columns exist before any export/helper."""
    if df is None:
        return pd.DataFrame()
    out = df.copy()
    date_val = str(run_date or date_cls.today())
    if "date" not in out.columns:
        out["date"] = date_val
    out["date"] = out["date"].fillna(date_val).astype(str)
    if "game_date" not in out.columns:
        out["game_date"] = out["date"]
    else:
        out["game_date"] = out["game_date"].fillna(out["date"]).astype(str)
    if "player_name" not in out.columns:
        out["player_name"] = ""
    if "batter_id" not in out.columns:
        out["batter_id"] = np.nan
    # Some slate/output paths do not carry MLB game_pk. Keep a harmless alias so
    # grouping/sorting/export helpers never crash when the schedule payload omits it.
    ...
    for col, default in {
        ...
        "p_hr_pa": 0.0,
        "power_gate_mult": 1.0,
        "daily_matchup_hr_score": 0.0,
        "matchup_score_clean": 50.0,
...
# ===== CORE SAFE HELPERS (kept above all callers) =====
def temp_multiplier(temp_f):
    """
    Stronger temperature-based HR environment multiplier.
    This intentionally makes game-time temperature matter more than the prior build,
    but remains capped so weather can sharpen the board without taking it over.
    """
    try:
        if temp_f is None or (isinstance(temp_f, float) and np.isnan(temp_f)):
            return 1.0
        temp_f = float(temp_f)
    except Exception:
        return 1.0

    # Stronger hot-air boost / cold-air suppression, based on GAME-TIME temp.
    if temp_f >= 95:
        return 1.16
    if temp_f >= 88:
        return 1.12
...
def wind_multiplier(wind_speed, wind_dir, out_to_cf_deg):
    """
    Stronger wind multiplier using park orientation.
    - diff near 0 degrees = wind blowing out toward CF/OF = boost
    - diff near 180 degrees = wind blowing in = suppression
    - crosswind = small effect only
    """
    try:
        if wind_speed is None or wind_dir is None or out_to_cf_deg is None:
            return 1.0
        wind_speed = max(0.0, float(wind_speed))
        wind_dir = float(wind_dir)
        out_to_cf_deg = float(out_to_cf_deg)
    except Exception:
        return 1.0

    diff = abs(((wind_dir - out_to_cf_deg + 180.0) % 360.0) - 180.0)
    # Component is +1 blowing out, 0 cross, -1 blowing in.
    out_component = float(np.cos(np.deg2rad(diff)))

    # Stronger than old build, but capped. A 12 mph true out wind is about +12% to +16%.
    if out_component >= 0.35:
        return 1.0 + min(0.18, 0.0125 * wind_speed * out_component)
    if out_component <= -0.35:
        return 1.0 - min(0.20, 0.0135 * wind_speed * abs(out_component))

    # Crosswind: slight directional drag only, never a major signal.
    return 1.0 + float(np.clip(0.0025 * wind_speed * out_component, -0.025, 0.025))


def sharpened_pitchtype_multiplier(pt_mult, dominant_pitch_usage, batter_vs_pitch_ratio):
    """
    Keep pitch-match signal useful without letting tiny-sample spikes blow up the board.
    """
    try:
    ...
    try:
        usage = float(dominant_pitch_usage or 0.0)
    except Exception:
        usage = 0.0
    try:
        ratio = float(batter_vs_pitch_ratio or 1.0)
    except Exception:
        ratio = 1.0

    # Confidence blend: stronger only if pitch is thrown enough and batter truly profiles well against it.
    confidence = 0.45 + min(0.35, max(0.0, usage - 0.20))
    confidence += min(0.20, max(0.0, ratio - 1.00) * 0.25)
    confidence = float(np.clip(confidence, 0.45, 0.90))
    blended = 1.0 + (pt_mult - 1.0) * confidence
    return float(np.clip(blended, 0.90, 1.20))


def exp_bbe_multiplier(k_rate, exp_pa):
    """
    Penalize poor BBE opportunity; modest bonus for contact + volume.
    """
    try:
        k_rate = float(k_rate)
    except Exception:
    ...
    return float(np.clip(mult, 0.90, 1.08))


def recent_power_spike_multiplier(recent_barrel_rate, recent_hard_hit_rate, recent_ev):
    """
    Small additive-style multiplier for hitters showing real recent damage.
    """
    try:
        rbr = float(recent_barrel_rate or 0.0)
    except Exception:
        rbr = 0.0
    try:
        rhh = float(recent_hard_hit_rate or 0.0)
    except Exception:
        rhh = 0.0
    try:
        rev = float(recent_ev or 0.0)
    except Exception:
        rev = 0.0
    ...
    return float(np.clip(mult, 0.94, 1.10))



def _signal_strength(mult, mild=1.02, solid=1.05, strong=1.08, neg_mild=0.98, neg_solid=0.95):
    try:
        mult = float(mult)
    except Exception:
        return 0
...
def compute_daily_context_multipliers(
    pt_mult: float,
    ...
    bp_mult: float,
    exp_bbe_mult: float,
    power_spike_mult: float,
    exp_pa: float,
    power_gate_mult: float,
) -> tuple[float, float, float, int, int, str]:
    """
    Let same-day context matter more when multiple independent signals line up.

    Returns:
      matchup_escalation_mult,
      environment_escalation_mult,
      overall_context_mult,
      matchup_signal_count,
      env_signal_count,
      context_flag
    """
    matchup_signals = {
        "pitch_mix": _signal_strength(pt_mult, mild=1.02, solid=1.05, strong=1.08),
        "recent_form": _signal_strength(recent_form_mult, mild=1.015, solid=1.03, strong=1.05),
        "pitcher_hr": _signal_strength(pitcher_hr_mult, mild=1.02, solid=1.04, strong=1.07),
        "platoon": _signal_strength(platoon_mult, mild=1.02, solid=1.05, strong=1.08),
        "damage_spike": _signal_strength(power_spike_mult, mild=1.015, solid=1.03, strong=1.05),
    }
    env_signals = {
        "park_weather": _signal_strength(env_mult, mild=1.03, solid=1.07, strong=1.12),
        "weather": _signal_strength(weather_mult, mild=1.02, solid=1.05, strong=1.09),
        "bullpen": _signal_strength(bp_mult, mild=1.015, solid=1.03, strong=1.05),
        "bbe": _signal_strength(exp_bbe_mult, mild=1.01, solid=1.025, strong=1.045),
        "pa": 2 if exp_pa >= 4.9 else 1 if exp_pa >= 4.6 else -1 if exp_pa <= 3.6 else 0,
    }

    matchup_count = int(sum(v > 0 for v in matchup_signals.values()))
    env_count = int(sum(v > 0 for v in env_signals.values()))
    matchup_neg = int(sum(v < 0 for v in matchup_signals.values()))
    env_neg = int(sum(v < 0 for v in env_signals.values()))
    matchup_strength = int(sum(max(v, 0) for v in matchup_signals.values()))
    env_strength = int(sum(max(v, 0) for v in env_signals.values()))

    matchup_escalation = 1.0 + 0.010 * matchup_count + 0.004 * matchup_strength
    if matchup_count >= 3:
        matchup_escalation += 0.020
    if matchup_count >= 4:
        matchup_escalation += 0.015
    if matchup_strength >= 8:
        matchup_escalation += 0.015
    if matchup_neg >= 2:
        matchup_escalation -= 0.020
    matchup_escalation = float(np.clip(matchup_escalation, 0.96, 1.18))

    environment_escalation = 1.0 + 0.010 * env_count + 0.004 * env_strength
    if env_count >= 3:
        environment_escalation += 0.020
    if env_count >= 4:
        environment_escalation += 0.015
    if env_strength >= 8:
        environment_escalation += 0.015
    if env_neg >= 2:
        environment_escalation -= 0.020
    environment_escalation = float(np.clip(environment_escalation, 0.96, 1.18))

    overall = matchup_escalation * environment_escalation
    if matchup_count >= 3 and env_count >= 3 and power_gate_mult >= 0.98:
        overall *= 1.035
        flag = 'PERFECT_STORM'
    elif matchup_count >= 4 or env_count >= 4:
        overall *= 1.015
        flag = 'STACKED'
    elif matchup_count >= 3 or env_count >= 3:
        flag = 'LIVE'
    else:
        flag = 'NONE'

    overall = float(np.clip(overall, 0.93, 1.28))
    return matchup_escalation, environment_escalation, overall, matchup_count, env_count, flag

def _relative_bucket(vals: pd.Series, high_q=0.75, med_q=0.40, labels=("HIGH","MED","LOW")):
    vals = pd.to_numeric(vals, errors="coerce")
    if vals.notna().sum() == 0:
        return pd.Series([labels[1]] * len(vals), index=vals.index)
    hi = vals.quantile(high_q)
    med = vals.quantile(med_q)
    return pd.Series(
        np.where(vals >= hi, labels[0], np.where(vals >= med, labels[1], labels[2])),
        index=vals.index
    )
...
def apply_power_context_probability_layer(
    calibrated_p1: float,
    *,
    power_score: float,
    matchup_score: float,
    matchup_signal_count: int,
    environment_signal_count: int,
    context_flag: str,
    power_gate_mult: float,
) -> dict:
    """
    Final post-calibration probability layer.

    Purpose:
      * Keep the calibrator as the long-run reality check.
      * Let true same-day spike spots escape after calibration.
      * Prevent context-only bats from jumping elite power bats too easily.
      * Prevent the top of the board from piling up at one hard cap.
    """
    calibrated_p1 = float(np.clip(calibrated_p1, 1e-6, 0.40))
    power_norm = float(np.clip(float(power_score) / 100.0, 0.0, 1.0))
    context_norm_raw = float(np.clip(float(matchup_score) / 100.0, 0.0, 1.0))

    # Context can matter, but it cannot totally outrun the hitter's actual HR skill.
    context_norm = float(min(context_norm_raw, power_norm * 1.80 + 0.10))

    signal_strength = float(np.clip((0.60 * int(matchup_signal_count) + 0.40 * int(environment_signal_count)) / 5.0, 0.0, 1.15))
    if signal_strength > 0:
        signal_strength = max(signal_strength, 0.35)

    flag = str(context_flag or "NONE").upper()
    base_boost = 0.0
    if flag == "PERFECT_STORM":
        base_boost = 0.032
    elif flag == "STACKED":
        base_boost = 0.023
    elif flag == "LIVE":
    ...
    # Diminishing returns: a player already near the top gets less extra lift.
    diminishing = float(np.clip(1.0 - calibrated_p1 / 0.245, 0.20, 1.00))
    boost = base_boost * signal_strength * diminishing

    spike_boost = 0.0
    if int(matchup_signal_count) >= 3 and int(environment_signal_count) >= 2 and power_norm >= 0.30:
        spike_boost = 0.012 * signal_strength * diminishing
    if int(matchup_signal_count) >= 4 and int(environment_signal_count) >= 3 and power_norm >= 0.40:
        spike_boost += 0.006 * diminishing

    # Elite power hitters should not disappear completely just because the spot is ugly.
    # This is a floor, not a boost; it only helps when calibration/context pushes too low.
    power_floor = 0.0
    if power_norm >= 0.45:
        power_floor = float(np.clip(0.045 + 0.115 * (power_norm ** 1.25), 0.070, 0.150))

    # Dynamic cap: better signal + better power earns a little more ceiling.
    dynamic_cap = float(np.clip(0.205 + 0.022 * signal_strength + 0.018 * power_norm, 0.205, 0.238))
    if flag == "NONE":
        dynamic_cap = min(dynamic_cap, 0.220)
    if float(power_gate_mult) < 0.80 and power_norm < 0.35:
        boost *= 0.70
        spike_boost *= 0.60
        dynamic_cap = min(dynamic_cap, 0.185)

    proposed = calibrated_p1 + boost + spike_boost
    if power_floor > 0:
        proposed = max(proposed, power_floor)

    final_p = float(min(proposed, dynamic_cap))
    cap_pressure = float(max(0.0, proposed - dynamic_cap))

    reasons = []
    if boost > 0:
        reasons.append(f"{flag.lower()}_scaled")
    if spike_boost > 0:
        reasons.append("true_spike_cluster")
    if power_floor > calibrated_p1 and final_p >= power_floor - 1e-9:
        reasons.append("power_anchor_floor")
    if cap_pressure > 0:
        reasons.append("soft_cap_limited")

    return {
        "p_final": final_p,
        "p_calibrated": calibrated_p1,
        "post_calibration_boost": float(max(0.0, final_p - calibrated_p1)),
        "post_calibration_boost_reason": ";".join(reasons) if reasons else "none",
        "power_context_blend_score": float(100.0 * (0.45 * power_norm + 0.55 * context_norm)),
        "power_anchor_floor": power_floor,
        "dynamic_prob_cap": dynamic_cap,
        "cap_pressure": cap_pressure,
        "boost_strength_factor": signal_strength,
        "context_norm_clamped": context_norm,
    }
...
# Small in-run cache so one bad network hiccup does not hammer the weather API repeatedly.
_WEATHER_CACHE: dict[tuple, dict] = {}

# Stadiums where outdoor weather should not drive HR probability. If a roof-state feed is
# available later, these can be switched from neutral to open/closed by game.
ROOF_OR_DOME_TEAMS = {
    "Arizona Diamondbacks", "Houston Astros", "Miami Marlins", "Milwaukee Brewers",
    "Seattle Mariners", "Tampa Bay Rays", "Toronto Blue Jays", "Texas Rangers",
}
...
def _local_game_datetime(game_datetime: str | None, api_timezone: str | None):
    """Convert MLB/StatsAPI UTC gameDate into the forecast timezone."""
    if not game_datetime:
        return None
    try:
        ...
        if api_timezone:
            try:
                return dt.tz_convert(api_timezone)
            except Exception:
...
def _nearest_hour_indices(hours, target_dt, game_window_hours: int = 3):
    if not hours:
        return []
    parsed = []
    for i, ts in enumerate(hours):
        try:
            hdt = pd.to_datetime(ts, errors="coerce")
            if pd.isna(hdt):
                continue
            if getattr(target_dt, "tzinfo", None) is not None:
                tdt = target_dt.tz_localize(None) if hasattr(target_dt, "tz_localize") else target_dt.replace(tzinfo=None)
            else:
                tdt = target_dt
            dist_hours = abs((hdt.to_pydatetime() - pd.Timestamp(tdt).to_pydatetime()).total_seconds()) / 3600.0
            parsed.append((dist_hours, i))
        except Exception:
            continue
    parsed.sort(key=lambda x: x[0])
    picked = [i for d, i in parsed if d <= max(1.5, game_window_hours / 2.0)]
    if not picked and parsed:
        picked = [parsed[0][1]]
    return picked[:max(1, game_window_hours)]
...
def _vector_mean_direction_from_to_toward(deg_from_values):
    """Open-Meteo wind direction is direction wind comes FROM.
    HR orientation needs where it is blowing TOWARD, so convert +180 after averaging.
    """
    vals = []
    for v in deg_from_values:
        try:
            if v is not None:
    ...
    from_deg = (np.rad2deg(np.arctan2(y, x)) + 360.0) % 360.0
    toward_deg = (from_deg + 180.0) % 360.0
    return from_deg, toward_deg


def get_game_weather(lat: float, lon: float, date_str: str, game_datetime: str | None = None, game_hour_local: int | None = None) -> dict:
    """Reliable hourly game-weather pull using Open-Meteo true hourly data.

    Upgrades:
    - pulls hourly temp, apparent temp, humidity, pressure, wind speed/gusts/direction, precip
    - converts MLB UTC first pitch to stadium-local forecast timezone
    - averages a 3-hour game window instead of grabbing one possibly wrong hour
    - converts wind direction FROM -> TOWARD before comparing with park orientation
    - caches results during the run
    """
    key = (round(float(lat), 4), round(float(lon), 4), str(date_str), str(game_datetime or game_hour_local or ""))
    if key in _WEATHER_CACHE:
        return dict(_WEATHER_CACHE[key])

    url = (
        "https://api.open-meteo.com/v1/forecast"
        f"?latitude={lat}&longitude={lon}"
        "&hourly=temperature_2m,apparent_temperature,relative_humidity_2m,surface_pressure,"
        "windspeed_10m,windgusts_10m,winddirection_10m,precipitation,rain,snowfall,weather_code"
    ...
    try:
        ...
        for _attempt in range(3):
            try:
                r = requests.get(url, timeout=15, headers={"User-Agent": "Mozilla/5.0 SSC-HR-Model/1.0"})
                r.raise_for_status()
                j = r.json()
                break
                ...
                last_err = exc
        if j is None:
            raise last_err or RuntimeError("weather request failed")
    except Exception:
        out = {
            "temp_f": None, "feels_like_f": None, "humidity_pct": None, "pressure_hpa": None,
            "wind_speed_mph": None, "wind_gust_mph": None, "wind_dir_from_deg": None, "wind_dir_deg": None,
            "precip_mm": None, "rain_mm": None, "snowfall_cm": None, "weather_code": None,
            "weather_status": "API_FAIL", "weather_source": "open-meteo-hourly",
        }
        _WEATHER_CACHE[key] = dict(out)
        return out

    api_tz = j.get("timezone")
    daily_temp_max = daily_precip = None
    try:
        daily = j.get("daily", {}) or {}
        if daily.get("temperature_2m_max"):
            daily_temp_max = float(daily["temperature_2m_max"][0])
    ...
    if not hours:
        out = {
            "temp_f": daily_temp_max, "feels_like_f": None, "humidity_pct": None, "pressure_hpa": None,
            "wind_speed_mph": None, "wind_gust_mph": None, "wind_dir_from_deg": None, "wind_dir_deg": None,
            "precip_mm": daily_precip, "rain_mm": None, "snowfall_cm": None, "weather_code": None,
            "weather_status": "NO_HOURLY_DAILY_FALLBACK", "weather_source": "open-meteo-daily-fallback",
        }
        _WEATHER_CACHE[key] = dict(out)
        return out

    target_dt = _local_game_datetime(game_datetime, api_tz)
    if target_dt is None:
        target_hour = 19 if game_hour_local is None else int(game_hour_local)
        try:
            target_dt = pd.Timestamp(f"{date_str} {target_hour:02d}:00:00")
        except Exception:
            target_dt = pd.Timestamp(f"{date_str} 19:00:00")

    idxs = _nearest_hour_indices(hours, target_dt, game_window_hours=3) or [0]
    def arr(name):
        return hourly.get(name, []) or []

    wind_from_vals = []
    wdir_arr = arr("winddirection_10m")
    for i in idxs:
        try:
            if len(wdir_arr) > i:
                wind_from_vals.append(wdir_arr[i])
        except Exception:
            pass
    wind_from, wind_toward = _vector_mean_direction_from_to_toward(wind_from_vals)

    temp_f = _mean_at_indices(arr("temperature_2m"), idxs)
    if temp_f is None:
        temp_f = daily_temp_max

    out = {
        "temp_f": temp_f,
        "feels_like_f": _mean_at_indices(arr("apparent_temperature"), idxs),
        "humidity_pct": _mean_at_indices(arr("relative_humidity_2m"), idxs),
        "pressure_hpa": _mean_at_indices(arr("surface_pressure"), idxs),
        "wind_speed_mph": _mean_at_indices(arr("windspeed_10m"), idxs),
        "wind_gust_mph": _max_at_indices(arr("windgusts_10m"), idxs),
        "wind_dir_from_deg": wind_from,
        "wind_dir_deg": wind_toward,
        "precip_mm": _max_at_indices(arr("precipitation"), idxs),
        "rain_mm": _max_at_indices(arr("rain"), idxs),
        "snowfall_cm": _max_at_indices(arr("snowfall"), idxs),
        "weather_code": _max_at_indices(arr("weather_code"), idxs),
        "weather_status": "HOURLY_GAME_WINDOW_OK",
        "weather_source": "open-meteo-hourly-game-window",
        "weather_timezone": api_tz,
        "weather_window_hours": len(idxs),
    }
    _WEATHER_CACHE[key] = dict(out)
    return out
...
def compute_weather_multiplier(loc: dict | None, home_team: str, game_datetime: str | None, date_str: str) -> tuple[float, dict]:
    if not loc or "lat" not in loc or "lon" not in loc:
        return 1.0, {
            "temp_f": None, "feels_like_f": None, "humidity_pct": None, "pressure_hpa": None,
            "wind_speed_mph": None, "wind_gust_mph": None, "wind_dir_deg": None,
            "precip_mm": None, "rain_mm": None, "snowfall_cm": None,
            "temp_mult": 1.0, "wind_mult": 1.0, "weather_status": "NO_COORDS",
            "weather_source": "none", "wx_tag": "WX_UNKNOWN", "wx_impact_score": 50,
        }

    game_hour = _parse_game_hour_local(game_datetime)
    weather = get_game_weather(float(loc["lat"]), float(loc["lon"]), date_str, game_datetime=game_datetime, game_hour_local=game_hour)

    roof_type = str(loc.get("roof", loc.get("roof_type", ""))).lower()
    roof_known = any(x in roof_type for x in ["dome", "retractable", "fixed", "closed"])
    is_roof_or_dome = roof_known or home_team in ROOF_OR_DOME_TEAMS
    ...
    wind_gust = weather.get("wind_gust_mph")
    wind_dir = weather.get("wind_dir_deg")  # toward direction
    ...
    if is_roof_or_dome:
        ...
        w_mult = 1.0
        weather.update({
            "temp_mult": t_mult, "wind_mult": w_mult, "out_to_cf_deg": out_to_cf_deg,
            "wind_out_component": None, "weather_mult_final": final_mult,
            "wx_tag": "WX_ROOF_NEUTRAL", "wx_impact_score": 50,
            "weather_status": str(weather.get("weather_status", "HOURLY_GAME_WINDOW_OK")) + "_ROOF_NEUTRAL",
        })
    ...
    try:
        if wind_speed is not None and wind_gust is not None:
            effective_wind = max(float(wind_speed), 0.65 * float(wind_gust))
    except Exception:
        effective_wind = wind_speed
    w_mult = wind_multiplier(effective_wind, wind_dir, out_to_cf_deg)

    final_mult = float(np.clip(t_mult * w_mult, 0.58, 1.34))

    try:
        if humidity_pct is not None and temp_f is not None and float(temp_f) >= 70:
            final_mult *= 1.0 + np.clip((float(humidity_pct) - 50.0) * 0.0006, -0.018, 0.025)
    except Exception:
        pass
    try:
        if pressure_hpa is not None:
            final_mult *= 1.0 + np.clip((1013.0 - float(pressure_hpa)) * 0.0007, -0.025, 0.030)
    except Exception:
        pass
    ...
    wind_component = None
    if effective_wind is not None and wind_dir is not None and out_to_cf_deg is not None:
        try:
            diff = abs(((float(wind_dir) - float(out_to_cf_deg) + 180.0) % 360.0) - 180.0)
            wind_component = float(np.cos(np.deg2rad(diff)))
            if float(effective_wind) >= 8:
                if wind_component >= 0.45:
                    final_mult *= 1.0 + min(0.08, 0.006 * float(effective_wind) * wind_component)
                elif wind_component <= -0.45:
                    final_mult *= 1.0 - min(0.09, 0.0065 * float(effective_wind) * abs(wind_component))
        except Exception:
            wind_component = None

    final_mult = float(np.clip(final_mult, 0.52, 1.38))
    wx_impact = int(round(np.clip(50 + ((final_mult - 1.0) * 250.0), 0, 100)))
    if final_mult >= 1.18:
        wx_tag = "WX🔥STRONG_BOOST"
    elif final_mult >= 1.09:
        wx_tag = "WX↑BOOST"
    elif final_mult <= 0.82:
        wx_tag = "WX🧊STRONG_SUPPRESS"
    elif final_mult <= 0.92:
        wx_tag = "WX↓SUPPRESS"
    else:
        wx_tag = "WX_NEUTRAL"
    ...
    weather.update({
        ...
        "effective_wind_mph": round(float(effective_wind), 2) if effective_wind is not None else None,
        "out_to_cf_deg": out_to_cf_deg,
        "wind_out_component": round(wind_component, 3) if wind_component is not None else None,
        "weather_mult_final": round(final_mult, 3),
        "wx_tag": wx_tag,
        "wx_impact_score": wx_impact,
        "weather_status": weather.get("weather_status", "HOURLY_GAME_WINDOW_OK"),
    })
    return final_mult, weather
...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    for g in games:
        ...
        w_mult = 1.0
        weather_info = {"temp_f": None, "wind_speed_mph": None, "wind_dir_deg": None, "temp_mult": 1.0, "wind_mult": 1.0, "weather_status": "WEATHER_OFF" if not use_weather else "NOT_REQUESTED"}
        if use_weather:
        ...
        for side in ["away", "home"]:
            ...
            for hid in hitter_ids:
                ...
                matchup_mult = float(np.clip(pt_mult * recent_form_mult * pitcher_hr_mult, 0.88, 1.30))
                base_context_mult = float(np.clip(env_mult * bp_mult * platoon_mult, 0.78, 1.38))
                p_pa_adj = float(np.clip(p_pa * matchup_mult * base_context_mult, 1e-6, 0.32))
                ...
                p_pa_adj = float(np.clip(p_pa_adj * exp_bbe_mult * power_spike_mult, 1e-6, 0.34))

                matchup_escalation_mult, environment_escalation_mult, daily_context_mult, matchup_signal_count, environment_signal_count, context_flag = compute_daily_context_multipliers(
                    pt_mult=pt_mult,
                    ...
                    power_spike_mult=power_spike_mult,
                    exp_pa=exp_pa,
                    power_gate_mult=power_gate_mult,
                )
                p_pa_adj = float(np.clip(p_pa_adj * daily_context_mult, 1e-6, 0.36))
                ...
                raw_p1 = p1
                calibrated_p1, calibration_method = calibrate_final_hr_probability(raw_p1)
                final_layer = apply_power_context_probability_layer(
                    calibrated_p1,
                    power_score=power_score,
                    matchup_score=matchup_scores["matchup_score"],
                    matchup_signal_count=matchup_signal_count,
                    environment_signal_count=environment_signal_count,
                    context_flag=context_flag,
                    power_gate_mult=power_gate_mult,
                )
                p1 = float(final_layer["p_final"])

                # keep 2+ probability coherent after final 1+ probability
                ...
                rows.append(
                    {
                        ...
                        "matchup_escalation_mult": round(matchup_escalation_mult, 3),
                        "environment_escalation_mult": round(environment_escalation_mult, 3),
                        "daily_context_mult": round(daily_context_mult, 3),
                        "matchup_signal_count": matchup_signal_count,
                        "environment_signal_count": environment_signal_count,
                        "context_flag": context_flag,
                        "p_hr_1plus_calibrated": round(float(final_layer["p_calibrated"]), 6),
                        "post_calibration_boost": round(float(final_layer["post_calibration_boost"]), 6),
                        "post_calibration_boost_reason": final_layer["post_calibration_boost_reason"],
                        "power_context_blend_score": round(float(final_layer["power_context_blend_score"]), 2),
                        "power_anchor_floor": round(float(final_layer["power_anchor_floor"]), 6),
                        "dynamic_prob_cap": round(float(final_layer["dynamic_prob_cap"]), 6),
                        "cap_pressure": round(float(final_layer["cap_pressure"]), 6),
                        "boost_strength_factor": round(float(final_layer["boost_strength_factor"]), 3),
                        "context_norm_clamped": round(float(final_layer["context_norm_clamped"]), 3),
                        "p_hr_pa": p_pa_adj,
                        "p_hr_1plus_sim": p1,
                        "p_hr_2plus_sim": p2,
                        ...
                        "weather_final_mult": weather_info.get("weather_mult_final", round(w_mult, 3)),
                        "weather_status": weather_info.get("weather_status"),
                        "wx_tag": weather_info.get("wx_tag"),
                        "wx_impact_score": weather_info.get("wx_impact_score"),
                        "wind_out_component": weather_info.get("wind_out_component"),
    ...
    board = ensure_board_schema(board, date_str)
    if board.empty:
        return board
    ...
    pitcher_score = (
        0.6 * pd.to_numeric(board.get("pitcher_hr_score",50), errors="coerce").fillna(50) +
        0.4 * ((pd.to_numeric(board.get("pitcher_hr_mult",1), errors="coerce").fillna(1)-1)*100+50)
    ).clip(0,100)

    batter_heat = (
        0.45 * board["recent_damage_score"] +
        0.35 * (board["recent_barrel_rate"]*100) +
        0.20 * (board["recent_hard_hit_rate"]*100)
    ).clip(0,100)

    pitch_mix_fit = (((pd.to_numeric(board.get("pitchtype_mult",1),errors="coerce").fillna(1)-1)*100)+50).clip(0,100)
    board["pitch_mix_fit_score"] = pitch_mix_fit

    bullpen_score = (((pd.to_numeric(board.get("pitcher_hr_mult",1),errors="coerce").fillna(1)-1)*100)+50).clip(0,100)
    board["bullpen_score"] = bullpen_score

    # Strong weather build: weather now carries more weight in environment_clean.
    # It still cannot fully dominate because park + bullpen remain in the blend.
    weather_score_clean = (((pd.to_numeric(board.get("weather_mult",1),errors="coerce").fillna(1)-1)*175)+50).clip(0,100)
    board["weather_score_clean"] = weather_score_clean
    environment_clean = (
        0.30 * (((pd.to_numeric(board.get("park_factor",1),errors="coerce").fillna(1)-1)*100)+50) +
        0.45 * weather_score_clean +
        0.25 * bullpen_score
    ).clip(0,100)
    ...
    board["matchup_score_clean"] = (
        ...
        0.26 * pitch_mix_fit +
        0.15 * batter_heat +
        0.17 * environment_clean +
        0.08 * bullpen_score
    ).clip(0,100)

    _ml = _relative_bucket(board["matchup_score_clean"], high_q=0.75, med_q=0.40, labels=("🔥","👍","👎"))
    board["matchup_label"] = _ml

    board["prob_rank"] = board["p_hr_1plus_sim"].rank(method="first",ascending=False).astype(int)

    # Balanced ranking blend: HR skill anchors the board; context breaks ties and finds spike days.
    matchup_vals = pd.to_numeric(board["matchup_score_clean"], errors="coerce")
    prob_vals = pd.to_numeric(board["p_hr_1plus_sim"], errors="coerce")
    power_vals = pd.to_numeric(board.get("power_score", 0.0), errors="coerce").fillna(0.0)
    blend_vals = pd.to_numeric(board.get("power_context_blend_score", np.nan), errors="coerce")

    matchup_rng = matchup_vals.max() - matchup_vals.min()
    prob_rng = prob_vals.max() - prob_vals.min()
    power_rng = power_vals.max() - power_vals.min()
    blend_rng = blend_vals.max() - blend_vals.min()

    board["_matchup_norm_raw"] = 0.5 if pd.isna(matchup_rng) or matchup_rng == 0 else (matchup_vals - matchup_vals.min()) / matchup_rng
    board["_prob_norm"] = 0.5 if pd.isna(prob_rng) or prob_rng == 0 else (prob_vals - prob_vals.min()) / prob_rng
    board["_power_norm"] = 0.5 if pd.isna(power_rng) or power_rng == 0 else (power_vals - power_vals.min()) / power_rng
    board["_blend_norm"] = 0.5 if pd.isna(blend_rng) or blend_rng == 0 else (blend_vals - blend_vals.min()) / blend_rng

    # Context domination guard: great spots can lift weak bats, but should not freely outrank real HR bats.
    board["_matchup_norm"] = np.minimum(board["_matchup_norm_raw"], board["_power_norm"] * 1.80 + 0.10)
    board["_final_score"] = (
        0.45 * board["_power_norm"]
        + 0.35 * board["_matchup_norm"]
        + 0.15 * board["_prob_norm"]
        + 0.05 * board["_blend_norm"].fillna(0.5)
    )
    board.loc[prob_vals >= 0.20, "_final_score"] += 0.035
    board.loc[(prob_vals >= 0.17) & (prob_vals < 0.20), "_final_score"] += 0.020
    board.loc[(prob_vals >= 0.145) & (prob_vals < 0.17), "_final_score"] += 0.010

    board = board.sort_values(
        ["_final_score", "p_hr_1plus_sim", "power_score", "recent_damage_score"],
        ascending=False
    ).reset_index(drop=True)

    board.insert(0,"rank",range(1,len(board)+1))
    board = board.drop(columns=[c for c in ["_matchup_norm_raw", "_matchup_norm", "_prob_norm", "_power_norm", "_blend_norm", "_final_score"] if c in board.columns])

    # ---- Slate-relative flags for readable export ----
    board["pitch_match_flag"] = _relative_bucket(board.get("pitch_mix_fit_score", 0), 0.75, 0.40)
    board["pitcher_risk_flag"] = _relative_bucket(board.get("pitcher_hr_score", 0), 0.70, 0.35)

    recent_vals = pd.to_numeric(board.get("recent_damage_score", 0), errors="coerce").fillna(0.0)
    if recent_vals.notna().sum() == 0:
        board["recent_power_flag"] = "NO"
    else:
        power_cut = recent_vals.quantile(0.60)
        board["recent_power_flag"] = np.where(recent_vals >= power_cut, "YES", "NO")

    bullpen_vals = pd.to_numeric(board.get("bullpen_score", 0), errors="coerce").fillna(0.0)
    if bullpen_vals.notna().sum() == 0:
        board["bullpen_flag"] = "AVG"
    else:
        weak_cut = bullpen_vals.quantile(0.70)
        avg_cut = bullpen_vals.quantile(0.35)
        board["bullpen_flag"] = np.where(
            bullpen_vals >= weak_cut,
            "WEAK",
            np.where(bullpen_vals >= avg_cut, "AVG", "OK")
        )

    board["flag_score"] = (
        board["pitch_match_flag"].map({"HIGH": 2, "MED": 1, "LOW": 0}).fillna(0)
        + board["pitcher_risk_flag"].map({"HIGH": 2, "MED": 1, "LOW": 0}).fillna(0)
        + board["recent_power_flag"].map({"YES": 2, "NO": 0}).fillna(0)
        + board["bullpen_flag"].map({"WEAK": 1, "AVG": 0.5, "OK": 0}).fillna(0)
    )

    board["flags"] = (
        "MATCH:" + board["pitch_match_flag"].astype(str)
        + " | RISK:" + board["pitcher_risk_flag"].astype(str)
        + " | POWER:" + board["recent_power_flag"].astype(str)
        + " | BP:" + board["bullpen_flag"].astype(str)
    )

    prob_pct = (pd.to_numeric(board.get("p_hr_1plus_sim", 0), errors="coerce").fillna(0.0) * 100.0).round(1)
    board["display_line"] = (
        board["player_name"].astype(str)
        + " | " + prob_pct.astype(str) + "%"
        + " | " + board.get("matchup_label", "").astype(str)
        + " | " + board["flags"].astype(str)
    )

    # Export view: strongest full-signal plays first, probability as tiebreaker.
    export_board = board.sort_values(
        ["flag_score", "p_hr_1plus_sim", "prob_rank", "recent_damage_score"],
        ascending=[False, False, True, False]
    ).reset_index(drop=True).copy()
    export_board["rank"] = np.arange(1, len(export_board) + 1)

    cols = [
        "rank","prob_rank","player_name","team","probable_pitcher_faced","venue",
        "p_hr_1plus_sim","matchup_label","pitch_match_flag","pitcher_risk_flag",
        "recent_power_flag","bullpen_flag","flag_score","flags","display_line",
        "matchup_score_clean","recent_damage_score","pitch_mix_fit_score","pitcher_hr_score","bullpen_score"
    ]
    cols = [c for c in cols if c in board.columns]

    date_str = str(board["date"].iloc[0]) if "date" in board.columns else "today"

    export_board[cols].to_csv(f"outputs/hr_board_full_slate_{date_str}.csv", index=False)

    top_n = int(globals().get("DEFAULT_TOPN", 75) or 75)
    export_board.head(top_n)[cols].to_csv(f"outputs/hr_board_top75_{date_str}.csv", index=False)

    # Home run report: summarize attackable games / environments using the live board.
    report_game_col = "game" if "game" in board.columns else ("venue" if "venue" in board.columns else None)
    if report_game_col is not None:
        report_df = board.copy()

        # Build a lightweight game key if only venue exists.
        if report_game_col == "venue":
            report_df["_report_game"] = report_df["venue"].fillna("Unknown Venue")
        else:
            report_df["_report_game"] = report_df[report_game_col].fillna("Unknown Game")

        # Top model bats per game.
        top_bats = (
            report_df.sort_values(["rank", "p_hr_1plus_sim"], ascending=[True, False])
            .groupby("_report_game")
            .head(5)
            .groupby("_report_game")["player_name"]
            .apply(lambda x: ", ".join(pd.Series(x).astype(str).tolist()))
            .reset_index(name="top_batters")
        )

        game_summary = (
            report_df.groupby("_report_game", dropna=False)
            .agg(
                avg_matchup_score=("matchup_score_clean", "mean"),
                avg_sim=("p_hr_1plus_sim", "mean"),
                max_sim=("p_hr_1plus_sim", "max"),
                pitcher_hr_score=("pitcher_hr_score", "mean"),
                bullpen_score=("bullpen_score", "mean"),
                top10_count=("rank", lambda s: int((pd.to_numeric(s, errors="coerce") <= 10).sum())),
                top25_count=("rank", lambda s: int((pd.to_numeric(s, errors="coerce") <= 25).sum())),
                top75_count=("rank", lambda s: int((pd.to_numeric(s, errors="coerce") <= top_n).sum())),
            )
            .reset_index()
            .rename(columns={"_report_game": "game"})
        )

        game_summary = game_summary.merge(top_bats.rename(columns={"_report_game": "game"}), on="game", how="left")

        # Optional pitcher/venue labels if available at the board level.
        if "probable_pitcher_faced" in report_df.columns:
            pitchers = (
                report_df.groupby("_report_game")["probable_pitcher_faced"]
                .apply(lambda s: ", ".join(sorted({str(x) for x in s.dropna() if str(x).strip()})))
                .reset_index(name="pitchers")
                .rename(columns={"_report_game": "game"})
            )
            game_summary = game_summary.merge(pitchers, on="game", how="left")

        if "venue" in report_df.columns and report_game_col != "venue":
            venues = (
                report_df.groupby("_report_game")["venue"]
                .apply(lambda s: ", ".join(sorted({str(x) for x in s.dropna() if str(x).strip()})))
                .reset_index(name="venue")
                .rename(columns={"_report_game": "game"})
            )
            game_summary = game_summary.merge(venues, on="game", how="left")

        def classify_env(row):
            if float(row.get("pitcher_hr_score", 0) or 0) >= 7.5 or float(row.get("avg_matchup_score", 0) or 0) >= 42:
                return "Strong HR Spot"
            if float(row.get("pitcher_hr_score", 0) or 0) >= 6.0 or float(row.get("avg_matchup_score", 0) or 0) >= 38:
                return "Decent Spot"
            return "Neutral"

        game_summary["hr_environment_flag"] = game_summary.apply(classify_env, axis=1)
        game_summary = game_summary.sort_values(
            ["pitcher_hr_score", "avg_matchup_score", "max_sim"],
            ascending=False
        ).reset_index(drop=True)

        report_cols = [
            "game", "venue", "pitchers", "pitcher_hr_score", "bullpen_score",
            "avg_matchup_score", "avg_sim", "max_sim",
            "top10_count", "top25_count", "top75_count",
            "hr_environment_flag", "top_batters",
        ]
        report_cols = [c for c in report_cols if c in game_summary.columns]
        game_summary[report_cols].to_csv(f"outputs/hr_home_run_report_{date_str}.csv", index=False)
    ...
    # Clean matchup engine: HR-prone pitcher + hot bat + pitch mix fit + environment + bullpen
    if "recent_damage_score" not in board.columns:
        board["recent_damage_score"] = 0.0
    if "recent_barrel_rate" not in board.columns:
        board["recent_barrel_rate"] = 0.0
    ...
    # Sharpen bullpen effect using explicit bullpen fields when present
    if "bullpen_hr_mult" in board.columns:
        bullpen_mult_series = pd.to_numeric(board["bullpen_hr_mult"], errors="coerce").fillna(1.0)
    elif "bullpen_mult" in board.columns:
        bullpen_mult_series = pd.to_numeric(board["bullpen_mult"], errors="coerce").fillna(1.0)
    else:
        bullpen_mult_series = pd.to_numeric(board["pitcher_hr_mult"], errors="coerce").fillna(1.0)

    if "bullpen_score" in board.columns:
        bullpen_score = pd.to_numeric(board["bullpen_score"], errors="coerce").fillna(50.0)
    else:
        bullpen_score = (((bullpen_mult_series - 1.0) * 100.0) + 50.0).clip(lower=0, upper=100)
        board["bullpen_score"] = bullpen_score

    pitcher_score = (
        0.55 * pd.to_numeric(board["pitcher_hr_score"], errors="coerce").fillna(50.0) +
        0.45 * (((pd.to_numeric(board["pitcher_hr_mult"], errors="coerce").fillna(1.0) - 1.0) * 100.0) + 50.0)
    ...
    pitch_mix_fit = ((((pd.to_numeric(board["pitchtype_mult"], errors="coerce").fillna(1.0) - 1.0) * 100.0) + 50.0)).clip(lower=0, upper=100)
    board["pitch_mix_fit_score"] = pitch_mix_fit

    environment_clean = (
        0.40 * ((((pd.to_numeric(board["park_factor"], errors="coerce").fillna(1.0) - 1.0) * 100.0) + 50.0)) +
        0.30 * ((((pd.to_numeric(board["weather_mult"], errors="coerce").fillna(1.0) - 1.0) * 100.0) + 50.0)) +
        0.30 * bullpen_score
    ).clip(lower=0, upper=100)
    board["environment_clean"] = environment_clean
    ...
    board["matchup_label"] = _relative_bucket(board["matchup_score_clean"], high_q=0.75, med_q=0.40, labels=("🔥","👍","👎"))

    # Probability rank shown next to the matchup-ranked board
    board["prob_rank"] = (
        pd.to_numeric(board["p_hr_1plus_sim"], errors="coerce")
        .rank(method="first", ascending=False)
        .astype(int)
    )

    # Display order = 60/40 matchup/probability blend with soft probability boosts
    matchup_vals = pd.to_numeric(board["matchup_score_clean"], errors="coerce")
    prob_vals = pd.to_numeric(board["p_hr_1plus_sim"], errors="coerce")
    matchup_rng = matchup_vals.max() - matchup_vals.min()
    prob_rng = prob_vals.max() - prob_vals.min()
    board["_matchup_norm"] = 0.5 if pd.isna(matchup_rng) or matchup_rng == 0 else (matchup_vals - matchup_vals.min()) / matchup_rng
    board["_prob_norm"] = 0.5 if pd.isna(prob_rng) or prob_rng == 0 else (prob_vals - prob_vals.min()) / prob_rng
    board["_final_score"] = 0.6 * board["_matchup_norm"] + 0.4 * board["_prob_norm"]
    board.loc[prob_vals >= 0.22, "_final_score"] += 0.08
    board.loc[(prob_vals >= 0.18) & (prob_vals < 0.22), "_final_score"] += 0.05
    board.loc[(prob_vals >= 0.15) & (prob_vals < 0.18), "_final_score"] += 0.025

    board = board.sort_values(
        ["_final_score", "p_hr_1plus_sim", "recent_damage_score"],
        ascending=False
    ).reset_index(drop=True)
    if "rank" in board.columns:
        board = board.drop(columns=["rank"])
    board.insert(0, "rank", np.arange(1, len(board) + 1))
    board = board.drop(columns=[c for c in ["_matchup_norm", "_prob_norm", "_final_score"] if c in board.columns])

    preferred_cols = [
        "rank", "prob_rank", "player_name", "matchup_score_clean", "matchup_label",
        "power_tier", "matchup_tier", "power_score", "daily_matchup_hr_score", "matchup_score_today",
        "pitcher_vulnerability_score", "pitch_mix_fit_score", "split_advantage_score", "opportunity_score", "environment_score", "environment_clean", "recent_damage_score",
        ...
        "bullpen_score", "power_qualified", "power_gate_mult", "exp_pa", "matchup_mult", "pitchtype_mult", "recent_form_mult", "pitcher_hr_mult", "park_factor", "weather_mult",
    ...
    if "date" not in board.columns:
        board["date"] = date_str if "date_str" in locals() else str(date_cls.today())
    board["date"] = board["date"].astype(str)
    if "game_date" not in board.columns:
        board["game_date"] = board["date"]
    else:
        board["game_date"] = board["game_date"].fillna(board["date"]).astype(str)
...
def _safe_num_series(df: pd.DataFrame, col: str, default=0.0) -> pd.Series:
    if col not in df.columns:
        return pd.Series(default, index=df.index, dtype=float)
    return pd.to_numeric(df[col], errors="coerce").fillna(default)


def enrich_supercomputer_output(board: pd.DataFrame) -> pd.DataFrame:
    board = ensure_board_schema(board)
    if board is None or board.empty:
        return board
    board = board.copy()
    if "prob_rank" not in board.columns and "p_hr_1plus_sim" in board.columns:
        board["prob_rank"] = _safe_num_series(board, "p_hr_1plus_sim").rank(method="first", ascending=False).astype(int)
    reason_text = board.get("bet_flag_reason", pd.Series("", index=board.index)).fillna("").astype(str)
    si_volume = reason_text.str.contains(r"\bSI volume\b", case=False, regex=True)
    si_fit = reason_text.str.contains(r"\bSI fit\b", case=False, regex=True)
    elite_pitch_fit = reason_text.str.contains(r"\belite pitch fit\b", case=False, regex=True)
    pitcher_hr = _safe_num_series(board, "pitcher_hr_score", 50.0)
    matchup_clean = _safe_num_series(board, "matchup_score_clean", 50.0)
    prob = _safe_num_series(board, "p_hr_1plus_sim", 0.0)
    power_score = _safe_num_series(board, "power_score", 0.0)
    env_clean = _safe_num_series(board, "environment_clean", 50.0)
    recent_damage = _safe_num_series(board, "recent_damage_score", 0.0)
    pitch_fit_score = _safe_num_series(board, "pitch_mix_fit_score", 50.0)
    exp_pa = _safe_num_series(board, "exp_pa", 4.1)
    split_adv = _safe_num_series(board, "split_advantage_score", 0.0)
    recent_cut = 18.0
    try:
        recent_cut = max(18.0, float(recent_damage.quantile(0.60)))
    except Exception:
        pass
    stack_signal_count = (
        si_volume.astype(int) + si_fit.astype(int) + elite_pitch_fit.astype(int)
        + (pitcher_hr >= 60).astype(int) + (env_clean >= 52).astype(int)
        + (prob >= 0.14).astype(int) + (power_score >= 35).astype(int)
        + (recent_damage >= recent_cut).astype(int) + (exp_pa >= 4.35).astype(int)
        + (split_adv >= 1).astype(int)
    )
    board["stack_signal_count"] = stack_signal_count.astype(int)
    board["stack_flag"] = np.select([stack_signal_count >= 7, stack_signal_count >= 5, stack_signal_count >= 3],["SUPER", "LIVE", "WATCH"],default="NONE")
    if "matchup_label" not in board.columns:
        board["matchup_label"] = _relative_bucket(matchup_clean, high_q=0.75, med_q=0.40, labels=("🔥", "👍", "👎"))
    def _norm(s: pd.Series) -> pd.Series:
        ...
        return (s - s.min()) / rng
    power_norm = _norm(power_score)
    matchup_norm_raw = _norm(matchup_clean)
    matchup_norm = np.minimum(matchup_norm_raw, power_norm * 1.80 + 0.10)
    prob_norm = _norm(prob)
    blend_score = _safe_num_series(board, "power_context_blend_score", 50.0)
    blend_norm = _norm(blend_score)

    # Super rank is intentionally NOT just probability. It balances real HR skill with today's spot.
    board["super_score"] = (
        36 * power_norm
        + 30 * matchup_norm
        + 16 * prob_norm
        + 6 * _norm(pitcher_hr)
        + 5 * _norm(pitch_fit_score)
        + 3 * _norm(env_clean)
        + 2 * _norm(recent_damage)
        + 2 * blend_norm
    )
    board.loc[board["stack_flag"] == "SUPER", "super_score"] += 5
    board.loc[board["stack_flag"] == "LIVE", "super_score"] += 2.5
    board.loc[elite_pitch_fit & (power_score >= 35), "super_score"] += 1.5
    board.loc[si_volume & si_fit & (power_score >= 35), "super_score"] += 1.5
    board["context_domination_guard"] = (matchup_norm_raw - matchup_norm).clip(lower=0)
    board = board.sort_values(["super_score", "p_hr_1plus_sim", "power_score", "matchup_score_clean"], ascending=False).reset_index(drop=True)
    board["super_rank"] = np.arange(1, len(board) + 1)
    prob_pct = (prob * 100).round(1)
    board["scan_flags"] = "STACK:" + board["stack_flag"].astype(str) + " | MATCH:" + board.get("pitch_match_flag", pd.Series("MED", index=board.index)).astype(str) + " | RISK:" + board.get("pitcher_risk_flag", pd.Series("MED", index=board.index)).astype(str) + " | POWER:" + board.get("recent_power_flag", pd.Series("NO", index=board.index)).astype(str) + " | BP:" + board.get("bullpen_flag", pd.Series("AVG", index=board.index)).astype(str)
    board["phone_view"] = board["player_name"].astype(str) + " | " + prob_pct.astype(str) + "% | SR:" + board["super_rank"].astype(str) + " | PR:" + board["prob_rank"].astype(str) + " | " + board["scan_flags"].astype(str)
    board = apply_decision_layer(board)

    # IMPORTANT: the decision layer should change the actual board order, not just add labels.
    # Keep the old model ranks for audit, but make the exported position reflect the decision score.
    if "super_rank" in board.columns and "original_super_rank" not in board.columns:
        board["original_super_rank"] = board["super_rank"]
    sort_cols = [c for c in ["decision_score", "p_hr_1plus_sim", "matchup_score_clean", "power_score"] if c in board.columns]
    if sort_cols:
        board = board.sort_values(sort_cols, ascending=[False] * len(sort_cols), na_position="last").reset_index(drop=True)
    board["decision_rank"] = np.arange(1, len(board) + 1)
    board["rank"] = board["decision_rank"]
    prob_pct2 = (pd.to_numeric(board.get("p_hr_1plus_sim", 0), errors="coerce") * 100).round(1).astype(str)
    board["phone_view"] = (
        board["player_name"].astype(str)
        + " | " + prob_pct2 + "%"
        + " | DR:" + board["decision_rank"].astype(str)
        + " | SR:" + board.get("original_super_rank", board.get("super_rank", pd.Series("", index=board.index))).astype(str)
        + " | PR:" + board.get("prob_rank", pd.Series("", index=board.index)).astype(str)
        + " | " + board.get("decision_tier", pd.Series("", index=board.index)).astype(str)
        + " | CONF:" + board.get("confidence_score", pd.Series("", index=board.index)).astype(str)
    )
    return board



def apply_decision_layer(board: pd.DataFrame) -> pd.DataFrame:
    """Add a final decision layer for cleaner betting exports."""
    board = ensure_board_schema(board)
    if board is None or board.empty:
        return board
    out = board.copy()
    ...
    prob = _safe_num_series(out, "p_hr_1plus_sim", 0.0)
    prob_rank = _safe_num_series(out, "prob_rank", 9999)
    super_rank = _safe_num_series(out, "super_rank", 9999)
    matchup = _safe_num_series(out, "matchup_score_clean", 50.0)
    power = _safe_num_series(out, "power_score", 0.0)
    pitch_fit = _safe_num_series(out, "pitch_mix_fit_score", 50.0)
    pitcher_hr = _safe_num_series(out, "pitcher_hr_score", 50.0)
    env = _safe_num_series(out, "environment_clean", 50.0)
    recent = _safe_num_series(out, "recent_damage_score", 0.0)
    exp_pa = _safe_num_series(out, "exp_pa", 4.1)
    cap_pressure = _safe_num_series(out, "cap_pressure", 0.0)
    context_guard = _safe_num_series(out, "context_domination_guard", 0.0)

    confidence = pd.Series(100.0, index=idx)
    reasons = pd.Series("", index=idx, dtype=object)

    def add_reason(mask, text):
        nonlocal reasons
        mask = pd.Series(mask, index=idx).fillna(False)
        cur = reasons.loc[mask].astype(str)
        reasons.loc[mask] = np.where(cur.str.len() > 0, cur + "; " + text, text)

    def col_missing(col):
        if col not in out.columns:
            return pd.Series(True, index=idx)
        vals = out[col]
        return vals.isna() | vals.astype(str).str.strip().isin(["", "nan", "None", "NONE", "null"])

    missing_weather = col_missing("weather_mult")
    weather_status = out.get("weather_status", pd.Series("UNKNOWN", index=idx)).fillna("UNKNOWN").astype(str).str.upper()
    weak_weather = weather_status.isin(["API_FAIL", "NO_COORDS", "NO_HOURLY", "NO_MATCHING_HOUR", "PARTIAL", "UNKNOWN", "NOT_REQUESTED"])
    missing_pitch = col_missing("dominant_pitch") | col_missing("dominant_pitch_usage")
    low_pitch_usage = pd.to_numeric(out.get("dominant_pitch_usage", pd.Series(np.nan, index=idx)), errors="coerce").fillna(0.0) < 0.25
    neutral_pitch_fit = pd.to_numeric(out.get("batter_vs_pitch_ratio", pd.Series(np.nan, index=idx)), errors="coerce").fillna(1.0).between(0.985, 1.015)
    # Do not penalize every row just because MLB game_pk is unavailable. Venue/game text is enough
    # for slate grouping; game_pk is only a nice-to-have identifier.
    missing_game = col_missing("game") & col_missing("venue") & col_missing("game_pk")
    missing_pitcher = col_missing("probable_pitcher_faced") | out.get("probable_pitcher_faced", pd.Series("", index=idx)).astype(str).str.upper().isin(["TBD", "NONE", "NAN", ""])

    confidence -= missing_weather.astype(float) * 10.0
    confidence -= weak_weather.astype(float) * 8.0
    confidence -= missing_pitch.astype(float) * 14.0
    confidence -= (low_pitch_usage & ~missing_pitch).astype(float) * 5.0
    confidence -= (neutral_pitch_fit & ~missing_pitch).astype(float) * 4.0
    confidence -= missing_game.astype(float) * 8.0
    confidence -= missing_pitcher.astype(float) * 12.0
    confidence -= (exp_pa < 3.75).astype(float) * 7.0
    confidence -= (context_guard > 0.12).astype(float) * 8.0
    confidence -= (cap_pressure > 0.06).astype(float) * 5.0

    add_reason(missing_weather | weak_weather, "weather weak/missing")
    add_reason(missing_pitch, "pitch mix missing")
    add_reason(low_pitch_usage & ~missing_pitch, "low dominant-pitch confidence")
    add_reason(neutral_pitch_fit & ~missing_pitch, "neutral pitch-fit fallback")
    add_reason(missing_game, "game id/key missing")
    add_reason(missing_pitcher, "pitcher missing/TBD")
    add_reason(exp_pa < 3.75, "low PA confidence")
    add_reason(context_guard > 0.12, "context-heavy profile")
    add_reason(cap_pressure > 0.06, "probability cap pressure")

    out["data_quality_flag"] = np.select(
        [confidence >= 85, confidence >= 75, confidence >= 65],
        ["CLEAN", "PLAYABLE", "THIN"],
        default="BAD_DATA",
    )

    confidence = confidence.clip(0, 100).round(0).astype(int)
    out["confidence_score"] = confidence
    out["confidence_band"] = pd.cut(confidence, bins=[-1, 69, 84, 100], labels=["LOW", "OK", "CLEAN"]).astype(str)

    if "game" in out.columns:
        game_key = out["game"].fillna(out.get("venue", "Unknown Game")).astype(str)
    elif "venue" in out.columns:
        game_key = out["venue"].fillna("Unknown Venue").astype(str)
    else:
        game_key = pd.Series("Unknown Game", index=idx)
    out["game_exposure_key"] = game_key

    tmp = out.copy()
    tmp["_tmp_super_rank"] = super_rank.fillna(9999)
    tmp["_tmp_prob"] = prob
    tmp = tmp.sort_values(["game_exposure_key", "_tmp_super_rank", "_tmp_prob"], ascending=[True, True, False])
    out.loc[tmp.index, "game_exposure_rank"] = (tmp.groupby("game_exposure_key").cumcount() + 1).values
    out["game_exposure_rank"] = pd.to_numeric(out["game_exposure_rank"], errors="coerce").fillna(99).astype(int)
    add_reason(out["game_exposure_rank"] >= 4, "4th+ option in same game")
    add_reason(out["game_exposure_rank"] == 3, "3rd option in same game")

    # Aggressive scoring: make probability separation matter.
    # This intentionally spreads the top names away from the 14-16% middle tier.
    prob_pct_aggr = (prob * 100.0).clip(lower=0.0)
    core_score = (
        (prob_pct_aggr ** 1.40)
        + (0.80 * matchup.clip(0, 100))
        + (0.50 * confidence.astype(float))
        + (0.18 * power.clip(lower=0))
        + (0.10 * pitch_fit.clip(0, 100))
        + (0.08 * pitcher_hr.clip(0, 100))
        + (0.08 * env.clip(0, 100))
        + (0.12 * pd.to_numeric(out.get("weather_score_clean", 50), errors="coerce").fillna(50).clip(0, 100))
    )

    # Hard cutoff effects: stop thin 12-14% / bad-data bats from living near elite names.
    core_score *= np.where(prob < 0.140, 0.85, 1.00)
    # Strong weather finish: meaningful weather can separate otherwise similar names.
    wx_mult_for_rank = pd.to_numeric(out.get("weather_mult", 1.0), errors="coerce").fillna(1.0)
    core_score *= np.where(wx_mult_for_rank >= 1.18, 1.08, np.where(wx_mult_for_rank >= 1.09, 1.035, 1.00))
    core_score *= np.where(wx_mult_for_rank <= 0.82, 0.88, np.where(wx_mult_for_rank <= 0.92, 0.955, 1.00))
    core_score *= np.where(confidence < 75, 0.80, 1.00)
    core_score *= np.where(confidence < 65, 0.68, 1.00)
    core_score *= np.where(missing_pitcher, 0.82, 1.00)
    core_score *= np.where(missing_pitch | weak_weather, 0.92, 1.00)
    core_score -= (out["game_exposure_rank"] == 3).astype(float) * 8.0
    core_score -= (out["game_exposure_rank"] >= 4).astype(float) * 18.0
    out["decision_score"] = pd.Series(core_score, index=idx).round(1)

    tier = pd.Series("RED_FADE", index=idx, dtype=object)
    core_mask = (
        (confidence >= 84)
        & (
            (prob >= 0.185)
            | ((prob >= 0.165) & (matchup >= 64) & (power >= 35))
            | ((prob_rank <= 12) & (matchup >= 58) & (power >= 35))
        )
        & (out["game_exposure_rank"] <= 2)
    )
    support_mask = (
        (confidence >= 75)
        & (
            ((prob >= 0.145) & (matchup >= 55))
            | ((prob_rank <= 30) & (matchup >= 52))
            | ((super_rank <= 30) & (prob >= 0.13) & (power >= 32))
        )
        & ~core_mask
        & (out["game_exposure_rank"] <= 3)
    )
    watch_mask = (
        (confidence >= 60)
        & ((prob >= 0.12) | (super_rank <= 50) | ((matchup >= 65) & (power >= 30)))
        & ~core_mask
        & ~support_mask
    )
    tier.loc[watch_mask] = "ORANGE_WATCH"
    tier.loc[support_mask] = "YELLOW_RR_ONLY"
    tier.loc[core_mask] = "GREEN_CORE"

    open_col = "odds_open_1plus" if "odds_open_1plus" in out.columns else ("odds_1plus" if "odds_1plus" in out.columns else None)
    if open_col:
        implied = out[open_col].map(_implied_prob_from_american)
        edge = prob - pd.to_numeric(implied, errors="coerce")
        out["market_implied_prob"] = implied
        out["model_edge"] = edge
        out["value_flag"] = np.select(
            [edge >= 0.035, edge >= 0.015, edge < -0.005],
            ["VALUE", "LEAN", "NO_VALUE"],
            default="FAIR_OR_UNKNOWN",
        )
    else:
        out["market_implied_prob"] = np.nan
        out["model_edge"] = np.nan
        out["value_flag"] = "NO_ODDS"

    out["decision_tier"] = tier
    out["decision_reason"] = reasons.replace("", "clean enough").astype(str)
    prob_pct = (prob * 100.0).round(1).astype(str)
    out["decision_view"] = (
        out["decision_tier"].astype(str)
        + " | " + out["player_name"].astype(str)
        + " | " + prob_pct + "%"
        + " | SR:" + out.get("super_rank", pd.Series("", index=idx)).astype(str)
        + " PR:" + out.get("prob_rank", pd.Series("", index=idx)).astype(str)
        + " | CONF:" + out["confidence_score"].astype(str)
        + " | " + out["value_flag"].astype(str)
    )
    return out


def _build_locked_betting_export(board: pd.DataFrame) -> pd.DataFrame:
    """Phone-first betting-board export: player name starts at column A, raw audit fields move right."""
    if board is None or board.empty:
        return board
    export = board.copy()
    if "p_hr_1plus_sim" in export.columns:
        prob_pct = (pd.to_numeric(export["p_hr_1plus_sim"], errors="coerce") * 100.0).round(1)
        export["prob_pct"] = prob_pct.astype(str) + "%"
    if "model_edge" in export.columns:
        edge_pct = (pd.to_numeric(export["model_edge"], errors="coerce") * 100.0).round(1)
        export["edge_pct"] = edge_pct.astype(str) + "%"

    front_cols = [
        "player_name", "team", "opponent", "prob_pct", "decision_tier", "confidence_score",
        "decision_score", "value_flag", "edge_pct", "wx_tag", "weather_temp_f", "weather_wind_mph", "wind_out_component",
        "probable_pitcher_faced", "game", "venue",
        "decision_rank", "prob_rank", "original_super_rank", "game_exposure_rank", "scan_flags",
        "decision_reason", "matchup_score_clean", "power_score", "pitch_mix_fit_score",
        "pitcher_hr_score", "environment_clean", "weather_score_clean", "weather_mult", "park_factor", "bullpen_score",
        "p_hr_1plus_sim", "p_hr_2plus_sim", "odds_1plus", "odds_open_1plus", "odds_close_1plus",
    ]
    front_cols = [c for c in front_cols if c in export.columns]
    remaining = [c for c in export.columns if c not in front_cols]
    export = export[front_cols + remaining].copy()
    ...
    rename_map = {
        ...
        "wind_out_component": "WindOut",
        "probable_pitcher_faced": "Pitcher",
        "game": "Game",
        "venue": "Venue",
        "decision_rank": "DR",
        "prob_rank": "PR",
        "original_super_rank": "Old_SR",
        "game_exposure_rank": "Game_Rank",
        "scan_flags": "Flags",
        "decision_reason": "Notes",
        "matchup_score_clean": "Matchup",
        "power_score": "Power",
        "pitch_mix_fit_score": "PitchFit",
        "pitcher_hr_score": "PitcherRisk",
        "environment_clean": "Env",
        "weather_score_clean": "WxScore",
        "weather_mult": "Weather",
        "park_factor": "Park",
        "bullpen_score": "Bullpen",
        "p_hr_1plus_sim": "raw_p_hr_1plus",
        "p_hr_2plus_sim": "raw_p_hr_2plus",
        "odds_1plus": "Odds",
        "odds_open_1plus": "OpenOdds",
        "odds_close_1plus": "CloseOdds",
    }
    return export.rename(columns={k: v for k, v in rename_map.items() if k in export.columns})


def write_supercomputer_exports(full_board: pd.DataFrame, date_str: str, top_n: int):
    full_board = ensure_board_schema(full_board, date_str)
    if full_board is None or full_board.empty:
        return
    board = enrich_supercomputer_output(full_board)
    os.makedirs("outputs", exist_ok=True)
    export = board.copy()
    sort_cols = [c for c in ["decision_score", "p_hr_1plus_sim", "matchup_score_clean", "power_score"] if c in export.columns]
    if sort_cols:
        export = export.sort_values(sort_cols, ascending=[False] * len(sort_cols), na_position="last").reset_index(drop=True)
        export["decision_rank"] = np.arange(1, len(export) + 1)
        export["rank"] = export["decision_rank"]
    locked_export = _build_locked_betting_export(export)
    locked_export.to_csv(f"outputs/hr_board_full_slate_{date_str}.csv", index=False)
    locked_export.head(top_n).to_csv(f"outputs/hr_board_top{top_n}_{date_str}.csv", index=False)
    locked_export.head(top_n).to_csv(f"outputs/hr_board_top75_{date_str}.csv", index=False)
    locked_export.to_csv(f"outputs/hr_board_supercomputer_{date_str}.csv", index=False)
    game_col = "game" if "game" in export.columns else ("venue" if "venue" in export.columns else None)
    if game_col:
        summary_rows = []
        out_cols_master = [c for c in ["decision_tier", "decision_score", "confidence_score", "value_flag", "game_exposure_rank", "decision_view", "super_rank", "prob_rank", "player_name", "team", "probable_pitcher_faced", "p_hr_1plus_sim", "stack_flag", "stack_signal_count", "matchup_label", "scan_flags", "phone_view", "super_score", "daily_matchup_hr_score", "matchup_score_clean", "pitcher_hr_score", "pitch_mix_fit_score", "environment_clean", "power_score", "bet_flag_tier", "bet_flag_reason"] if c in export.columns]

        # Always write CSV game sheets first. GitHub runners do not always have openpyxl installed,
        # so the XLSX workbook is optional instead of a hard dependency that can kill the board.
        game_dir = f"outputs/hr_game_sheets_{date_str}"
        os.makedirs(game_dir, exist_ok=True)

        for i, (game_name, g) in enumerate(export.groupby(game_col, dropna=False), start=1):
            game_sort_cols = [c for c in ["decision_score", "p_hr_1plus_sim", "matchup_score_clean"] if c in g.columns]
            g2 = g.sort_values(game_sort_cols, ascending=[False] * len(game_sort_cols)).copy() if game_sort_cols else g.copy()
            top3 = g2.head(3)
            summary_rows.append({"game": game_name, "top_3": " | ".join(top3["player_name"].astype(str).tolist()), "best_stack": ", ".join(top3.loc[top3["stack_flag"].isin(["SUPER", "LIVE", "PERFECT_STORM", "STACKED"]), "player_name"].astype(str).tolist()) if "stack_flag" in top3.columns else "", "best_prob": round(float(pd.to_numeric(g2["p_hr_1plus_sim"], errors="coerce").max() * 100.0), 1) if len(g2) and "p_hr_1plus_sim" in g2.columns else 0.0})
            safe_name = re.sub(r"[^A-Za-z0-9_@.-]+", "_", str(game_name)).strip("_")[:80] or f"game_{i}"
            _build_locked_betting_export(g2).to_csv(os.path.join(game_dir, f"{i:02d}_{safe_name}.csv"), index=False)

        pd.DataFrame(summary_rows).to_csv(f"outputs/hr_top3_by_game_{date_str}.csv", index=False)

        # Optional workbook for local/desktop use. If openpyxl is missing, skip it cleanly.
        try:
            import openpyxl  # noqa: F401
            sheets_path = f"outputs/hr_game_sheets_{date_str}.xlsx"
            with pd.ExcelWriter(sheets_path, engine="openpyxl") as writer:
                locked_export.to_excel(writer, sheet_name="Full Slate", index=False)
                for game_name, g in export.groupby(game_col, dropna=False):
                    game_sort_cols = [c for c in ["decision_score", "p_hr_1plus_sim", "matchup_score_clean"] if c in g.columns]
                    g2 = g.sort_values(game_sort_cols, ascending=[False] * len(game_sort_cols)).copy() if game_sort_cols else g.copy()
                    safe_sheet = re.sub(r"[\/*?:\[\]]", "", str(game_name))[:31] or "Game"
                    _build_locked_betting_export(g2).to_excel(writer, sheet_name=safe_sheet, index=False)
                pd.DataFrame(summary_rows).to_excel(writer, sheet_name="Top 3 by Game", index=False)
        except ModuleNotFoundError:
            print("[exports] openpyxl not installed; wrote CSV game sheets instead of XLSX workbook.")
        except Exception as e:
            print(f"[exports] XLSX workbook skipped ({e}); CSV game sheets were written.")
...
def write_outputs(board: pd.DataFrame, date_str: str, top_n: int):
    ...
    board = enrich_supercomputer_output(board)
    csv_path = f"outputs/hr_board_{date_str}.csv"
    html_path = f"outputs/hr_board_{date_str}.html"

    _build_locked_betting_export(board).to_csv(csv_path, index=False)
    ...
    show = _build_locked_betting_export(show)
    display_cols = [c for c in ["Name", "Tm", "Opp", "HR%", "Tier", "Conf", "Score", "Value", "Edge", "Pitcher", "DR", "PR", "Old_SR", "Flags", "Notes"] if c in show.columns]
    html = f"""
    <html>
    <head>
    ...
    <body>
      <h2>HR Supercomputer Board — {date_str} (Top {top_n})</h2>
      <p>Percentages shown for HR probabilities. STACK flag highlights advantageous pitcher-matchup spots.</p>
      {show[display_cols].to_html(index=False, escape=True)}
    </body>
    </html>
    """
...
def auto_settle_yesterday_hr_results(run_date_str: str, log_path: str = PERF_LOG_PATH):
    ...
    log_df = build_hr_profile_score(df)
...
def main():
    ...
    board = ensure_board_schema(board, args.date)

    # Defensive date aliases for downstream helpers that still expect game_date.
    if board is not None and not board.empty:
        if "date" not in board.columns:
            board["date"] = args.date
        board["date"] = board["date"].astype(str)
        if "game_date" not in board.columns:
            board["game_date"] = board["date"]
        else:
    ...
    try:
        ...
        print(f"[market_clv] skipped because required column was missing: {e}")
    except Exception as e:
        print(f"[market_clv] skipped after non-fatal error: {e}")
    board = ensure_board_schema(board, args.date)
    ...
    # Preserve the real full slate for export, then build a trimmed betting view separately.
    full_board = ensure_board_schema(board.copy(), args.date)
    write_supercomputer_exports(full_board, args.date, top_n=args.top)
    ...
    print(enrich_supercomputer_output(board).head(25).to_string(index=False))


if __name__ == "__main__":
    main()

def build_hr_profile_score(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    ...
    out["recent_norm"] = safe_norm(out.get("recent_damage_score", 0))
    out["matchup_norm"] = safe_norm(out.get("daily_matchup_hr_score", 0))
    out["power_norm"] = safe_norm(out.get("power_gate_mult", 0))
    out["prob_norm"] = safe_norm(out.get("p_hr_1plus_sim", 0))

    if "env_mult" not in out.columns:
        out["env_mult"] = out.get("temp_mult", 1.0) * out.get("wind_mult", 1.0)

    out["env_norm"] = safe_norm(out["env_mult"])

    out["hr_profile_score"] = (
        0.40 * out["recent_norm"] +
        0.30 * out["matchup_norm"] +
        0.20 * out["env_norm"] +
        0.10 * out["power_norm"]
    )
