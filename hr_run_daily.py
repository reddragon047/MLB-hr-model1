...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    # Export view: probability-first, with matchup context as tiebreakers.
    export_board = board.sort_values(
        ["p_hr_1plus_sim", "matchup_score_clean", "recent_damage_score", "prob_rank"],
        ascending=[False, False, False, True]
    ).reset_index(drop=True).copy()
    export_board["rank"] = np.arange(1, len(export_board) + 1)
    export_board = _ensure_game_key(export_board)

    cols = [
        "rank","prob_rank","player_name","team","opponent","game_key","probable_pitcher_faced","venue",
    ...
    write_game_sheets(export_board[cols].copy(), date_str=date_str, top_n=top_n)
...
def _make_game_sheet_name(game_key: str, max_len: int = 31) -> str:
    """Excel-safe sheet names."""
    safe = str(game_key or "Unknown Game")
    for ch in ["\\", "/", "*", "?", ":", "[", "]"]:
        safe = safe.replace(ch, "-")
    safe = re.sub(r"\s+", " ", safe).strip()
    return (safe[:max_len] or "Game")


def _ensure_game_key(df: pd.DataFrame) -> pd.DataFrame:
    """Create a stable per-game key for exports and anti-overlap workflow."""
    out = df.copy()
    if "game_key" in out.columns:
        return out

    venue = out["venue"].astype(str) if "venue" in out.columns else pd.Series(["Unknown Venue"] * len(out), index=out.index)
    pitcher = out["probable_pitcher_faced"].astype(str) if "probable_pitcher_faced" in out.columns else pd.Series(["Unknown Pitcher"] * len(out), index=out.index)

    if "opponent" in out.columns:
        opp = out["opponent"].astype(str)
        out["game_key"] = venue + " | vs " + opp + " | " + pitcher
    else:
        out["game_key"] = venue + " | " + pitcher

    return out


def write_game_sheets(board: pd.DataFrame, date_str: str, top_n: int = 75):
    """Write one workbook with one sheet per game, plus per-game CSVs."""
    if board is None or board.empty:
        return

    os.makedirs("outputs/game_sheets", exist_ok=True)
    game_board = _ensure_game_key(board)

    sort_cols = [c for c in ["p_hr_1plus_sim", "daily_matchup_hr_score", "matchup_score_clean", "recent_damage_score"] if c in game_board.columns]
    asc = [False] * len(sort_cols)
    if sort_cols:
        game_board = game_board.sort_values(sort_cols, ascending=asc).reset_index(drop=True)

    preferred_cols = [
        "rank", "prob_rank", "player_name", "team", "opponent", "game_key", "probable_pitcher_faced", "venue",
        "p_hr_1plus_sim", "p_hr_2plus_sim", "p_hr_pa", "matchup_label",
        "pitch_match_flag", "pitcher_risk_flag", "recent_power_flag", "bullpen_flag",
        "flag_score", "flags", "display_line", "daily_matchup_hr_score",
        "matchup_score_clean", "recent_damage_score", "pitch_mix_fit_score", "pitcher_hr_score", "bullpen_score"
    ]
    export_cols = [c for c in preferred_cols if c in game_board.columns]
    if not export_cols:
        export_cols = list(game_board.columns)

    xlsx_path = f"outputs/hr_game_sheets_{date_str}.xlsx"
    with pd.ExcelWriter(xlsx_path, engine="openpyxl") as writer:
        for game_key, game_df in game_board.groupby("game_key", dropna=False):
            one_game = game_df.copy()
            if sort_cols:
                one_game = one_game.sort_values(sort_cols, ascending=asc).reset_index(drop=True)
            one_game.insert(0, "game_rank", np.arange(1, len(one_game) + 1))
            sheet_name = _make_game_sheet_name(game_key)
            one_game[(["game_rank"] + export_cols) if "game_rank" not in export_cols else export_cols].to_excel(writer, sheet_name=sheet_name, index=False)

            safe_file = re.sub(r"[^A-Za-z0-9._ -]+", "", str(sheet_name)).replace(" ", "_")
            one_game.head(top_n)[(["game_rank"] + export_cols) if "game_rank" not in export_cols else export_cols].to_csv(
                f"outputs/game_sheets/{safe_file}_{date_str}.csv", index=False
            )
...
def write_outputs(board: pd.DataFrame, date_str: str, top_n: int):
    if "rank_score" not in board.columns and "p_hr_1plus_sim" in board.columns:
        board["rank_score"] = board["p_hr_1plus_sim"]
    board = _ensure_game_key(board)
    sort_cols = [c for c in ["p_hr_1plus_sim", "daily_matchup_hr_score", "matchup_score_clean"] if c in board.columns]
    if sort_cols:
        board = board.sort_values(sort_cols, ascending=[False] * len(sort_cols)).reset_index(drop=True)
    ...
    write_game_sheets(board, date_str=date_str, top_n=top_n)
...
def _coalesce_date_column(df: pd.DataFrame, target: str = "date") -> pd.DataFrame:
    """Return a copy with a normalized date column if any common alias exists.

    This hardens the pipeline against upstream shape changes like `game_date`,
    `gameDate`, `game_datetime`, etc. If no alias exists, the target column is
    created and filled with NaN so downstream code can fail soft instead of
    ...
    aliases = [
        "game_date", "gameDate", "game_dt", "date_str",
        "game_datetime", "gameDateTime", "datetime",
        "date_x", "date_y", "game_date_x", "game_date_y",
    ]
    for col in aliases:
...
def _recompute_cum_and_dd(log_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = _coalesce_date_column(log_df, target="date")
    df["date"] = pd.to_datetime(df["date"], errors="coerce").dt.date.astype(str)

    df["units"] = pd.to_numeric(df.get("units", 0), errors="coerce").fillna(0.0)

    # stable sort
    sort_cols = [c for c in ["date", "player_name"] if c in df.columns]
    if sort_cols:
...
def main():
    ...
    board = board.sort_values(["p_hr_1plus_sim", "daily_matchup_hr_score"], ascending=False).reset_index(drop=True)
