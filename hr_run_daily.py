...
def make_bet_flag(
    ...
    """
    Flag strong pitch-fit + recent-power spots.
    """
    reasons = []
    if dominant_pitch and dominant_usage >= 0.42 and batter_vs_pitch_ratio >= 1.12:
        reasons.append(f"{dominant_pitch} fit")
    if recent_barrel_rate >= 0.11:
        reasons.append("barrels")
    if recent_hard_hit_rate >= 0.45:
    ...
    if ("barrels" in reasons or "hard-hit" in reasons) and dominant_pitch and dominant_usage >= 0.42 and batter_vs_pitch_ratio >= 1.12:
        return 1, "; ".join(reasons[:4])
    if p1 >= 0.14 and len(reasons) >= 3:
        return 1, "; ".join(reasons[:4])
    return 0, ""
    ...
    for g in games:
        ...
        for side in ["away", "home"]:
            ...
            for hid in hitter_ids:
                ...
                matchup_mult = float(np.clip(pt_mult * recent_form_mult, 0.90, 1.18))
                ...
                bet_flag, bet_flag_reason = make_bet_flag(
                    p1=p1,
