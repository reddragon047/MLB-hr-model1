...
MODEL_VERSION = "v5_power_air_park_full16_xgb"
...
def compute_park_factors(stat_df: pd.DataFrame, k_barrel: int = 1500, k_fb: int = 2000, k_hand: int = 1200) -> pd.DataFrame:
    """Compute park HR conversion multipliers keyed by home_team.

    Outputs:
      - park_barrel_mult: HR per barrel multiplier
      - park_fb_mult: HR per fly-ball multiplier
      - park_hand_mult_L / park_hand_mult_R: handedness-specific HR multipliers
      - park_hand_mult: neutral average of L/R handedness multipliers
    """
    df = stat_df.copy()
    ...
    if df.empty:
        return pd.DataFrame(columns=[
            ...
            "park_hand_mult_L", "park_hand_mult_R", "park_hand_mult",
    ...
    park["park_hand_mult_L"] = 1.0
    park["park_hand_mult_R"] = 1.0
    if "stand" in df.columns:
        for hand in ("L", "R"):
            sub = df[df["stand"].astype(str).str.upper().eq(hand)].copy()
            if sub.empty:
                continue
            league_hr_pa_hand = float(sub["is_hr"].sum() / max(1.0, len(sub)))
            hand_park = sub.groupby("home_team").agg(
                pa=("is_hr", "size"),
                hr=("is_hr", "sum"),
            ).reset_index()
            hand_park[f"park_hand_mult_{hand}"] = hand_park.apply(
                lambda r: shrink_rate(
                    successes=r["hr"],
                    trials=r["pa"],
                    prior_mean=max(league_hr_pa_hand, 1e-6),
                    prior_strength=int(k_hand),
                ) / max(league_hr_pa_hand, 1e-6),
                axis=1,
            )
            park = park.merge(hand_park[["home_team", f"park_hand_mult_{hand}"]], on="home_team", how="left", suffixes=("", "_new"))
            if f"park_hand_mult_{hand}_new" in park.columns:
                park[f"park_hand_mult_{hand}"] = park[f"park_hand_mult_{hand}_new"].fillna(park[f"park_hand_mult_{hand}"])
                park.drop(columns=[f"park_hand_mult_{hand}_new"], inplace=True)

    park["park_hand_mult"] = park[[c for c in ["park_hand_mult_L", "park_hand_mult_R"] if c in park.columns]].mean(axis=1).fillna(1.0)
...
def add_batter_park_exposure_features(stat_all: pd.DataFrame, bat_all: pd.DataFrame, park_df: pd.DataFrame) -> pd.DataFrame:
    """Add season-level park exposure features to batter training rows."""
    bat_all = bat_all.copy()
    for c in ["park_barrel_mult", "park_fb_mult", "park_hand_mult"]:
        if c not in bat_all.columns:
            bat_all[c] = 1.0

    if stat_all is None or stat_all.empty or park_df is None or park_df.empty:
        return bat_all

    pa = stat_all.copy()
    pa["season"] = pd.to_datetime(pa.get("game_date"), errors="coerce").dt.year
    pa = pa[pa["events"].notna()].copy()
    if pa.empty or "batter" not in pa.columns or "home_team" not in pa.columns:
        return bat_all

    park_cols = ["home_team", "park_barrel_mult", "park_fb_mult", "park_hand_mult", "park_hand_mult_L", "park_hand_mult_R"]
    park_tmp = park_df[[c for c in park_cols if c in park_df.columns]].copy()
    pa = pa.merge(park_tmp, on="home_team", how="left")

    if "stand" in pa.columns:
        stand = pa["stand"].astype(str).str.upper().str[:1]
        pa["park_hand_mult"] = np.where(
            stand.eq("L"),
            pd.to_numeric(pa.get("park_hand_mult_L", 1.0), errors="coerce").fillna(pd.to_numeric(pa.get("park_hand_mult", 1.0), errors="coerce").fillna(1.0)),
            np.where(
                stand.eq("R"),
                pd.to_numeric(pa.get("park_hand_mult_R", 1.0), errors="coerce").fillna(pd.to_numeric(pa.get("park_hand_mult", 1.0), errors="coerce").fillna(1.0)),
                pd.to_numeric(pa.get("park_hand_mult", 1.0), errors="coerce").fillna(1.0),
            ),
        )

    pa["park_barrel_mult"] = pd.to_numeric(pa.get("park_barrel_mult", 1.0), errors="coerce").fillna(1.0)
    pa["park_fb_mult"] = pd.to_numeric(pa.get("park_fb_mult", 1.0), errors="coerce").fillna(1.0)
    pa["park_hand_mult"] = pd.to_numeric(pa.get("park_hand_mult", 1.0), errors="coerce").fillna(1.0)

    park_exp = pa.groupby(["batter", "season"]).agg(
        park_barrel_mult=("park_barrel_mult", "mean"),
        park_fb_mult=("park_fb_mult", "mean"),
        park_hand_mult=("park_hand_mult", "mean"),
    ).reset_index()

    bat_all = bat_all.merge(park_exp, on=["batter", "season"], how="left", suffixes=("", "_new"))
    for c in ["park_barrel_mult", "park_fb_mult", "park_hand_mult"]:
        new_c = f"{c}_new"
        if new_c in bat_all.columns:
            bat_all[c] = pd.to_numeric(bat_all[new_c], errors="coerce").fillna(pd.to_numeric(bat_all[c], errors="coerce").fillna(1.0))
            bat_all.drop(columns=[new_c], inplace=True)
...
def get_training_cached(train_seasons: list[int]) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    bat_all = add_batter_park_exposure_features(stat_all, bat_all, park)
...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    handL_map = dict(zip(park_df["home_team"], park_df.get("park_hand_mult_L", park_df.get("park_hand_mult", pd.Series(dtype=float)))))
    handR_map = dict(zip(park_df["home_team"], park_df.get("park_hand_mult_R", park_df.get("park_hand_mult", pd.Series(dtype=float)))))
    ...
    for g in games:
        ...
        for side in ["away", "home"]:
            ...
            for hid in hitter_ids:
                ...
                batter_hand = ""
                try:
                    _, batter_hand = platoon.get_handedness(int(hid))
                except Exception:
                    batter_hand = ""
                batter_hand = (batter_hand or "").upper()[:1]
                park_hand_mult_live = float(handL_map.get(home, 1.0)) if batter_hand == "L" else float(handR_map.get(home, 1.0)) if batter_hand == "R" else 1.0
                feat.loc[:, "park_barrel_mult"] = float(barrel_mult)
                feat.loc[:, "park_fb_mult"] = float(fb_mult)
                feat.loc[:, "park_hand_mult"] = float(park_hand_mult_live)

                p_raw = float(model.predict(feat[FEATURE_COLS])[0])
    ...
    # Final display order = 60/40 matchup/probability blend with smooth sim boosts
    ...
    matchup_min, matchup_max = matchup_vals.min(), matchup_vals.max()
    prob_min, prob_max = prob_vals.min(), prob_vals.max()

    if pd.isna(matchup_min) or pd.isna(matchup_max) or matchup_max <= matchup_min:
        board["matchup_norm"] = 0.5
    else:
        board["matchup_norm"] = ((matchup_vals - matchup_min) / (matchup_max - matchup_min)).clip(0.0, 1.0)

    if pd.isna(prob_min) or pd.isna(prob_max) or prob_max <= prob_min:
        board["prob_norm"] = 0.5
    else:
        board["prob_norm"] = ((prob_vals - prob_min) / (prob_max - prob_min)).clip(0.0, 1.0)

    board["final_score"] = (0.60 * board["matchup_norm"]) + (0.40 * board["prob_norm"])
    board.loc[prob_vals >= 0.22, "final_score"] += 0.08
    board.loc[(prob_vals >= 0.18) & (prob_vals < 0.22), "final_score"] += 0.05
    board.loc[(prob_vals >= 0.15) & (prob_vals < 0.18), "final_score"] += 0.025

    board = board.sort_values(
        ["final_score", "matchup_score_clean", "p_hr_1plus_sim", "recent_damage_score"],
        ascending=False
    ).reset_index(drop=True)
