...
def apply_decision_layer(board: pd.DataFrame) -> pd.DataFrame:
    ...
    missing_game = col_missing("game") & col_missing("game_pk")
...
def write_supercomputer_exports(full_board: pd.DataFrame, date_str: str, top_n: int):
    ...
    os.makedirs("outputs", exist_ok=True)
    preferred_cols = ["decision_tier", "decision_score", "confidence_score", "confidence_band", "value_flag", "model_edge", "game_exposure_rank", "decision_view", "decision_reason", "super_rank", "prob_rank", "rank", "player_name", "team", "opponent", "game", "venue", "probable_pitcher_faced", "p_hr_1plus_sim", "p_hr_1plus_calibrated", "post_calibration_boost", "post_calibration_boost_reason", "p_hr_2plus_sim", "super_score", "power_context_blend_score", "power_anchor_floor", "dynamic_prob_cap", "cap_pressure", "boost_strength_factor", "context_flag", "stack_flag", "stack_signal_count", "matchup_label", "scan_flags", "phone_view", "daily_matchup_hr_score", "matchup_score_clean", "pitch_mix_fit_score", "pitcher_hr_score", "environment_clean", "recent_damage_score", "power_score", "context_domination_guard", "bet_flag_tier", "bet_flag_score", "bet_flag_reason", "power_tier", "matchup_tier", "flags", "display_line", "exp_pa", "pitchtype_mult", "pitcher_hr_mult", "weather_mult", "park_factor", "bullpen_score", "date"]
    cols = [c for c in preferred_cols if c in board.columns]
    remaining = [c for c in board.columns if c not in cols]
    export = board[cols + remaining].copy()
    export.to_csv(f"outputs/hr_board_full_slate_{date_str}.csv", index=False)
    export.head(top_n).to_csv(f"outputs/hr_board_top{top_n}_{date_str}.csv", index=False)
    export.head(top_n).to_csv(f"outputs/hr_board_top75_{date_str}.csv", index=False)
    export.to_csv(f"outputs/hr_board_supercomputer_{date_str}.csv", index=False)
    ...
    if game_col:
        ...
        for i, (game_name, g) in enumerate(export.groupby(game_col, dropna=False), start=1):
            g2 = g.sort_values(["super_rank", "p_hr_1plus_sim"], ascending=[True, False]).copy()
            top3 = g2.head(3)
            ...
            g2[out_cols_master].to_csv(os.path.join(game_dir, f"{i:02d}_{safe_name}.csv"), index=False)
        ...
        try:
            ...
            with pd.ExcelWriter(sheets_path, engine="openpyxl") as writer:
                export.to_excel(writer, sheet_name="Full Slate", index=False)
                for game_name, g in export.groupby(game_col, dropna=False):
                    g2 = g.sort_values(["super_rank", "p_hr_1plus_sim"], ascending=[True, False]).copy()
                    safe_sheet = re.sub(r"[\/*?:\[\]]", "", str(game_name))[:31] or "Game"
                    g2[out_cols_master].to_excel(writer, sheet_name=safe_sheet, index=False)
...
def write_outputs(board: pd.DataFrame, date_str: str, top_n: int):
    ...
    display_cols = [c for c in ["decision_tier", "decision_score", "confidence_score", "value_flag", "super_rank", "prob_rank", "player_name", "team", "probable_pitcher_faced", "p_hr_1plus_sim", "stack_flag", "matchup_label", "decision_view", "scan_flags"] if c in show.columns]
    html = f"""
    <html>
    <head>
