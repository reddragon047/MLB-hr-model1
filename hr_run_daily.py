...
def _safe_round(df: pd.DataFrame, cols, digits=4):
    out = df.copy()
    for c in cols:
        if c in out.columns:
...
def env_label_from_score(v):
    try:
        if pd.isna(v):
            return "?"
    except Exception:
        return "?"
    # Tuned for the current environment_score scale in this script
    if v >= 0.60:
        return "🔥"
    if v >= 0.45:
        return "👍"
...
def build_mobile_views(board: pd.DataFrame) -> dict:
    b = board.copy()
    b = force_sequential_ranks(b)

    # Normalize a phone-friendly environment signal
    if "env_score" not in b.columns:
        if "environment_score" in b.columns:
            b["env_score"] = pd.to_numeric(b["environment_score"], errors="coerce")
        else:
            # Fallback composite using what this script already produces
            pieces = []
            for c in ["matchup_score_today", "park_factor", "weather_mult", "pitcher_hr_mult"]:
                if c in b.columns:
                    pieces.append(pd.to_numeric(b[c], errors="coerce"))
            if pieces:
                tmp = sum(pieces) / len(pieces)
                b["env_score"] = pd.to_numeric(tmp, errors="coerce")
            else:
                b["env_score"] = np.nan

    b["env_label"] = b["env_score"].apply(env_label_from_score)

    # Tier buckets based on rank
    def _tier(r):
        try:
            r = int(r)
        except Exception:
    ...
    b["tier_bucket"] = b["rank"].apply(_tier)

    # Decision board = the readable version for phone
    decision_cols = [
        "rank", "tier_bucket", "player_name", "team", "probable_pitcher_faced", "venue",
        "p_hr_1plus_sim", "fair_odds_hr", "bet_flag_tier", "bet_flag_reason",
        "power_tier", "matchup_tier", "env_score", "env_label",
        "pitcher_hr_mult", "park_factor", "weather_mult",
        "power_score", "matchup_score_today", "recent_damage_score"
    ]
    decision_cols = [c for c in decision_cols if c in b.columns]
    decision = b[decision_cols].copy()
    decision = _safe_round(decision, [
        "p_hr_1plus_sim", "fair_odds_hr", "env_score", "pitcher_hr_mult",
        "park_factor", "weather_mult", "power_score", "matchup_score_today",
        "recent_damage_score"
    ], 4)

    # Compact = just enough to pick from on a phone
    compact_cols = [
        "rank", "player_name", "team", "probable_pitcher_faced",
        "p_hr_1plus_sim", "fair_odds_hr", "env_label", "env_score"
    ]
    compact_cols = [c for c in compact_cols if c in b.columns]
    compact = b[b["rank"] <= 35][compact_cols].copy()
    compact = _safe_round(compact, ["p_hr_1plus_sim", "fair_odds_hr", "env_score"], 4)

    tier1 = decision[decision["rank"] <= 15].sort_values(
        ["env_score", "p_hr_1plus_sim"], ascending=[False, False]
    ).copy()

    tier2 = decision[(decision["rank"] >= 16) & (decision["rank"] <= 35)].sort_values(
        ["env_score", "p_hr_1plus_sim"], ascending=[False, False]
    ).copy()

    tier3 = decision[(decision["rank"] >= 36) & (decision["rank"] <= 60)].sort_values(
        ["env_score", "p_hr_1plus_sim"], ascending=[False, False]
    ).copy()
...
def write_outputs(board: pd.DataFrame, date_str: str, top_n: int):
    ...
    # Root files for GitHub artifact convenience
    board.to_csv("hr_board.csv", index=False)

    views = build_mobile_views(board)
    views["decision"].to_csv("hr_board_decision.csv", index=False)
    views["compact"].to_csv("hr_board_top35_compact.csv", index=False)
    views["tier1"].to_csv("hr_board_tier1.csv", index=False)
    views["tier2"].to_csv("hr_board_tier2.csv", index=False)
    views["tier3"].to_csv("hr_board_tier3.csv", index=False)

    views["decision"].to_csv(f"outputs/hr_board_decision_{date_str}.csv", index=False)
    views["compact"].to_csv(f"outputs/hr_board_top35_compact_{date_str}.csv", index=False)
    views["tier1"].to_csv(f"outputs/hr_board_tier1_{date_str}.csv", index=False)
    views["tier2"].to_csv(f"outputs/hr_board_tier2_{date_str}.csv", index=False)
    views["tier3"].to_csv(f"outputs/hr_board_tier3_{date_str}.csv", index=False)

    debug = pd.DataFrame([{
        "date": date_str,
        "rows_total": len(board),
        "rows_top35": int((board["rank"] <= 35).sum()) if "rank" in board.columns else min(len(board), 35),
        "rows_tier1": int((board["rank"] <= 15).sum()) if "rank" in board.columns else min(len(board), 15),
        "rows_tier2": int(((board["rank"] >= 16) & (board["rank"] <= 35)).sum()) if "rank" in board.columns else 0,
        "rows_tier3": int(((board["rank"] >= 36) & (board["rank"] <= 60)).sum()) if "rank" in board.columns else 0,
    }])
    debug.to_csv("hr_debug.csv", index=False)
    debug.to_csv(f"outputs/hr_debug_{date_str}.csv", index=False)
