...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
        ...
        for side in ["away", "home"]:
            ...
            for hid in hitter_ids:
                ...
                rows.append(
                    {
                        "date": date_str,
                        "game_pk": int(game_pk) if game_pk is not None else np.nan,
                        "home_team": home,
                        "away_team": away,
                        "team": batting_team,
                        "venue": venue,
    ...
    board = board.sort_values(
        ["p_hr_1plus_sim", "matchup_score_clean", "recent_damage_score"],
    ...
    # Export view: probability first, then matchup score / recent damage as tiebreakers.
...
def _sanitize_game_filename(value, max_len=60):
    s = str(value or "Unknown_Game")
    s = re.sub(r'[\\/*?:"<>|\[\]]+', '_', s)
    s = re.sub(r'\s+', '_', s)
    s = re.sub(r'_+', '_', s).strip('._ ')
    s = s.encode("ascii", "ignore").decode("ascii")
    return (s[:max_len] or "Unknown_Game")
...
def _ensure_game_key(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "game_key" in out.columns:
        gk = out["game_key"].astype(str).fillna("").str.strip()
        if (gk != "").any():
            out["game_key"] = gk
            return out

    if "game_pk" in out.columns:
        gpk = pd.to_numeric(out["game_pk"], errors="coerce")
        if gpk.notna().any():
            out["game_key"] = "gamepk_" + gpk.fillna(-1).astype(int).astype(str)
            return out

    if {"date", "venue", "home_team", "away_team"}.issubset(out.columns):
        date_part = out["date"].astype(str).fillna("").str.strip()
        venue_part = out["venue"].astype(str).fillna("UnknownVenue").str.strip()
        home_part = out["home_team"].astype(str).fillna("UnknownHome").str.strip()
        away_part = out["away_team"].astype(str).fillna("UnknownAway").str.strip()
        out["game_key"] = date_part + "_" + venue_part + "_" + away_part + "_at_" + home_part
        return out

    if "game" in out.columns:
        game_col = out["game"].astype(str).fillna("").str.strip()
        if (game_col != "").any():
            out["game_key"] = game_col
            return out

    venue = out["venue"].astype(str).fillna("UnknownVenue") if "venue" in out.columns else pd.Series(["UnknownVenue"] * len(out), index=out.index)
    pitcher = out["probable_pitcher_faced"].astype(str).fillna("UnknownPitcher") if "probable_pitcher_faced" in out.columns else pd.Series(["UnknownPitcher"] * len(out), index=out.index)
    out["game_key"] = venue + "_" + pitcher
    return out


def write_game_sheets(board: pd.DataFrame, date_str: str, top_n: int):
    if board is None or board.empty:
        return

    out = _ensure_game_key(board)
    sort_cols = [c for c in ["p_hr_1plus_sim", "matchup_score_clean", "recent_damage_score", "prob_rank"] if c in out.columns]
    ascending = [(c == "prob_rank") for c in sort_cols]

    for game_key, game_df in out.groupby("game_key", dropna=False):
        game_df = game_df.copy()
        if sort_cols:
            game_df = game_df.sort_values(sort_cols, ascending=ascending).reset_index(drop=True)
        game_df.insert(0, "game_rank", np.arange(1, len(game_df) + 1))

        if {"venue", "home_team", "away_team"}.issubset(game_df.columns):
            venue_name = str(game_df["venue"].iloc[0])
            away_name = str(game_df["away_team"].iloc[0])
            home_name = str(game_df["home_team"].iloc[0])
            filename_key = f"{venue_name}_{away_name}_at_{home_name}"
        else:
            filename_key = str(game_key)

        safe_name = _sanitize_game_filename(filename_key)
        game_df.to_csv(f"outputs/hr_game_{safe_name}_{date_str}.csv", index=False)
        game_df.head(int(top_n)).to_csv(f"outputs/hr_game_{safe_name}_{date_str}_top{int(top_n)}.csv", index=False)
...
def write_outputs(board: pd.DataFrame, date_str: str, top_n: int):
    ...
    full_csv_path = f"outputs/hr_board_full_slate_{date_str}.csv"
    top_csv_path = f"outputs/hr_board_top{int(top_n)}_{date_str}.csv"
    html_path = f"outputs/hr_board_{date_str}.html"

    board.to_csv(full_csv_path, index=False)

    show = board.head(int(top_n)).copy()
    show.to_csv(top_csv_path, index=False)
    ...
    <body>
      <h2>HR Probability Board - {date_str} (Top {int(top_n)})</h2>
    ...
    write_game_sheets(board.copy(), date_str=date_str, top_n=top_n)
