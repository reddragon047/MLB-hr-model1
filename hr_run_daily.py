...
def export_individual_game_sheets(board: pd.DataFrame, date_str: str) -> int:
    """Write one top-level CSV per game plus an index. Never raises."""
    try:
        if board is None or board.empty:
        ...
        if "venue" not in df.columns:
            print("[game_sheets] created 0 per-game CSV sheets (missing venue).")
            return 0
        ...
        if "p_hr_1plus_sim" in df.columns:
            ...
            df["p_hr_pct"] = np.nan
        if "matchup_score_clean" in df.columns:
            df["matchup_score_clean"] = pd.to_numeric(df["matchup_score_clean"], errors="coerce")
        else:
        ...
        for gkey, g in df.groupby(group_cols, dropna=False):
            ...
            if g.empty:
                continue
            venue = str(g["venue"].dropna().iloc[0]) if "venue" in g.columns and g["venue"].notna().any() else str(gkey)
            teams = [str(x) for x in g.get("team", pd.Series(dtype=str)).dropna().astype(str).unique().tolist()]
            teams_sorted = sorted(teams)
            matchup_name = "_vs_".join(_safe_sheet_slug(t) for t in teams_sorted) if teams_sorted else _safe_sheet_slug(venue)
            slug = _safe_sheet_slug(f"{matchup_name}_{venue}_{date_str}")
            flat_path = os.path.join(out_dir, f"hr_game_sheet_{slug}.csv")
            nested_path = os.path.join(nested_dir, f"hr_game_sheet_{slug}.csv")
            ...
            for idx, (_, r) in enumerate(top3.iterrows(), start=1):
                top3_rows.append({
                    ...
                    "team": r.get("team", ""),
                    "hr_prob_pct": round(float(pd.to_numeric(r.get("p_hr_pct"), errors="coerce")), 2) if pd.notna(pd.to_numeric(r.get("p_hr_pct"), errors="coerce")) else np.nan,
                    ...
                    "venue": venue,
                    "game_date": date_str,
                    "weather_temp_f": r.get("weather_temp_f", np.nan),
                    "weather_wind_mph": r.get("weather_wind_mph", np.nan),
                    "weather_wind_dir_deg": r.get("weather_wind_dir_deg", np.nan),
                    "wind_impact_label": r.get("wind_impact_label", ""),
                    "pitcher_hr_score": r.get("pitcher_hr_score", np.nan),
                    "pitch_mix_fit_score": r.get("pitch_mix_fit_score", np.nan),
                    "bullpen_score": r.get("bullpen_score", np.nan),
                    "recent_damage_score": r.get("recent_damage_score", np.nan),
                    "dominant_pitch": r.get("dominant_pitch", ""),
                    "dominant_pitch_usage": r.get("dominant_pitch_usage", np.nan),
                    "batter_vs_pitch_ratio": r.get("batter_vs_pitch_ratio", np.nan),
                    "bet_flag_reason": r.get("bet_flag_reason", ""),
                })
            ...
            lineup_cols = [
                ...
                "batter_vs_pitch_ratio","bet_flag_reason","venue","weather_temp_f",
                "weather_wind_mph","weather_wind_dir_deg","wind_impact_label"
            ]
            ...
            lineup.insert(1, "target_rank", np.nan)
            lineup.rename(columns={"p_hr_pct":"hr_prob_pct"}, inplace=True)

            out = pd.concat([pd.DataFrame(top3_rows), lineup], ignore_index=True, sort=False)
            out.to_csv(flat_path, index=False)
            out.to_csv(nested_path, index=False)
            exported.append({
