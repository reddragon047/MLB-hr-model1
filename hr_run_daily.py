...
def apply_post_calibration_context_boost(
    ...
    if m_count >= 3 and e_count >= 2:
        boost += 0.012
        reasons.append("matchup_env_cluster")
    if m_count >= 4 and e_count >= 3:
        boost += 0.010
    ...
    if raw_gap > 0.005 and (m_count >= 3 or e_count >= 3):
        gap_keep = min(0.020, raw_gap * 0.45)
        boost += gap_keep
    ...
    if ctx_strength > 0 and (m_count + e_count) >= 5:
        boost += min(0.012, ctx_strength * 0.050)
    ...
    hard_cap = 0.235 if (flag == "PERFECT_STORM" or (m_count >= 4 and e_count >= 3)) else 0.220
