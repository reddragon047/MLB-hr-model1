...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    # Rebalance final ordering toward true HR probability without changing output layout.
    # Target blend: 60% matchup quality / 40% probability quality.
    prob_score_rank = (100.0 * (1.0 - ((board["prob_rank"] - 1) / max(len(board) - 1, 1)))).clip(0, 100)
    p1 = pd.to_numeric(board["p_hr_1plus_sim"], errors="coerce").fillna(0.0)
    p1_max = float(p1.max()) if len(p1) else 0.0
    if p1_max > 0:
        prob_score_raw = (p1 / p1_max) * 100.0
    else:
        prob_score_raw = pd.Series(0.0, index=board.index)
    prob_score = (0.65 * prob_score_rank + 0.35 * prob_score_raw).clip(0, 100)
    high_prob_boost = np.where(p1 >= 0.22, 8.0,
                        np.where(p1 >= 0.18, 5.0,
                        np.where(p1 >= 0.15, 2.5, 0.0)))
    board["_final_rank_score"] = (
        0.60 * pd.to_numeric(board["matchup_score_clean"], errors="coerce").fillna(0.0) +
        0.40 * prob_score +
        high_prob_boost
    )

    board = board.sort_values(
        ["_final_rank_score","p_hr_1plus_sim","recent_damage_score"],
        ascending=False
    ).reset_index(drop=True)

    board.insert(0,"rank",range(1,len(board)+1))

    if "_final_rank_score" in board.columns:
        board = board.drop(columns=["_final_rank_score"])
