...
def batter_pitcher_tables(stat_df: pd.DataFrame, season: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    bbe["is_barrel_proxy"] = ((bbe["launch_speed"] >= 98) & (bbe["launch_angle"].between(26, 30))).astype(int)
    bbe["is_hard_hit"] = (bbe["launch_speed"] >= 95).astype(int)
    bbe["is_sweet_spot"] = bbe["launch_angle"].between(8, 32).astype(int)
    bbe["is_flyball"] = bbe["launch_angle"].between(20, 50).astype(int)
    bbe["air_power_proxy"] = (
        (bbe["launch_speed"] >= 95) &
        (bbe["launch_angle"].between(20, 40))
    ).astype(int)
    ...
    bat_bbe = bbe.groupby(["batter", "season"]).agg(
        ...
        flyball_rate=("is_flyball", "mean"),
        air_power_rate=("air_power_proxy", "mean"),
    ).reset_index()
    ...
    pit_bbe = bbe.groupby(["pitcher", "season"]).agg(
        ...
        sweet_spot_rate_allowed=("is_sweet_spot", "mean"),
        flyball_rate_allowed=("is_flyball", "mean"),
        air_power_rate_allowed=("air_power_proxy", "mean"),
    ).reset_index()
