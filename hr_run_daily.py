...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    # Normalize environment onto a readable 0.85-1.15 style scale if env_score is not already present
    if "env_score" in board.columns:
        env_series = pd.to_numeric(board["env_score"], errors="coerce").fillna(1.0)
    elif "environment_score" in board.columns:
        env_raw = pd.to_numeric(board["environment_score"], errors="coerce").fillna(100.0)
        env_series = (env_raw / 100.0).clip(lower=0.85, upper=1.15)
        board["env_score"] = env_series
    else:
        env_series = 1.0
        board["env_score"] = 1.0

    board["final_score"] = (
        pd.to_numeric(board["p_hr_1plus_sim"], errors="coerce").fillna(0.0) * 0.60 +
        env_series * 0.25 +
        (pd.to_numeric(board["recent_damage_score"], errors="coerce").fillna(0.0) / 100.0) * 0.15
    )

    board["elite_flag"] = (
        (pd.to_numeric(board["p_hr_1plus_sim"], errors="coerce").fillna(0.0) >= 0.23) &
        (env_series >= 1.00)
    )

    board["spike_flag"] = (
        (pd.to_numeric(board["recent_barrel_rate"], errors="coerce").fillna(0.0) >= 0.12) |
        (pd.to_numeric(board["recent_damage_score"], errors="coerce").fillna(0.0) >= 40.0)
    )
    ...
    preferred_cols = [
        ...
        "final_score", "elite_flag", "spike_flag", "env_score",
...
def force_sequential_ranks(board: pd.DataFrame) -> pd.DataFrame:
    if board is None or board.empty:
        return board
    board = board.sort_values(["final_score", "p_hr_1plus_sim", "p_hr_pa"], ascending=False).reset_index(drop=True)
