...
# Temporary pre-calibration shrink on final HR intensity.
# Keeps mid-tier hitters from running too hot until a true outcome-based
# final calibrator is trained on a larger settled sample.
FINAL_LAMBDA_SHRINK = 0.90
...
def load_stadium_coords():
    candidates = [
        "inputs/stadium_coords.json",
        "stadium_coords.json",
        "inputs/stadium_coords.json.txt",
        "stadium_coords.json.txt",
    ]
    for path in candidates:
        ...
        try:
            ...
            if isinstance(data, dict):
                print(f"[DEBUG] loaded stadium coords from {path} ({len(data)} keys)")
                return data
        except Exception as e:
            print(f"[WARN] failed to load stadium coords from {path}: {e}")
    print("[WARN] no stadium coords file found; weather disabled for this run")
    return {}
...
def get_daily_temp_f(lat: float, lon: float, date_str: str) -> float | None:
    ...
    try:
        ...
        print(f"[WARN] open-meteo returned no daily temperature for {lat},{lon} on {date_str}")
    except Exception as e:
        print(f"[WARN] weather fetch failed for {lat},{lon} on {date_str}: {e}")
        return None
    return None
    ...
    for g in games:
        ...
        if use_weather:
            ...
            if loc and "lat" in loc and "lon" in loc:
                weather_temp_f = get_daily_temp_f(float(loc["lat"]), float(loc["lon"]), date_str)
                w_mult = temp_multiplier(weather_temp_f)
                print(f"[DEBUG] weather {home} / {venue}: temp_f={weather_temp_f}, mult={w_mult:.3f}")
            else:
                print(f"[WARN] no stadium coordinates match for home='{home}' venue='{venue}'")
            ...
            for hid in hitter_ids:
                ...
                if pitcher_id and pitcher_id in mix_map and hid in dmg_map:
                    ...
                    if wsum > 0:
                        ...
                        # ---- Simulate ----
                        lam_raw = p_pa_adj * exp_pa

                        # Temporary pre-calibration shrink: reduce final HR intensity a bit
                        # until a real outcome-based final calibrator is trained.
                        lam = float(np.clip(lam_raw * FINAL_LAMBDA_SHRINK, 1e-9, 10.0))
                        ...
                        final_cal = None
                        final_cal_paths = [
                            "models_hr/final_calibrator.pkl",
                            "models/final_calibrator.pkl",
                            "models_hr/final_calibrator.pk1",
                            "models/final_calibrator.pk1",
                        ]
                        for FINAL_CAL_PATH in final_cal_paths:
                        ...
                        if final_cal is not None:
                            try:
                                p1 = float(np.clip(final_cal.predict([p1])[0], 1e-6, 0.40))
                            except Exception:
                                pass
...
df = df[df["p_hr_1plus_sim"] >= PROB_FLOOR].copy()
df = df.sort_values("p_hr_1plus_sim", ascending=False).head(MAX_ROWS)
