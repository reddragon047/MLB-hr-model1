    ...
    if game_col:
        ...
        with pd.ExcelWriter(sheets_path, engine="openpyxl") as writer:
            ...
            for game_name, g in export.groupby(game_col, dropna=False):
                ...
                top3 = g2.head(3)
                summary_rows.append({"game": game_name, "top_3": " | ".join(top3["player_name"].astype(str).tolist()), "best_stack": ", ".join(top3.loc[top3["stack_flag"].isin(["SUPER", "LIVE"]), "player_name"].astype(str).tolist()), "best_prob": round(float(pd.to_numeric(g2["p_hr_1plus_sim"], errors="coerce").max() * 100.0), 1) if len(g2) else 0.0})
                safe_sheet = re.sub(r"[\/*?:\[\]]", "", str(game_name))[:31] or "Game"
                out_cols = [c for c in ["super_rank", "prob_rank", "player_name", "team", "probable_pitcher_faced", "p_hr_1plus_sim", "stack_flag", "stack_signal_count", "matchup_label", "scan_flags", "phone_view", "super_score", "daily_matchup_hr_score", "matchup_score_clean", "pitcher_hr_score", "pitch_mix_fit_score", "environment_clean", "power_score", "bet_flag_tier", "bet_flag_reason"] if c in g2.columns]
                g2[out_cols].to_excel(writer, sheet_name=safe_sheet, index=False)
