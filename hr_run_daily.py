...
MODEL_VERSION = "v2_recent_form_hybrid_ranker"
...
def ensure_game_date_column(df: pd.DataFrame) -> pd.DataFrame:
    """
    Normalize date columns so downstream code can safely use `game_date`.
    """
    if df is None:
        return df
    if "game_date" not in df.columns:
        if "date" in df.columns:
            df = df.copy()
            df[("game_date" if "game_date" in df.columns else "date")] = df["date"]
        else:
            df = df.copy()
            df[("game_date" if "game_date" in df.columns else "date")] = None
    return df
...
def compute_pitch_matchup_multiplier(
    ...
    if not pitcher_id or pitcher_id not in mix_map or batter_id not in dmg_map:
...
def compute_hybrid_matchup_score(
    ...
    """
    Create a matchup-driven daily score from already-available columns in the v2 pipeline.
    This is not a replacement for probability; it is a ranking layer so the board reacts
    to today's spot instead of repeating yesterday's names.
    Returns: (daily_matchup_hr_score, matchup_tier)
    """
    def score(v, lo, hi):
        try:
            x = float(v)
    ...
    pitch_mix_fit_score = (
        0.45 * score(pitchtype_mult, 0.94, 1.20) +
        0.35 * score(batter_vs_pitch_ratio, 0.92, 1.24) +
        0.20 * score(dominant_pitch_usage, 0.22, 0.60)
    )
    pitcher_vulnerability_score = score(pitcher_hr_mult, 0.94, 1.10)
    environment_score = (
        0.45 * score(matchup_mult, 0.94, 1.18) +
        0.20 * score(park_factor, 0.92, 1.12) +
        0.20 * score(weather_mult, 0.92, 1.12) +
        0.15 * score(exp_pa, 3.6, 5.0)
    )
    recent_damage_score = score(recent_form_mult, 0.96, 1.08)

    daily_matchup_hr_score = (
        0.34 * pitch_mix_fit_score +
        0.26 * pitcher_vulnerability_score +
        0.24 * environment_score +
        0.16 * recent_damage_score
    )
    ...
    return daily_matchup_hr_score, matchup_tier


def compute_rank_score(p1: float, daily_matchup_hr_score: float) -> float:
    """
    Hybrid ranking score:
    60% today's matchup / 40% final calibrated probability.
    Keeps probability honest while letting today's environment move the board.
    """
    return float(
        np.clip(
            0.60 * (float(daily_matchup_hr_score) / 100.0) +
            0.40 * float(p1),
            1e-6,
            0.999,
        )
...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    bat_df = ensure_game_date_column(bat_df)
    pit_df = ensure_game_date_column(pit_df)
    mix_df = ensure_game_date_column(mix_df)
    dmg_df = ensure_game_date_column(dmg_df)
    park_df = ensure_game_date_column(park_df)
    bullpen_df = ensure_game_date_column(bullpen_df)
    platoon_df = ensure_game_date_column(platoon_df)
    ...
    for g in games:
        ...
        for side in ["away", "home"]:
            ...
            for hid in hitter_ids:
                ...
                daily_matchup_hr_score, matchup_tier = compute_hybrid_matchup_score(
                    matchup_mult=matchup_mult,
                    pitchtype_mult=pt_mult,
    ...
    board = board.sort_values(["rank_score", "daily_matchup_hr_score", "p_hr_1plus_sim", "p_hr_pa"], ascending=False).reset_index(drop=True)
    board.insert(0, "rank", np.arange(1, len(board) + 1))
    preferred_cols = [
        "rank", "player_name", "bet_flag_tier", "bet_flag_score", "bet_flag", "bet_flag_reason",
        "team", "probable_pitcher_faced", "venue", "rank_score", "daily_matchup_hr_score", "matchup_tier", "p_hr_1plus_sim", "p_hr_2plus_sim", "p_hr_pa",
        "exp_pa", "matchup_mult", "pitchtype_mult", "recent_form_mult", "pitcher_hr_mult", "park_factor", "weather_mult",
        "dominant_pitch", "dominant_pitch_usage", "batter_vs_pitch_ratio", "recent_power_score", "pitcher_hr_score",
        "recent_barrel_rate", "recent_hard_hit_rate", "recent_ev", "batter_id", "date"
    ]
...
def force_sequential_ranks(board: pd.DataFrame) -> pd.DataFrame:
    if board is None or board.empty:
        return board
    sort_cols = [c for c in ["rank_score", "daily_matchup_hr_score", "p_hr_1plus_sim", "p_hr_pa"] if c in board.columns]
    if sort_cols:
...
def main():
    ...
    PROB_FLOOR = 0.12  # 12%
    board = board[board["p_hr_1plus_sim"] >= PROB_FLOOR].copy()
    board = board.sort_values("p_hr_1plus_sim", ascending=False)
