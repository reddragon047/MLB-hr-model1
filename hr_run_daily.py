...
MODEL_VERSION = "v4_park_xgb_retrain"
...
def ensure_recent_feature_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Ensure recent-form and contact-shape columns always exist so training/inference never crash."""
    fallback_map = {
...
def batter_pitcher_tables(stat_df: pd.DataFrame, season: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if "stand" not in pa.columns:
        pa["stand"] = "R"
    pa["stand"] = pa["stand"].astype(str).str.upper().str[0]
    pa.loc[~pa["stand"].isin(["L", "R"]), "stand"] = "R"
    ...
    season_park = compute_park_factors(df)
    if not season_park.empty and "home_team" in pa.columns:
        pa_park = pa[["batter", "season", "home_team", "stand"]].copy()
        pa_park = pa_park.merge(
            season_park[[
                "home_team",
                "park_barrel_mult",
                "park_fb_mult",
        ...
        pa_park["park_barrel_mult"] = pd.to_numeric(pa_park["park_barrel_mult"], errors="coerce").fillna(1.0)
        pa_park["park_fb_mult"] = pd.to_numeric(pa_park["park_fb_mult"], errors="coerce").fillna(1.0)
        pa_park["park_hand_mult"] = np.where(
            pa_park["stand"].eq("L"),
            (pd.to_numeric(pa_park["park_barrel_mult_L"], errors="coerce").fillna(pa_park["park_barrel_mult"]) ** 0.55) *
            (pd.to_numeric(pa_park["park_fb_mult_L"], errors="coerce").fillna(pa_park["park_fb_mult"]) ** 0.45),
            (pd.to_numeric(pa_park["park_barrel_mult_R"], errors="coerce").fillna(pa_park["park_barrel_mult"]) ** 0.55) *
            (pd.to_numeric(pa_park["park_fb_mult_R"], errors="coerce").fillna(pa_park["park_fb_mult"]) ** 0.45),
        )
        bat_park = pa_park.groupby(["batter", "season"]).agg(
            park_barrel_mult=("park_barrel_mult", "mean"),
            park_fb_mult=("park_fb_mult", "mean"),
            park_hand_mult=("park_hand_mult", "mean"),
        ).reset_index()
        bat = bat.merge(bat_park, on=["batter", "season"], how="left")
    else:
        bat["park_barrel_mult"] = 1.0
        bat["park_fb_mult"] = 1.0
        bat["park_hand_mult"] = 1.0

    for c in ["park_barrel_mult", "park_fb_mult", "park_hand_mult"]:
        bat[c] = pd.to_numeric(bat[c], errors="coerce").fillna(1.0).clip(0.85, 1.20)
...
def compute_pitcher_hr_profile_multiplier(
    ...
    Adds air-contact depth using fly-ball, sweet-spot, and hard-hit rates allowed,
    but keeps the final effect bounded so the board doesn't get distorted.
    """
    ...
    hard_hit_rate_allowed = float(p.get("hard_hit_rate_allowed", pit_league.get("hard_hit_rate_allowed", 0.40)) or pit_league.get("hard_hit_rate_allowed", 0.40))
    sweet_spot_rate_allowed = float(p.get("sweet_spot_rate_allowed", pit_league.get("sweet_spot_rate_allowed", 0.34)) or pit_league.get("sweet_spot_rate_allowed", 0.34))
    flyball_rate_allowed = float(p.get("flyball_rate_allowed", pit_league.get("flyball_rate_allowed", 0.27)) or pit_league.get("flyball_rate_allowed", 0.27))
    air_power_rate_allowed = float(p.get("air_power_rate_allowed", pit_league.get("air_power_rate_allowed", 0.16)) or pit_league.get("air_power_rate_allowed", 0.16))
    ...
    league_hh = max(float(pit_league.get("hard_hit_rate_allowed", 0.40) or 0.40), 1e-6)
    league_ss = max(float(pit_league.get("sweet_spot_rate_allowed", 0.34) or 0.34), 1e-6)
    league_fb = max(float(pit_league.get("flyball_rate_allowed", 0.27) or 0.27), 1e-6)
    league_air = max(float(pit_league.get("air_power_rate_allowed", 0.16) or 0.16), 1e-6)

    la_center = 14.0
    la_edge = 8.0
    la_score = 1.0 + 0.05 * np.clip((avg_la_allowed - la_center) / la_edge, -1.0, 1.0)

    raw = (
        1.0
        + 0.30 * ((hr_rate_allowed / league_hr) - 1.0)
        + 0.24 * ((barrel_rate_allowed / league_barrel) - 1.0)
        + 0.08 * ((hard_hit_rate_allowed / league_hh) - 1.0)
        + 0.10 * ((sweet_spot_rate_allowed / league_ss) - 1.0)
        + 0.12 * ((flyball_rate_allowed / league_fb) - 1.0)
        + 0.16 * ((air_power_rate_allowed / league_air) - 1.0)
        + 0.010 * (avg_ev_allowed - league_ev)
    ) * la_score

    raw = float(np.clip(raw, 0.87, 1.18))
    confidence = float(np.clip(min(pa / 350.0, bbe / 120.0), 0.0, 1.0))
    mult = 1.0 + (raw - 1.0) * confidence
    mult = float(np.clip(mult, 0.94, 1.11))

    score = float(np.clip((mult - 1.0) * 100.0, -11.0, 11.0))
    return mult, score
...
def compute_park_factors(stat_df: pd.DataFrame, k_barrel: int = 1500, k_fb: int = 2000) -> pd.DataFrame:
    """Compute overall and handedness-specific park HR conversion multipliers keyed by home_team."""
    df = stat_df.copy()
    ...
    if df.empty:
        return pd.DataFrame(columns=[
            ...
            "park_barrel_mult_L", "park_fb_mult_L",
            "park_barrel_mult_R", "park_fb_mult_R"
        ])

    df["is_hr"] = (df["events"] == "home_run").astype(int)
    la = pd.to_numeric(df["launch_angle"], errors="coerce").fillna(0.0)
    ev = pd.to_numeric(df["launch_speed"], errors="coerce").fillna(0.0)
    ...
    if "stand" not in df.columns:
        df["stand"] = "R"
    df["stand"] = df["stand"].astype(str).str.upper().str[0]
    df.loc[~df["stand"].isin(["L", "R"]), "stand"] = "R"

    def _agg(group: pd.DataFrame) -> pd.Series:
        barrels = float(group["is_barrel_proxy"].sum())
        hrs_on_barrels = float((group["is_hr"] * group["is_barrel_proxy"]).sum())
        flyballs = float(group["is_fb_proxy"].sum())
        hrs_on_flyballs = float((group["is_hr"] * group["is_fb_proxy"]).sum())
        return pd.Series({
            "barrels": barrels,
            "hr_on_barrels": hrs_on_barrels,
            "flyballs": flyballs,
            "hr_on_flyballs": hrs_on_flyballs,
        })

    def _with_shrink(park_slice: pd.DataFrame, league_hr_per_barrel: float, league_hr_per_fb: float, suffix: str = "") -> pd.DataFrame:
        if park_slice.empty:
            return pd.DataFrame(columns=["home_team", f"park_barrel_mult{suffix}", f"park_fb_mult{suffix}"])
        park = park_slice.groupby("home_team").apply(_agg).reset_index()
        park[f"hr_per_barrel_shrunk{suffix}"] = park.apply(
        ...
        park[f"hr_per_fb_shrunk{suffix}"] = park.apply(
            lambda r: shrink_rate(
        ...
        park[f"park_barrel_mult{suffix}"] = park[f"hr_per_barrel_shrunk{suffix}"] / max(float(league_hr_per_barrel), 1e-6)
        park[f"park_fb_mult{suffix}"] = park[f"hr_per_fb_shrunk{suffix}"] / max(float(league_hr_per_fb), 1e-6)
        return park[["home_team", f"park_barrel_mult{suffix}", f"park_fb_mult{suffix}"]]
    ...
    overall = _with_shrink(df, league_hr_per_barrel, league_hr_per_fb, "")
    left = _with_shrink(df[df["stand"] == "L"].copy(), league_hr_per_barrel, league_hr_per_fb, "_L")
    right = _with_shrink(df[df["stand"] == "R"].copy(), league_hr_per_barrel, league_hr_per_fb, "_R")

    out = overall.merge(left, on="home_team", how="left").merge(right, on="home_team", how="left")
    for col in ["park_barrel_mult_L", "park_fb_mult_L", "park_barrel_mult_R", "park_fb_mult_R"]:
        base = "park_barrel_mult" if "barrel" in col else "park_fb_mult"
        if col not in out.columns:
            out[col] = out[base]
        out[col] = out[col].fillna(out[base]).clip(0.85, 1.20)

    out["park_barrel_mult"] = out["park_barrel_mult"].fillna(1.0).clip(0.85, 1.20)
    out["park_fb_mult"] = out["park_fb_mult"].fillna(1.0).clip(0.85, 1.20)
    return out
...
def get_training_cached(train_seasons: list[int]) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    required_bat_cols = {"hard_hit_rate", "sweet_spot_rate", "flyball_rate", "air_power_rate"}
    required_pit_cols = {"hard_hit_rate_allowed", "sweet_spot_rate_allowed", "flyball_rate_allowed", "air_power_rate_allowed"}
    if all(os.path.exists(p) for p in [bat_path, pit_path, mix_path, dmg_path, park_path, bp_path, platoon_path]):
        bat_cached = pd.read_parquet(bat_path)
        pit_cached = pd.read_parquet(pit_path)
        if required_bat_cols.issubset(set(bat_cached.columns)) and required_pit_cols.issubset(set(pit_cached.columns)):
            return (
                bat_cached,
                pit_cached,
...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    for _c in FEATURE_COLS:
        if _c not in league_feat.columns:
            league_feat[_c] = 0.0
    for extra_col in ["hard_hit_rate", "flyball_rate", "sweet_spot_rate", "air_power_rate"]:
        if extra_col not in bat_latest.columns:
            bat_latest[extra_col] = 0.0
        if extra_col not in bat_df.columns:
            bat_df[extra_col] = 0.0
        league_feat[extra_col] = float(pd.to_numeric(bat_df.get(extra_col, 0.0), errors="coerce").mean())
    ...
    barrel_map_l = dict(zip(park_df["home_team"], park_df.get("park_barrel_mult_L", park_df.get("park_barrel_mult", pd.Series(dtype=float)))))
    fb_map_l = dict(zip(park_df["home_team"], park_df.get("park_fb_mult_L", park_df.get("park_fb_mult", pd.Series(dtype=float)))))
    barrel_map_r = dict(zip(park_df["home_team"], park_df.get("park_barrel_mult_R", park_df.get("park_barrel_mult", pd.Series(dtype=float)))))
    fb_map_r = dict(zip(park_df["home_team"], park_df.get("park_fb_mult_R", park_df.get("park_fb_mult", pd.Series(dtype=float)))))
    ...
    pit_league = {
        ...
        "hard_hit_rate_allowed": float(pd.to_numeric(pit_df.get("hard_hit_rate_allowed", pd.Series(dtype=float)), errors="coerce").mean()) if pit_df is not None and "hard_hit_rate_allowed" in pit_df.columns else 0.40,
        "sweet_spot_rate_allowed": float(pd.to_numeric(pit_df.get("sweet_spot_rate_allowed", pd.Series(dtype=float)), errors="coerce").mean()) if pit_df is not None and "sweet_spot_rate_allowed" in pit_df.columns else 0.34,
        "flyball_rate_allowed": float(pd.to_numeric(pit_df.get("flyball_rate_allowed", pd.Series(dtype=float)), errors="coerce").mean()) if pit_df is not None and "flyball_rate_allowed" in pit_df.columns else 0.27,
        "air_power_rate_allowed": float(pd.to_numeric(pit_df.get("air_power_rate_allowed", pd.Series(dtype=float)), errors="coerce").mean()) if pit_df is not None and "air_power_rate_allowed" in pit_df.columns else 0.16,
    }
    ...
    for g in games:
        ...
        park_contact_mult = (barrel_mult ** 0.58) * (fb_mult ** 0.42)
        park_contact_mult = float(np.clip(park_contact_mult, 0.82, 1.18))
        ...
        for side in ["away", "home"]:
            ...
            for hid in hitter_ids:
                ...
                env_mult = float(np.clip(park_contact_mult * w_mult, 0.78, 1.32))
                ...
                flyball_rate = float(feat.iloc[0].get("flyball_rate", 0.0))
                sweet_spot_rate = float(feat.iloc[0].get("sweet_spot_rate", 0.0))
                hard_hit_rate = float(feat.iloc[0].get("hard_hit_rate", recent_hard_hit_rate))
                air_power_rate = float(feat.iloc[0].get("air_power_rate", 0.0))

                hitter_air_mult = 1.0 + (
                    0.05 * np.clip((flyball_rate - 0.24) / 0.16, -1.0, 1.0)
                    + 0.05 * np.clip((sweet_spot_rate - 0.32) / 0.16, -1.0, 1.0)
                    + 0.06 * np.clip((air_power_rate - 0.14) / 0.10, -1.0, 1.0)
                    + 0.03 * np.clip((hard_hit_rate - 0.40) / 0.18, -1.0, 1.0)
                )
                hitter_air_mult = float(np.clip(hitter_air_mult, 0.94, 1.12))

                # ---- Final per-PA probability ----
                matchup_mult = float(np.clip(pt_mult * recent_form_mult * pitcher_hr_mult * hitter_air_mult, 0.90, 1.24))
    ...
    batter_heat = (
        0.38 * board["recent_damage_score"] +
        0.28 * (board["recent_barrel_rate"]*100) +
        0.16 * (board["recent_hard_hit_rate"]*100) +
        0.08 * to_100_score(pd.to_numeric(board.get("p_hr_1plus_sim", 0), errors="coerce").fillna(0.0), 0.08, 0.24) +
        0.10 * to_100_score(pd.to_numeric(board.get("matchup_mult", 1), errors="coerce").fillna(1.0), 0.96, 1.14)
    ).clip(0,100)
    ...
    board = board.sort_values(
        ["matchup_score_clean","p_hr_1plus_sim","recent_damage_score"],
        ascending=False
    ...
    board.head(50)[cols].to_csv(f"outputs/hr_board_top50_{date_str}.csv",index=False)
    ...
    # Display order = matchup, probability as tiebreaker
    board = board.sort_values(
        ["matchup_score_clean", "p_hr_1plus_sim", "recent_damage_score"],
        ascending=False
    ).reset_index(drop=True)
