...
def apply_post_calibration_context_boost(
    ...
    """
    Final-day escape hatch applied AFTER isotonic/smooth calibration.

    Calibration keeps the board honest over time, but it can flatten rare same-day
    spike spots. This layer lets only stacked context spots keep part of their
    matchup/environment edge in the final displayed probability.

    Returns:
      boosted_probability, absolute_boost_added, boost_reason
    """
    try:
    ...
    # Base flat boosts: these are intentionally AFTER calibration so Marsh-type
    # spots do not get crushed back into the 10-12% bucket.
    boost = 0.0
    reasons = []
    if flag == "PERFECT_STORM":
        boost += 0.050
        reasons.append("perfect_storm_plus")
    elif flag == "STACKED":
        boost += 0.032
        reasons.append("stacked_plus")
    elif flag == "LIVE":
        boost += 0.018
        reasons.append("live_plus")

    # Explicit all-aspects trigger: matchup + environment both aligned.
    if m_count >= 3 and e_count >= 2:
        boost += 0.030
        reasons.append("true_spike_override")
    if m_count >= 4 and e_count >= 3:
        boost += 0.018
        reasons.append("deep_cluster")
    if m_count >= 5 and e_count >= 4:
        boost += 0.010
        reasons.append("slate_breaker_cluster")

    # Preserve a controlled piece of the pre-calibration day-specific edge.
    # If calibration already kept it, this term is small/zero.
    raw_gap = max(0.0, raw - p)
    if raw_gap > 0.005 and (m_count >= 3 or e_count >= 3):
        gap_keep = min(0.030, raw_gap * 0.60)
        boost += gap_keep
        reasons.append("raw_gap_escape")

    # Small multiplier-based kicker only for genuinely above-neutral context.
    ctx_strength = max(0.0, (m_mult - 1.0) + (e_mult - 1.0) + 0.50 * (ctx_mult - 1.0))
    if ctx_strength > 0 and (m_count + e_count) >= 5:
        boost += min(0.020, ctx_strength * 0.075)
        reasons.append("context_strength")

    # Don't reward weak baseline power gates as aggressively.
    if pg < 0.82:
        boost *= 0.45
    ...
    # Realistic one-game HR ceiling. Elite perfect-storm spots can reach the low
    # 20s, but the board cannot inflate into fantasy 30%+ numbers.
    hard_cap = 0.245 if (flag == "PERFECT_STORM" or (m_count >= 4 and e_count >= 3)) else 0.232 if (flag == "STACKED" or (m_count >= 3 and e_count >= 2)) else 0.220
    boosted = float(np.clip(p + boost, 1e-6, hard_cap))
    actual_boost = float(max(0.0, boosted - p))
    reason = "+".join(reasons) if reasons else "none"
    return boosted, actual_boost, reason
    ...
    for g in games:
        ...
        for side in ["away", "home"]:
            ...
            for hid in hitter_ids:
                ...
                p1, post_calibration_boost, post_calibration_boost_reason = apply_post_calibration_context_boost(
