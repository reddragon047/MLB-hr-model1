...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    def _bucket_series(s: pd.Series, high_q: float, med_q: float) -> pd.Series:
        vals = pd.to_numeric(s, errors="coerce")
        if vals.notna().sum() == 0:
            return pd.Series(["MED"] * len(vals), index=vals.index)
        hi = vals.quantile(high_q)
        med = vals.quantile(med_q)
        return pd.Series(
            np.where(vals >= hi, "HIGH", np.where(vals >= med, "MED", "LOW")),
            index=vals.index,
        )

    board["pitch_match_flag"] = _bucket_series(board.get("pitch_mix_fit_score", 0), 0.75, 0.40)
    board["pitcher_risk_flag"] = _bucket_series(board.get("pitcher_hr_score", 0), 0.70, 0.35)
