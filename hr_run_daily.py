...
def build_pitch_mix_and_batter_damage(stat_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Build pitcher pitch mix and batter HR/BBE-vs-pitch tables.

    We intentionally use HR per batted-ball event instead of HR per plate appearance,
    then shrink each batter/pitch-type cell toward the league pitch-type baseline.
    This keeps the signal HR-specific and reduces noise from tiny split samples.
    """
    df = stat_df.copy()
    ...
    bbe = df[df["pitch_type"].isin(PITCH_TYPES)].copy()
    bbe = bbe[bbe["launch_speed"].notna() & bbe["launch_angle"].notna()].copy()
    bbe["is_hr"] = (bbe["events"] == "home_run").astype(int)

    bat_pt = bbe.groupby(["batter", "pitch_type"]).agg(
        BBE=("is_hr", "size"),
        HR=("is_hr", "sum"),
    ).reset_index()

    league = bat_pt.groupby("pitch_type").agg(BBE=("BBE", "sum"), HR=("HR", "sum")).reset_index()
    league["league_hr_bbe"] = league["HR"] / league["BBE"].clip(lower=1)

    bat_pt = bat_pt.merge(league[["pitch_type", "league_hr_bbe"]], on="pitch_type", how="left")
    bat_pt["hr_bbe_shrunk"] = bat_pt.apply(
        lambda r: shrink_rate(r["HR"], r["BBE"], r["league_hr_bbe"], prior_strength=200),
        axis=1,
    )

    damage = bat_pt.pivot_table(index="batter", columns="pitch_type", values="hr_bbe_shrunk", aggfunc="mean").fillna(np.nan)
    damage.columns = [f"bat_hr_bbe_{c}" for c in damage.columns]
    damage = damage.reset_index()

    bbe_ct = bat_pt.pivot_table(index="batter", columns="pitch_type", values="BBE", aggfunc="sum").fillna(0)
    bbe_ct.columns = [f"bat_bbe_{c}" for c in bbe_ct.columns]
    bbe_ct = bbe_ct.reset_index()

    bat_tot = bat_pt.groupby("batter", as_index=False)["BBE"].sum().rename(columns={"BBE": "bat_bbe_total"})
    damage = damage.merge(bbe_ct, on="batter", how="left").merge(bat_tot, on="batter", how="left")
...
def compute_pitch_mix_hr_multiplier(
    mix_row: dict,
    dmg_row: dict,
    baseline_hr_bbe: float,
    max_conf_bbe: int = 300,
) -> float:
    """Pitch-mix HR multiplier using pitcher usage × batter HR/BBE-vs-pitch.

    Confidence-weighted toward 1.0 so small batter samples do not create noisy boosts.
    """
    weighted = 0.0
    wsum = 0.0
    for pt in PITCH_TYPES:
        usage = float(mix_row.get(f"pit_usage_{pt}", 0.0) or 0.0)
        dmg = dmg_row.get(f"bat_hr_bbe_{pt}", np.nan)
        if usage > 0 and pd.notna(dmg):
            weighted += usage * float(dmg)
            wsum += usage

    if wsum <= 0 or baseline_hr_bbe <= 0:
        return 1.0

    raw_mult = float(np.clip(weighted / max(baseline_hr_bbe, 1e-6), 0.75, 1.35))
    batter_bbe = float(dmg_row.get("bat_bbe_total", 0.0) or 0.0)
    confidence = float(np.clip(batter_bbe / max_conf_bbe, 0.0, 1.0))
    return float(1.0 + (raw_mult - 1.0) * confidence)
        ...
        for side in ["away", "home"]:
            ...
            for hid in hitter_ids:
                ...
                if pitcher_id and pitcher_id in mix_map and hid in dmg_map:
                    ...
                    dmg = dmg_map[hid]
                    baseline = float(meta.get("league_hr_per_bbe", meta.get("league_hr_bbe", 0.06)))
                    pt_mult = compute_pitch_mix_hr_multiplier(mix, dmg, baseline_hr_bbe=baseline)
                ...
                rows.append(
                    {
                        ...
                        "recent_barrel_rate": round(float(feat.iloc[0].get("recent_barrel_rate", 0.0)), 4),
                        "recent_hard_hit_rate": round(float(feat.iloc[0].get("recent_hard_hit_rate", 0.0)), 4),
                        "recent_ev": round(float(feat.iloc[0].get("recent_ev", 0.0)), 2),
                    }
                )
