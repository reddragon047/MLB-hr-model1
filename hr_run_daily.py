...
def _wind_out_in_components(wind_speed_mph: float | None,
    ...
    # diff ~= 0 means wind is blowing in from CF; diff ~= 180 means out to CF
...
def _park_weather_override(home_team: str,
    ...
    # Wrigley = massive weather amplifier
    ...
    # Coors gets downgraded hard in cold/snow; otherwise modest persistent boost
    if team == "Colorado Rockies":
        if (temp_f is not None and temp_f <= 45) or ((snowfall_cm or 0) > 0):
            return 0.70
        return 1.08

    # Yankee Stadium gets a short-porch / RF carry bump
    if team == "New York Yankees":
        if out_mph >= 8:
            return 1.06 if out_mph < 12 else 1.10

    # Great American tends to really play in warmth
    if team == "Cincinnati Reds":
        if temp_f is not None and temp_f >= 75:
            return 1.05

    # Oracle / T-Mobile / Petco are extra suppressive with cold or incoming wind
    if team in {"San Diego Padres", "Seattle Mariners", "San Francisco Giants"}:
        if in_mph >= 10:
            return 0.90 if team != "San Diego Padres" else 0.88
        if temp_f is not None and temp_f <= 55:
            return 0.95 if team != "San Diego Padres" else 0.93
