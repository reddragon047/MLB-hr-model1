...
def sharpened_pitchtype_multiplier(
    pt_mult: float | int | None,
    dominant_pitch_usage: float | int | None,
    batter_vs_pitch_ratio: float | int | None,
) -> float:
    """Slightly sharpen stable pitch-mix signal without letting it hijack the board."""
    try:
        base = float(pt_mult)
    except Exception:
        base = 1.0
    ...
    base = float(np.clip(base, 0.93, 1.10))
    usage = float(np.clip(usage, 0.0, 0.80))
    ratio = float(np.clip(ratio, 0.75, 1.35))

    if usage < 0.25:
        return float(np.clip(base, 0.95, 1.08))

    usage_strength = float(np.clip((usage - 0.25) / 0.30, 0.0, 1.0))
    ratio_edge = float(np.clip(ratio - 1.0, -0.18, 0.18))
    sharpen = 1.0 + ratio_edge * (0.35 + 0.25 * usage_strength)
    sharpen = float(np.clip(sharpen, 0.94, 1.08))
    out = base * sharpen
    return float(np.clip(out, 0.93, 1.12))


def exp_bbe_multiplier(
    k_rate: float | int | None,
    exp_pa: float | int | None,
) -> float:
    """Small expected-contact multiplier based on projected opportunities after strikeout drag."""
    try:
        k = float(k_rate)
    except Exception:
        k = 0.22
    ...
    k = float(np.clip(k, 0.08, 0.40))
    pa = float(np.clip(pa, 2.5, 5.5))
    expected_bbe = pa * max(0.0, 1.0 - k)
    league_expected_bbe = 4.1 * (1.0 - 0.22)
    raw = expected_bbe / max(league_expected_bbe, 1e-6)
    raw = float(np.clip(raw, 0.88, 1.12))
    mult = 1.0 + (raw - 1.0) * 0.60
    return float(np.clip(mult, 0.90, 1.08))


def recent_power_spike_multiplier(
    recent_barrel_rate: float | int | None,
    recent_hard_hit_rate: float | int | None,
    recent_ev: float | int | None,
) -> float:
    """Tiny recent-power kicker for true spike games only."""
    try:
    ...
    if not np.isfinite(rbr):
        rbr = 0.0
    if not np.isfinite(rhh):
        rhh = 0.0
    if not np.isfinite(rev):
        rev = 88.5

    raw = 1.0 + 0.20 * (rbr - 0.08) + 0.08 * (rhh - 0.40) + 0.006 * (rev - 89.0)
    raw = float(np.clip(raw, 0.94, 1.08))
    mult = 1.0 + (raw - 1.0) * 0.70
    return float(np.clip(mult, 0.96, 1.06))


def temp_multiplier(temp_f: float | int | None) -> float:
    """Conservative temperature effect centered near neutral spring conditions."""
    try:
        t = float(temp_f)
    except Exception:
        return 1.0
...
def wind_multiplier(
    wind_speed_mph: float | int | None,
    wind_dir_deg: float | int | None,
    out_to_cf_deg: float | int | None,
) -> float:
    """Conservative directional wind adjustment using wind direction as coming FROM."""
    try:
        speed = float(wind_speed_mph)
    except Exception:
        return 1.0
    try:
        wdir = float(wind_dir_deg)
        out = float(out_to_cf_deg)
    except Exception:
        return 1.0

    if not np.isfinite(speed) or not np.isfinite(wdir) or not np.isfinite(out):
        return 1.0

    speed = float(np.clip(speed, 0.0, 30.0))
    toward_deg = (wdir + 180.0) % 360.0
    diff = abs(((toward_deg - out + 180.0) % 360.0) - 180.0)
    out_component = float(np.cos(np.deg2rad(diff)))
    raw = 1.0 + 0.012 * speed * out_component
    return float(np.clip(raw, 0.88, 1.12))
...
def _report_num(v, default=np.nan):
    try:
...
def _build_game_key_columns(board: pd.DataFrame) -> pd.DataFrame:
    out = board.copy()
    if "game_key" in out.columns:
        out["_game_key"] = out["game_key"].fillna("")
        bad = out["_game_key"].astype(str).str.strip().eq("")
    else:
        out["_game_key"] = ""
        bad = pd.Series(True, index=out.index)
    if bad.any():
        venue = out["venue"].astype(str) if "venue" in out.columns else pd.Series("Unknown Venue", index=out.index)
        if "team" in out.columns:
            team_lists = out.groupby(venue.fillna("Unknown Venue"))["team"].transform(
                lambda s: " vs ".join(sorted({str(x).strip() for x in s.dropna() if str(x).strip()})) or "Unknown Teams"
            )
        else:
            team_lists = pd.Series("Unknown Teams", index=out.index)
        out.loc[bad, "_game_key"] = venue.fillna("Unknown Venue").astype(str) + " | " + team_lists.astype(str)
    out["_game_key"] = out["_game_key"].astype(str).str.replace(r"\s+", " ", regex=True).str.strip()
    return out


def _game_slug(s: str) -> str:
    s = re.sub(r"[^A-Za-z0-9]+", "_", str(s or "game")).strip("_")
    return s[:140] if s else "game"


def _report_wind_text(game_df: pd.DataFrame) -> str:
    wind_mph = None
    for col in ["weather_wind_mph", "wind_speed_mph", "wind_mph"]:
        if col in game_df.columns and game_df[col].notna().any():
            wind_mph = game_df[col].dropna().iloc[0]
            break
    wind_dir = ""
    for col in ["wind_impact_label", "wind_direction_label", "wind_dir_label"]:
        if col in game_df.columns and game_df[col].notna().any():
            wind_dir = str(game_df[col].dropna().iloc[0]).strip()
            break
    parts = []
    if wind_mph is not None and str(wind_mph) != "":
        try:
            parts.append(f"{float(wind_mph):.0f} mph")
        except Exception:
            parts.append(str(wind_mph))
    if wind_dir:
        parts.append(wind_dir)
    return " | ".join(parts) if parts else "Unknown"


def _report_game_score(game_df: pd.DataFrame) -> float:
    temp_f = np.nan
    for col in ["weather_temp_f", "temp_f", "temperature_f"]:
        if col in game_df.columns and game_df[col].notna().any():
            temp_f = _report_num(game_df[col].dropna().iloc[0], np.nan)
            break
    wind_mph = np.nan
    for col in ["weather_wind_mph", "wind_speed_mph", "wind_mph"]:
        if col in game_df.columns and game_df[col].notna().any():
            wind_mph = _report_num(game_df[col].dropna().iloc[0], np.nan)
            break
    wind_dir = ""
    for col in ["wind_impact_label", "wind_direction_label", "wind_dir_label"]:
        if col in game_df.columns and game_df[col].notna().any():
            wind_dir = str(game_df[col].dropna().iloc[0]).lower()
            break
    pitcher_hr = _report_num(game_df["pitcher_hr_score"].mean(), 0.0) if "pitcher_hr_score" in game_df.columns else 0.0
    bullpen = _report_num(game_df["bullpen_score"].mean(), 0.0) if "bullpen_score" in game_df.columns else 0.0
    score = 1.0
    if pd.notna(temp_f):
        if temp_f >= 80:
            score += 0.08
            ...
            score -= 0.05
    if pd.notna(wind_mph):
        if "out" in wind_dir:
            score += 0.08 if wind_mph >= 10 else 0.04
        elif "in" in wind_dir:
            score -= 0.08 if wind_mph >= 10 else 0.04
    score += min(max((pitcher_hr - 5.0) / 100.0, -0.05), 0.08)
    score += min(max((bullpen - 30.0) / 150.0, -0.03), 0.05)
    return round(score, 2)


def _report_game_tag(score: float) -> str:
    if score >= 1.12:
        return "ATTACK"
    if score <= 0.94:
        return "FADE"
...
def _report_pitcher_summary(team_df: pd.DataFrame) -> str:
    if team_df is None or team_df.empty:
        return "No opposing pitcher context available"
    pitcher = ""
    if "probable_pitcher_faced" in team_df.columns and team_df["probable_pitcher_faced"].notna().any():
        pitcher = _report_text(team_df["probable_pitcher_faced"].dropna().iloc[0], "")
    hr_score = _report_num(team_df["pitcher_hr_score"].mean(), np.nan) if "pitcher_hr_score" in team_df.columns else np.nan
    pitch_fit = _report_num(team_df["pitch_mix_fit_score"].mean(), np.nan) if "pitch_mix_fit_score" in team_df.columns else np.nan
    bullpen = _report_num(team_df["bullpen_score"].mean(), np.nan) if "bullpen_score" in team_df.columns else np.nan
    notes = []
    if pitcher:
        notes.append(pitcher)
    if pd.notna(hr_score):
        if hr_score >= 8:
            notes.append("HR-prone starter")
        elif hr_score <= 4:
            notes.append("HR suppressor")
        else:
            notes.append(f"starter HR score {hr_score:.1f}")
    if pd.notna(pitch_fit):
        if pitch_fit >= 60:
            notes.append("favorable pitch mix")
        elif pitch_fit <= 40:
            notes.append("tough pitch mix")
    if pd.notna(bullpen):
        if bullpen >= 45:
            notes.append("weak bullpen behind him")
        elif bullpen <= 20:
            notes.append("strong bullpen behind him")
    return "; ".join(notes) if notes else "No opposing pitcher context available"


def export_game_scouting_sheets(board: pd.DataFrame, date_str: str) -> None:
    if board is None or board.empty:
        return
    ensure_dirs()
    game_dir = os.path.join("outputs", "game_sheets")
    os.makedirs(game_dir, exist_ok=True)

    working = _build_game_key_columns(board)
    if "p_hr_1plus_sim" in working.columns:
        working["hr_prob_pct"] = (pd.to_numeric(working["p_hr_1plus_sim"], errors="coerce") * 100.0).round(2)
    else:
        working["hr_prob_pct"] = np.nan
    if "matchup_score_clean" not in working.columns:
        working["matchup_score_clean"] = np.nan
    else:
        working["matchup_score_clean"] = pd.to_numeric(working["matchup_score_clean"], errors="coerce")
    working["_target_sort"] = pd.to_numeric(working.get("hr_prob_pct"), errors="coerce").fillna(0) * 1000 + pd.to_numeric(working.get("matchup_score_clean"), errors="coerce").fillna(0)

    exported = []
    for game_key, game_df in working.groupby("_game_key", dropna=False):
        game_df = game_df.copy().reset_index(drop=True)
        teams = [str(x).strip() for x in game_df.get("team", pd.Series(dtype=object)).dropna() if str(x).strip()]
        teams = sorted(set(teams))
        matchup = f"{teams[0]} @ {teams[1]}" if len(teams) >= 2 else _report_text(game_key, "Unknown Matchup")
        venue = _report_text(game_df["venue"].dropna().iloc[0], "Unknown Venue") if "venue" in game_df.columns and game_df["venue"].notna().any() else "Unknown Venue"
        temp_f = ""
        for col in ["weather_temp_f", "temp_f", "temperature_f"]:
            if col in game_df.columns and game_df[col].notna().any():
                temp_f = game_df[col].dropna().iloc[0]
                break
        score = _report_game_score(game_df)
        tag = _report_game_tag(score)
        wind_text = _report_wind_text(game_df)
        park_factor = game_df["park_factor"].dropna().iloc[0] if "park_factor" in game_df.columns and game_df["park_factor"].notna().any() else ""
        weather_mult = game_df["weather_mult"].dropna().iloc[0] if "weather_mult" in game_df.columns and game_df["weather_mult"].notna().any() else ""

        sort_cols = ["_target_sort"] + (["rank"] if "rank" in game_df.columns else [])
        asc = [False] + ([True] if "rank" in game_df.columns else [])
        top3 = game_df.sort_values(sort_cols, ascending=asc).head(3).copy()

        rows = []
        def add(section, key, value):
            rows.append({"section": section, "key": key, "value": value})
        add("GAME_INFO", "matchup", matchup)
        add("GAME_INFO", "venue", venue)
        add("GAME_INFO", "date", date_str)
        add("ENVIRONMENT", "temp_f", temp_f)
        add("ENVIRONMENT", "wind", wind_text)
        add("ENVIRONMENT", "park_factor", park_factor)
        add("ENVIRONMENT", "weather_multiplier", weather_mult)
        add("ENVIRONMENT", "game_score", score)
        add("ENVIRONMENT", "game_tag", tag)
        for tm in teams:
            tdf = game_df[game_df["team"].astype(str) == tm].copy() if "team" in game_df.columns else pd.DataFrame()
            add("PITCHING", f"{tm}_opposing_pitcher", _report_pitcher_summary(tdf))
        for idx, (_, r) in enumerate(top3.iterrows(), start=1):
            bits = []
            prob = _report_num(r.get("hr_prob_pct"), np.nan)
            if pd.notna(prob):
                bits.append(f"{prob:.1f}% HR")
            rank_val = _report_num(r.get("rank"), np.nan)
            if pd.notna(rank_val):
                bits.append(f"Rank {rank_val:.0f}")
            label = _report_text(r.get("matchup_label", ""), "")
            if label:
                bits.append(label)
            fit = _report_num(r.get("pitch_mix_fit_score"), np.nan)
            if pd.notna(fit):
                if fit >= 60:
                    bits.append("elite pitch fit")
                elif fit >= 50:
                    bits.append("good pitch fit")
            add("TOP_TARGET", str(idx), f"{_report_text(r.get('player_name',''))} ({_report_text(r.get('team',''))}) | " + " | ".join(bits))
        team_notes = []
        if "team" in game_df.columns:
            for tm, tdf in game_df.groupby("team", dropna=False):
                tdf = tdf.sort_values(sort_cols, ascending=asc)
                if not tdf.empty:
                    p = _report_num(tdf.iloc[0].get("hr_prob_pct"), np.nan)
                    team_notes.append(f"{tm}: {tdf.iloc[0].get('player_name','')} ({f'{p:.1f}%' if pd.notna(p) else 'n/a'})")
        play_type = "Play 2-3 bats from this game" if tag == "ATTACK" else ("One-off only or full fade" if tag == "FADE" else "Play 1-2 bats max")
        add("RECOMMENDATION", "play_type", play_type)
        add("RECOMMENDATION", "notes", " ; ".join(team_notes[:2]) if team_notes else "Use top targets only")

        report_df = pd.DataFrame(rows)
        hitter_cols = [c for c in ["team", "player_name", "probable_pitcher_faced", "rank", "prob_rank", "hr_prob_pct", "matchup_label", "matchup_score_clean", "pitcher_hr_score", "pitch_mix_fit_score", "bullpen_score", "recent_damage_score", "dominant_pitch", "dominant_pitch_usage", "batter_vs_pitch_ratio", "bet_flag_reason", "bet_flag_tier", "bet_flag_score"] if c in game_df.columns]
        hitters = game_df[hitter_cols].copy()
        if "dominant_pitch_usage" in hitters.columns:
            hitters["dominant_pitch_usage"] = (pd.to_numeric(hitters["dominant_pitch_usage"], errors="coerce") * 100.0).round(1)
        for col in ["hr_prob_pct", "matchup_score_clean", "pitcher_hr_score", "pitch_mix_fit_score", "bullpen_score", "recent_damage_score", "batter_vs_pitch_ratio"]:
            if col in hitters.columns:
                hitters[col] = pd.to_numeric(hitters[col], errors="coerce").round(2)
        hitters.insert(0, "section", "ALL_HITTERS")
        hitters = hitters.sort_values([c for c in ["rank", "prob_rank", "player_name"] if c in hitters.columns], ascending=[True, True, True][:len([c for c in ["rank", "prob_rank", "player_name"] if c in hitters.columns])])

        csv_name = f"hr_game_sheet_{_game_slug(game_key)}_{date_str}.csv"
        flat_path = os.path.join("outputs", csv_name)
        nested_path = os.path.join(game_dir, csv_name)
        ...
        with open(nested_path, "w", encoding="utf-8", newline="") as f:
            ...
            f.write("\n")
            hitters.to_csv(f, index=False)
        exported.append({"game_sheet_file": csv_name, "matchup": matchup, "venue": venue, "game_tag": tag, "game_score": score, "top_target_1": top3.iloc[0]["player_name"] if len(top3) >= 1 else "", "top_target_2": top3.iloc[1]["player_name"] if len(top3) >= 2 else "", "top_target_3": top3.iloc[2]["player_name"] if len(top3) >= 3 else ""})

    if exported:
        pd.DataFrame(exported).to_csv(os.path.join("outputs", f"hr_game_sheet_index_{date_str}.csv"), index=False)
...
def write_outputs(board: pd.DataFrame, date_str: str, top_n: int):
    ...
    try:
        export_game_scouting_sheets(board, date_str)
    except Exception as e:
        print(f"[warn] game sheet export failed: {e}")
