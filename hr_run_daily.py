...
def _sheet_safe_name(name: str, fallback: str = "Game") -> str:
...
def _game_slug(name: str) -> str:
    raw = unicodedata.normalize("NFKD", str(name or "game")).encode("ascii", "ignore").decode("ascii")
    raw = raw.lower()
...
def _build_game_key_columns(board: pd.DataFrame) -> pd.DataFrame:
    ...
    if bad.any():
        ...
        if "venue" not in out.columns:
            venue = pd.Series("Unknown Venue", index=out.index)
...
def export_game_scouting_sheets(board: pd.DataFrame, date_str: str) -> None:
    ...
    working = working.sort_values([c for c in ["_game_key", "rank", "p_hr_1plus_sim"] if c in working.columns], ascending=[True, True, False][:len([c for c in ["_game_key", "rank", "p_hr_1plus_sim"] if c in working.columns])]).reset_index(drop=True)

    workbook_path = os.path.join("outputs", f"hr_game_sheets_{date_str}.xlsx")
    writer = None
    try:
    ...
    for game_key, game_df in working.groupby("_game_key", dropna=False):
        ...
        venue = _first_present(game_df, ["venue"], default="Unknown Venue")
        teams = sorted({str(x).strip() for x in game_df.get("team", pd.Series(dtype=object)).dropna() if str(x).strip()})
        teams_str = " vs ".join(teams) if teams else "Unknown Teams"
        temp_f = _first_present(game_df, ["weather_temp_f", "temp_f", "temperature_f"])
        wind_mph = _first_present(game_df, ["weather_wind_mph", "wind_speed_mph", "wind_mph"])
        wind_dir = _first_present(game_df, ["wind_impact_label", "wind_direction_label", "wind_dir_label"], default="Neutral")
        wind_deg = _first_present(game_df, ["weather_wind_dir_deg", "wind_dir_deg"])
        park_factor = _first_present(game_df, ["park_factor"])
        weather_mult = _first_present(game_df, ["weather_mult"])

        rows = []
        rows.append({
            "section": "GAME_INFO", "team": "", "player_name": "", "opposing_pitcher": "",
            "venue": venue, "game": game_key, "teams": teams_str, "weather_temp_f": temp_f,
            "weather_wind_mph": wind_mph, "weather_wind_dir_deg": wind_deg, "wind_impact_label": wind_dir,
            "park_factor": park_factor, "weather_mult": weather_mult,
        })

        pitcher_summary = (
            game_df.groupby([c for c in ["team", "probable_pitcher_faced"] if c in game_df.columns], dropna=False)
            .agg(
                hitters_in_sheet=("player_name", "count"),
                avg_rank=("rank", "mean") if "rank" in game_df.columns else ("player_name", "count"),
                avg_prob=("p_hr_1plus_sim", "mean") if "p_hr_1plus_sim" in game_df.columns else ("player_name", "count"),
                max_prob=("p_hr_1plus_sim", "max") if "p_hr_1plus_sim" in game_df.columns else ("player_name", "count"),
                avg_matchup_score=("matchup_score_clean", "mean") if "matchup_score_clean" in game_df.columns else ("player_name", "count"),
                pitcher_hr_score=("pitcher_hr_score", "mean") if "pitcher_hr_score" in game_df.columns else ("player_name", "count"),
                bullpen_score=("bullpen_score", "mean") if "bullpen_score" in game_df.columns else ("player_name", "count"),
            )
            .reset_index()
        ) if {"team", "probable_pitcher_faced"}.issubset(game_df.columns) else pd.DataFrame()

        if not pitcher_summary.empty:
            for _, r in pitcher_summary.iterrows():
                rows.append({
                    "section": "PITCHER_SUMMARY",
                    "team": r.get("team", ""),
                    "player_name": "",
                    "opposing_pitcher": r.get("probable_pitcher_faced", ""),
                    "pitcher_hr_score": r.get("pitcher_hr_score"),
                    "bullpen_score": r.get("bullpen_score"),
                    "avg_matchup_score": r.get("avg_matchup_score"),
                    "avg_rank": r.get("avg_rank"),
                    "avg_hr_prob_pct": (float(r.get("avg_prob")) * 100.0) if pd.notna(r.get("avg_prob")) else np.nan,
                    "max_hr_prob_pct": (float(r.get("max_prob")) * 100.0) if pd.notna(r.get("max_prob")) else np.nan,
                    "hitters_in_sheet": r.get("hitters_in_sheet"),
                })

        hitter_cols = [
            c for c in [
                "team", "player_name", "probable_pitcher_faced", "rank", "prob_rank", "p_hr_1plus_sim",
                ...
                "batter_vs_pitch_ratio", "bet_flag_reason", "bet_flag_tier", "bet_flag_score",
                "weather_temp_f", "weather_wind_mph", "weather_wind_dir_deg", "wind_impact_label", "venue"
            ] if c in game_df.columns
        ]

        hitters = game_df[hitter_cols].copy()
        if "p_hr_1plus_sim" in hitters.columns:
            hitters["hr_prob_pct"] = (pd.to_numeric(hitters["p_hr_1plus_sim"], errors="coerce") * 100.0).round(2)
            hitters = hitters.drop(columns=["p_hr_1plus_sim"])
        ...
        for col in ["matchup_score_clean", "pitcher_hr_score", "pitch_mix_fit_score", "bullpen_score", "recent_damage_score", "batter_vs_pitch_ratio"]:
            if col in hitters.columns:
                hitters[col] = pd.to_numeric(hitters[col], errors="coerce").round(2)
        hitters.insert(0, "section", "HITTER")

        sheet_df = pd.concat([pd.DataFrame(rows), hitters], ignore_index=True, sort=False)
        ordered_cols = [
            "section", "game", "teams", "venue", "team", "opposing_pitcher", "player_name",
            "rank", "prob_rank", "hr_prob_pct", "matchup_label", "matchup_score_clean",
            "pitcher_hr_score", "pitch_mix_fit_score", "bullpen_score", "recent_damage_score",
            "dominant_pitch", "dominant_pitch_usage", "batter_vs_pitch_ratio", "bet_flag_tier",
            "bet_flag_score", "bet_flag_reason", "avg_rank", "avg_hr_prob_pct", "max_hr_prob_pct",
            "avg_matchup_score", "hitters_in_sheet", "weather_temp_f", "weather_wind_mph",
            "weather_wind_dir_deg", "wind_impact_label", "park_factor", "weather_mult"
        ]
        existing_cols = [c for c in ordered_cols if c in sheet_df.columns]
        extra_cols = [c for c in sheet_df.columns if c not in existing_cols]
        sheet_df = sheet_df[existing_cols + extra_cols]

        csv_name = f"{_game_slug(game_key)}_{date_str}.csv"
        sheet_df.to_csv(os.path.join(game_dir, csv_name), index=False)

        if writer is not None:
            try:
                sheet_df.to_excel(writer, sheet_name=_sheet_safe_name(game_key), index=False)
            except Exception:
                pass
