...
def temp_multiplier(temp_f):
    """
    Temperature-based HR environment multiplier.
    Input can be None/NaN; returns neutral 1.0 in that case.
    """
    try:
...
def wind_multiplier(wind_speed, wind_dir, out_to_cf_deg):
    """
    Convert raw wind speed + direction into an HR multiplier using park orientation.
    - wind_speed: mph
    - wind_dir: meteorological/field direction in degrees
    - out_to_cf_deg: direction from home plate out to center field in degrees
    """
    try:
    ...
    # Convert to in/out/cross component.
    if diff <= 45:
        # blowing out
        return 1.0 + min(0.12, 0.010 * wind_speed)
    if diff >= 135:
        # blowing in
        return 1.0 - min(0.12, 0.010 * wind_speed)
    # crosswind: tiny/no effect
    return 1.0
...
# -------------------------
# Active / inactive roster guard
# -------------------------
_ROSTER_STATUS_MEMO: dict[str, dict[int, dict]] = {}
_INACTIVE_STATUS_PATTERNS = (
    "injured", "injury", " il", "10-day", "15-day", "60-day", "7-day",
    "restricted", "suspended", "bereavement", "paternity", "family medical",
    "administrative", "optioned", "minors", "minor league", "rehab", "not active",
)


def _status_text_from_roster_item(item: dict, roster_type: str, active_ids: set[int]) -> tuple[int | None, dict]:
    if not isinstance(item, dict):
        return None, {}
    person = item.get("person", {}) if isinstance(item.get("person", {}), dict) else {}
    position = item.get("position", {}) if isinstance(item.get("position", {}), dict) else {}
    ...
    try:
        ...
        return None, {}
    pieces = [
        roster_type, str(item.get("status", "")), str(status.get("code", "")),
        str(status.get("description", "")), str(status.get("name", "")),
        str(item.get("rosterStatus", "")), str(item.get("statusCode", "")), str(person.get("status", "")),
    ]
    text = " ".join(p for p in pieces if p and p.lower() not in ("none", "nan")).strip()
    return pid, {
        "player_name": person.get("fullName") or person.get("full_name") or "",
        "position": (position.get("abbreviation") or "").upper(),
        "status_text": text,
        "roster_type_seen": roster_type,
        "active_roster": pid in active_ids,
    }


def get_team_roster_status_map(team_name: str) -> dict[int, dict]:
    """Official MLB roster guard used to keep IL/minors/restricted players off the board."""
    if not team_name:
        return {}
    if team_name in _ROSTER_STATUS_MEMO:
        return _ROSTER_STATUS_MEMO[team_name]
    team_id = get_team_id(team_name)
    if not team_id:
        _ROSTER_STATUS_MEMO[team_name] = {}
        return {}
    status_map: dict[int, dict] = {}
    active_ids: set[int] = set()
    try:
        active = get_json(f"https://statsapi.mlb.com/api/v1/teams/{team_id}/roster?rosterType=active")
        for item in active.get("roster", []) or []:
            pid, rec = _status_text_from_roster_item(item, "active", set())
            if pid is None:
                continue
            active_ids.add(pid)
            rec["active_roster"] = True
            status_map[pid] = {**status_map.get(pid, {}), **rec}
    except Exception as e:
        print(f"[roster_guard] active roster failed for {team_name}: {e}")
    for roster_type in ["injuredList", "40Man", "fullSeason"]:
        try:
            j = get_json(f"https://statsapi.mlb.com/api/v1/teams/{team_id}/roster?rosterType={roster_type}")
            for item in j.get("roster", []) or []:
                pid, rec = _status_text_from_roster_item(item, roster_type, active_ids)
                if pid is None:
                    continue
                old = status_map.get(pid, {})
                merged_text = " ".join([str(old.get("status_text", "")), str(rec.get("status_text", ""))]).strip()
                status_map[pid] = {**old, **rec, "status_text": merged_text, "active_roster": pid in active_ids}
        except Exception:
            continue
    for pid, rec in list(status_map.items()):
        text = (str(rec.get("status_text", "")) + " " + str(rec.get("roster_type_seen", ""))).lower()
        pos = str(rec.get("position", "")).upper()
        inactive_hit = any(pat in f" {text} " for pat in _INACTIVE_STATUS_PATTERNS)
        inactive = inactive_hit or (not bool(rec.get("active_roster", False))) or pos == "P"
        rec["inactive_flag"] = bool(inactive)
        rec["active_check"] = "OUT" if inactive else "ACTIVE"
        if not rec.get("status_text"):
            rec["status_text"] = "active" if not inactive else "not active"
    _ROSTER_STATUS_MEMO[team_name] = status_map
    return status_map


def is_active_board_hitter(player_id: int, roster_map: dict[int, dict]) -> bool:
    try:
        pid = int(player_id)
    except Exception:
        return False
    if not roster_map:
        return True
    rec = roster_map.get(pid)
    if not rec:
        return False
    return not bool(rec.get("inactive_flag", False))


def filter_inactive_players_from_board(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty or "batter_id" not in df.columns:
        return df
    out = df.copy()
    if "roster_active_check" in out.columns:
        mask = out["roster_active_check"].astype(str).str.upper().eq("OUT")
        if mask.any():
            removed = out.loc[mask, "player_name"].astype(str).head(12).tolist() if "player_name" in out.columns else []
            print(f"[roster_guard] removed {int(mask.sum())} inactive/IL rows before export: {removed}")
...
def compute_weather_multiplier(loc: dict | None, home_team: str, game_datetime: str | None, date_str: str) -> tuple[float, dict]:
    ...
    # Controlled positive boosts
    if temp_f is not None and temp_f >= 80:
        final_mult *= 1.05
    elif temp_f is not None and temp_f >= 72:
        final_mult *= 1.02

    if wind_speed is not None and wind_speed >= 8 and wind_dir is not None and out_to_cf_deg is not None:
        diff = abs(((float(wind_dir) - float(out_to_cf_deg) + 180.0) % 360.0) - 180.0)
        outward_component = max(0.0, -float(np.cos(np.deg2rad(diff))))
        if outward_component > 0:
            wind_bonus = 1.0 + min(0.05, 0.01 * float(wind_speed) * outward_component)
            final_mult *= wind_bonus

    final_mult = float(np.clip(final_mult, 0.55, 1.22))
    weather.update({
...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    for g in games:
        ...
        for side in ["away", "home"]:
            ...
            roster_status_map = get_team_roster_status_map(batting_team)
            hitter_ids_raw = get_team_hitters(batting_team)
            hitter_ids = [int(pid) for pid in hitter_ids_raw if is_active_board_hitter(int(pid), roster_status_map)]
            removed_count = len(hitter_ids_raw) - len(hitter_ids)
            if removed_count:
                print(f"[roster_guard] {batting_team}: removed {removed_count} inactive/IL/non-active hitters")
            ...
            for hid in hitter_ids:
                ...
                roster_rec = roster_status_map.get(hid, {}) if isinstance(roster_status_map, dict) else {}
                ...
                rows.append(
                    {
                        ...
                        "batter_id": int(hid),
                        "roster_active_check": roster_rec.get("active_check", "ACTIVE"),
                        "roster_status": roster_rec.get("status_text", "active"),
    ...
    board = filter_inactive_players_from_board(board)
    if board.empty:
        return board
    ...
    environment_clean = (
        0.4 * (((pd.to_numeric(board.get("park_factor",1),errors="coerce").fillna(1)-1)*100)+50) +
        0.3 * (((pd.to_numeric(board.get("weather_mult",1),errors="coerce").fillna(1)-1)*100)+50) +
        0.3 * bullpen_score
    ).clip(0,100)
...
def _build_locked_betting_export(board: pd.DataFrame) -> pd.DataFrame:
    ...
    front_cols = [
        ...
        "decision_score", "roster_active_check", "roster_status", "value_flag", "edge_pct", "probable_pitcher_faced", "game", "venue",
        ...
        "pitcher_hr_score", "environment_clean", "weather_mult", "park_factor", "bullpen_score",
