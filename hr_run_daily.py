...
CALIBRATION_FILE = "data/processed/calibration_training.csv"
...
def calibrate_final_hr_probability(raw_p1: float) -> tuple[float, str]:
    ...
    if cal is not None:
        try:
            return float(np.clip(cal.predict([raw_p1])[0], 1e-6, 0.999)), "isotonic"
        except Exception:
            pass
...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    board = apply_team_cluster_boost(board)
    board = apply_conviction_boost(board)
    ...
    preferred_cols = [
        ...
        "recent_barrel_rate", "recent_hard_hit_rate", "recent_ev", "exp_bbe_mult", "power_spike_mult", "cluster_mult", "conviction_mult", "p_hr_1plus_raw", "calibration_method", "batter_id", "date"
    ]
...
def log_calibration_data(df: pd.DataFrame, date_str: str):
    """
    Append raw model predictions to calibration training file.
    Uses p_hr_1plus_raw when available, otherwise falls back to p_hr_1plus_sim.
    Leaves actual blank for later completion after games finish.
    """
    if df is None or df.empty:
        return

    prob_col = "p_hr_1plus_raw" if "p_hr_1plus_raw" in df.columns else "p_hr_1plus_sim"
    if prob_col not in df.columns or "player_name" not in df.columns:
        return

    log_df = df[["player_name", prob_col]].copy()
    log_df = log_df.rename(columns={prob_col: "predicted"})
    log_df["date"] = date_str
    log_df["actual"] = ""

    log_df = log_df[["date", "player_name", "predicted", "actual"]]
    log_df["predicted"] = pd.to_numeric(log_df["predicted"], errors="coerce")
    log_df = log_df.dropna(subset=["predicted"])

    os.makedirs(os.path.dirname(CALIBRATION_FILE), exist_ok=True)

    if os.path.exists(CALIBRATION_FILE):
        try:
            existing = pd.read_csv(CALIBRATION_FILE)
            required_cols = ["date", "player_name", "predicted", "actual"]
    ...
    # prevent duplicate date/player rows from repeated runs; keep latest
    combined = combined.drop_duplicates(subset=["date", "player_name"], keep="last")
    combined.to_csv(CALIBRATION_FILE, index=False)
