...
def resolve_live_pitchers(game_pk) -> dict:
    """Fallback to live MLB feed so started/confirmed games don't stay TBD."""
    out = {
        "home_name": None, "home_id": None,
        "away_name": None, "away_id": None,
    }
    if not game_pk:
        return out
    try:
        j = get_json(f"https://statsapi.mlb.com/api/v1.1/game/{game_pk}/feed/live", timeout=20)
    except Exception:
        return out
    ...
    for side in ("home", "away"):
        pp = probs.get(side) or {}
        if isinstance(pp, dict):
            out[f"{side}_name"] = pp.get("fullName") or out[f"{side}_name"]
            try:
                out[f"{side}_id"] = int(pp.get("id")) if pp.get("id") is not None else out[f"{side}_id"]
            except Exception:
                pass

    # Fallback to actual game boxscore starters/current pitchers if game is underway.
    box_teams = ((j.get("liveData", {}) or {}).get("boxscore", {}) or {}).get("teams", {}) or {}
    for side in ("home", "away"):
        side_box = box_teams.get(side, {}) or {}
        player_map = side_box.get("players", {}) or {}
        pitcher_ids = side_box.get("pitchers", []) or []

        chosen = None

        # Prefer actual starter from current-game stats if available
        for pid in pitcher_ids:
            pobj = player_map.get(f"ID{pid}", {}) or {}
            stats = (pobj.get("stats", {}) or {}).get("pitching", {}) or {}
            games_started = stats.get("gamesStarted", 0) or 0
            batters_faced = stats.get("battersFaced", 0) or 0
            innings_pitched = stats.get("inningsPitched")
            if games_started or batters_faced or innings_pitched:
                chosen = pobj
                break

        # Fallback to listed first pitcher if nothing else exists
        if chosen is None and pitcher_ids:
            chosen = player_map.get(f"ID{pitcher_ids[0]}", {}) or {}
        ...
        if chosen:
            ...
            try:
                out[f"{side}_id"] = int(pid) if pid is not None else out[f"{side}_id"]
            except Exception:
                pass
...
def compute_env_score_row(row) -> float:
    """Readable matchup environment score that separates players on phone screens."""
    def f(key, default):
        try:
    ...
    pitcher = f("pitcher_hr_mult", 1.0)
    park = f("park_factor", 1.0)
    weather = f("weather_mult", 1.0)
    matchup_today = f("matchup_score_today", 50.0) / 100.0

    # Weighted average, not product, so one weird factor doesn't flatten or explode everything.
    score = (
        0.45 * pitcher +
        0.20 * park +
        0.20 * weather +
...
def env_label_from_score(v: float) -> str:
    if pd.isna(v):
        return "?"
    if v >= 1.03:
        return "🔥"
...
def tier_bucket(rank_value: int) -> str:
    if rank_value <= 15:
        return "T1_1to15"
    if rank_value <= 35:
        return "T2_16to35"
...
def prune_junk_hitters(board: pd.DataFrame) -> pd.DataFrame:
    b = board.copy()
    # Keep actual hitters with real playing time / power paths.
    if "exp_pa" in b.columns:
        b = b[b["exp_pa"].fillna(0) >= 3.0].copy()

    keep_mask = (
        (b.get("power_score", 0).fillna(0) >= 40) |
        (b.get("p_hr_1plus_sim", 0).fillna(0) >= 0.10) |
        (b.get("recent_barrel_rate", 0).fillna(0) >= 0.06) |
        (b.get("recent_hard_hit_rate", 0).fillna(0) >= 0.40)
    )
    b = b[keep_mask].copy()

    # If pitcher is still blank after live fallback, don't trust the row.
    if "probable_pitcher_faced" in b.columns:
        bad_pitcher = b["probable_pitcher_faced"].fillna("").astype(str).str.strip().isin(["", "TBD", "None", "nan"])
        b = b[~bad_pitcher].copy()

    return b


def build_mobile_views(board: pd.DataFrame) -> dict:
    b = board.copy()
    ...
    b["env_score"] = b.apply(compute_env_score_row, axis=1)
    b["env_label"] = b["env_score"].apply(env_label_from_score)
    b["tier_bucket"] = b["rank"].apply(tier_bucket)

    decision_cols = [
        "rank", "tier_bucket", "player_name", "team", "probable_pitcher_faced", "venue",
        "p_hr_1plus_sim", "bet_flag_tier", "bet_flag_reason", "power_tier", "matchup_tier",
        "env_score", "env_label", "pitcher_hr_mult", "park_factor", "weather_mult",
        "power_score", "matchup_score_today", "recent_damage_score"
    ]
    decision_cols = [c for c in decision_cols if c in b.columns]
    decision_board = b[decision_cols].copy()

    for c in ["p_hr_1plus_sim", "env_score", "pitcher_hr_mult", "park_factor", "weather_mult"]:
        if c in decision_board.columns:
            decision_board[c] = pd.to_numeric(decision_board[c], errors="coerce").round(4)

    t1 = decision_board[decision_board["rank"] <= 15].sort_values(
        ["env_score", "p_hr_1plus_sim"], ascending=[False, False]
    ).copy()
    t2 = decision_board[(decision_board["rank"] >= 16) & (decision_board["rank"] <= 35)].sort_values(
        ["env_score", "p_hr_1plus_sim"], ascending=[False, False]
    ).copy()
    t3 = decision_board[(decision_board["rank"] >= 36) & (decision_board["rank"] <= 60)].sort_values(
        ["env_score", "p_hr_1plus_sim"], ascending=[False, False]
    ).copy()

    compact_cols = [
        "rank", "player_name", "team", "probable_pitcher_faced", "bet_flag_tier",
        "p_hr_1plus_sim", "env_label", "env_score", "power_score", "matchup_score_today"
    ]
    compact_cols = [c for c in compact_cols if c in decision_board.columns]
    compact_top35 = decision_board[decision_board["rank"] <= 35][compact_cols].copy()
    compact_top35 = compact_top35.sort_values(["rank"], ascending=[True])
...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    for g in games:
        ...
        # Fallback to live game feed so started / confirmed games don't stay blank.
        if (not home_sp_id or home_sp_name in ("TBD", "", None)) or (not away_sp_id or away_sp_name in ("TBD", "", None)):
            live_pitch = resolve_live_pitchers(game_pk)
            if (not home_sp_id or home_sp_name in ("TBD", "", None)) and (live_pitch.get("home_name") or live_pitch.get("home_id")):
                home_sp_name = live_pitch.get("home_name") or home_sp_name
                home_sp_id = live_pitch.get("home_id") or home_sp_id
            if (not away_sp_id or away_sp_name in ("TBD", "", None)) and (live_pitch.get("away_name") or live_pitch.get("away_id")):
                away_sp_name = live_pitch.get("away_name") or away_sp_name
                away_sp_id = live_pitch.get("away_id") or away_sp_id
...
def write_outputs(board: pd.DataFrame, date_str: str, top_n: int, full_board: pd.DataFrame | None = None):
    ...
    if full_board is not None:
        full_board = force_sequential_ranks(full_board.copy())
        full_board.to_csv("hr_board_full_slate.csv", index=False)
        full_board.to_csv(f"outputs/hr_board_full_slate_{date_str}.csv", index=False)
    ...
    mobile_views = build_mobile_views(board)
    mobile_views["decision_board"].to_csv("hr_board_decision.csv", index=False)
    mobile_views["compact_top35"].to_csv("hr_board_top35_compact.csv", index=False)
    mobile_views["tier1_board"].to_csv("hr_board_tier1.csv", index=False)
    mobile_views["tier2_board"].to_csv("hr_board_tier2.csv", index=False)
    mobile_views["tier3_board"].to_csv("hr_board_tier3.csv", index=False)

    mobile_views["decision_board"].to_csv(f"outputs/hr_board_decision_{date_str}.csv", index=False)
    mobile_views["compact_top35"].to_csv(f"outputs/hr_board_top35_compact_{date_str}.csv", index=False)
    mobile_views["tier1_board"].to_csv(f"outputs/hr_board_tier1_{date_str}.csv", index=False)
    mobile_views["tier2_board"].to_csv(f"outputs/hr_board_tier2_{date_str}.csv", index=False)
    mobile_views["tier3_board"].to_csv(f"outputs/hr_board_tier3_{date_str}.csv", index=False)
...
def main():
    ...
    # --- Full-slate cleanup: remove junk hitters and blank-pitcher rows, but keep a broader audit board ---
    board_full = prune_junk_hitters(board)
    board_full = board_full.sort_values(["daily_matchup_hr_score", "p_hr_1plus_sim"], ascending=False).copy()

    # --- Bettable board: keep real power bats and obvious premium probabilities ---
    PROB_FLOOR = 0.08
    board_bettable = board_full[
        (board_full["power_gate_mult"] >= 0.80) &
        (board_full["p_hr_1plus_sim"] >= PROB_FLOOR)
    ].copy()
    board_bettable = board_bettable.sort_values(["daily_matchup_hr_score", "p_hr_1plus_sim"], ascending=False)

    write_outputs(board_bettable, args.date, top_n=args.top, full_board=board_full)
    print("\nTop 25 (by daily matchup HR score):")
    print(board_bettable.head(25).to_string(index=False))
