...
def safe_df_sort_values(self, by=None, *args, **kwargs):
    """Harden sort_values against upstream column-name drift like game_date/date."""
    if by is not None:
        by_list = [by] if isinstance(by, str) else list(by)
        alias_map = {
            "game_date": ["date", "gameDate", "date_str", "game_datetime", "gameDateTime", "datetime"],
            "date": ["game_date", "gameDate", "date_str", "game_datetime", "gameDateTime", "datetime"],
            "game_datetime": ["datetime", "gameDateTime", "game_date", "date"],
        }
        resolved = []
        for col in by_list:
        ...
        if not resolved:
            return self.copy()
        by = resolved[0] if isinstance(by, str) else resolved
    return _original_df_sort_values(self, by=by, *args, **kwargs)

pd.DataFrame.sort_values = safe_df_sort_values
...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    # Rank the board off HR probability first.
    # Matchup/context still stay on the sheet as supporting signals and tiebreakers,
    # but they should not overrule the actual simulated HR probability.
    ...
    # Export view: keep the same probability-first ordering the board uses.
    export_board = board.sort_values(
        ["rank", "matchup_score_clean", "recent_damage_score"],
...
def main():
    ...
    board = board.sort_values(["p_hr_1plus_sim", "daily_matchup_hr_score"], ascending=False)
