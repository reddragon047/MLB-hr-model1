...
def apply_decision_layer(board: pd.DataFrame) -> pd.DataFrame:
    ...
    missing_pitcher = col_missing("probable_pitcher_faced")

    confidence -= missing_weather.astype(float) * 10.0
    confidence -= missing_pitch.astype(float) * 12.0
    confidence -= missing_game.astype(float) * 8.0
    confidence -= missing_pitcher.astype(float) * 10.0
    ...
    core_score = (
        38.0 * (prob / max(float(prob.max()), 0.001)).clip(0, 1)
        + 20.0 * (matchup / 100.0).clip(0, 1)
        + 17.0 * (power / max(float(power.max()), 1.0)).clip(0, 1)
        + 9.0 * (pitch_fit / 100.0).clip(0, 1)
        + 7.0 * (pitcher_hr / 100.0).clip(0, 1)
        + 5.0 * (env / 100.0).clip(0, 1)
        + 4.0 * (recent / max(float(recent.max()), 1.0)).clip(0, 1)
    )
    core_score *= (confidence / 100.0)
    core_score -= (out["game_exposure_rank"] == 3).astype(float) * 3.0
    core_score -= (out["game_exposure_rank"] >= 4).astype(float) * 8.0
    out["decision_score"] = core_score.round(1)
