...
def ensure_game_date(df: pd.DataFrame) -> pd.DataFrame:
    """
    Force a `game_date` column onto any dataframe that might later sort/group on it.
    """
    if df is None:
        return df
    try:
...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    bat_df = ensure_game_date(bat_df)
    pit_df = ensure_game_date(pit_df)
    mix_df = ensure_game_date(mix_df)
    dmg_df = ensure_game_date(dmg_df)
    park_df = ensure_game_date(park_df)
    bullpen_df = ensure_game_date(bullpen_df)
    platoon_df = ensure_game_date(platoon_df)
