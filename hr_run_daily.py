...
def write_game_sheets(df: pd.DataFrame, date_str: str = "today", top_n: int = 75) -> None:
    """Write one CSV per game directly into outputs/ so artifact upload catches them."""
    ...
    ascending = [True if c == "prob_rank" else False for c in sort_cols]
    top_n = int(top_n or 75)
    ...
    for game_key, game_df in out.groupby("game_key", dropna=False):
        ...
        full_path = f"outputs/hr_game_{safe_name}_{date_str}.csv"
        top_path = f"outputs/hr_game_{safe_name}_{date_str}_top{top_n}.csv"

        game_df.to_csv(full_path, index=False)
        game_df.head(top_n).to_csv(top_path, index=False)
...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    _ml = _relative_bucket(board["matchup_score_clean"], high_q=0.75, med_q=0.40, labels=("HIGH","MED","LOW"))
    ...
    board["matchup_label"] = _relative_bucket(board["matchup_score_clean"], high_q=0.75, med_q=0.40, labels=("HIGH","MED","LOW"))
