...
def parse_args():
    parser = argparse.ArgumentParser(description="Build a sharp daily HR board with Monte Carlo.")
...
def build_hitter_pool(sc: pd.DataFrame, slate_teams: set, slate_hitter_ids: set) -> pd.DataFrame:
    ...
    hitters["raw_hr_pa"] = np.where(hitters["pa_count"] > 0, hitters["hr_count"] / hitters["pa_count"], 0.0)
    hitters["bbe_per_pa"] = np.where(hitters["pa_count"] > 0, hitters["bbe_count"] / hitters["pa_count"], 0.0)
    ...
    league_hr_pa = float(hitters["raw_hr_pa"].replace([np.inf, -np.inf], np.nan).fillna(0).mean())
    hitters["skill_hr_pa"] = (hitters["hr_count"] + 50.0 * league_hr_pa) / (hitters["pa_count"] + 50.0)
    hitters["skill_hr_pa"] = hitters["skill_hr_pa"].clip(lower=0.005, upper=0.09)
    return hitters
...
def expected_pa_from_rank(team_bbe_rank: float) -> float:
    r = float(team_bbe_rank)
    if r <= 2:
        return 4.65
    if r <= 4:
        return 4.45
...
def run_monte_carlo(board: pd.DataFrame, sims: int, seed: int = MC_SEED) -> pd.DataFrame:
    rng = np.random.default_rng(seed)
    probs = []
    fair_odds = []
    for _, row in board.iterrows():
        p_pa = float(row["p_hr_pa"])
        exp_pa = float(row["exp_pa"])
        pa_sims = rng.poisson(lam=max(exp_pa, 0.1), size=sims)
        pa_sims = np.clip(pa_sims, 2, 6)
        u = rng.random((sims, 6))
        hr_any = (u < p_pa)
        mask = np.arange(6)[None, :] < pa_sims[:, None]
        hit = (hr_any & mask).any(axis=1)
        p_1plus = float(hit.mean())
        probs.append(p_1plus)
        fair = (100.0 / p_1plus - 100.0) if p_1plus > 0 else np.nan
        fair_odds.append(round(fair, 1) if pd.notna(fair) else np.nan)
    board["p_hr_1plus_mc"] = probs
    board["fair_odds_hr"] = fair_odds
    return board


def apply_percentile_grades(board: pd.DataFrame) -> pd.DataFrame:
    pct = board["bet_grade_score"].rank(pct=True, method="max", ascending=False)
    def label(p):
        if p <= 0.05:
            return "A+"
        if p <= 0.15:
        ...
        if p <= 0.65:
            return "C"
        return "D"
    board["bet_grade"] = pct.apply(label)
    return board
...
def build_daily_board(hitters: pd.DataFrame, pitchers: pd.DataFrame, bullpens: pd.DataFrame, schedule_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, game in schedule_df.iterrows():
        ...
        for _, h in away_hitters.iterrows():
            ...
            final_mult = away_pitcher_mult * park_ctx["park_mult"] * park_ctx["weather_mult"] * away_bullpen_mult
            p_pa = float(h["skill_hr_pa"] * final_mult)
            p_pa = clamp(p_pa, 0.005, 0.12)
            rows.append({
                ...
                "final_score": float(h["base_score"] * final_mult),
                "skill_hr_pa": round(float(h["skill_hr_pa"]), 4),
                "p_hr_pa": round(float(p_pa), 4),
                "exp_pa": round(expected_pa_from_rank(float(h["team_bbe_rank"])), 2),
        ...
        for _, h in home_hitters.iterrows():
            ...
            final_mult = home_pitcher_mult * park_ctx["park_mult"] * park_ctx["weather_mult"] * home_bullpen_mult
            p_pa = float(h["skill_hr_pa"] * final_mult)
            p_pa = clamp(p_pa, 0.005, 0.12)
            rows.append({
                "player_name": h["player_name"],
    ...
    board = run_monte_carlo(board, sims=MC_SIMS)
    board = apply_percentile_grades(board)
    board = board.sort_values(["p_hr_1plus_mc", "bet_grade_score", "final_score", "bbe_count", "pa_count"], ascending=[False, False, False, False, False]).reset_index(drop=True)
