...
def ensure_dirs():
    ...
    os.makedirs("outputs/game_sheets", exist_ok=True)
...
def sharpened_pitchtype_multiplier(pt_mult: float,
                                   dominant_pitch_usage: float = 0.0,
                                   batter_vs_pitch_ratio: float = 1.0) -> float:
    """Safely sharpen pitch-type fit without allowing helper failures to kill the run."""
    try:
        pt = float(pt_mult)
    except Exception:
        return 1.0
    ...
    # Keep the original multiplier as the anchor and only allow modest sharpening.
    usage_boost = np.clip(usage / 0.35, 0.0, 1.0)
    ratio_boost = np.clip((ratio - 1.0) / 0.5, -1.0, 1.0)
    sharpen = 1.0 + 0.08 * usage_boost * max(0.0, ratio_boost)
    out = pt * sharpen
    return float(np.clip(out, 0.85, 1.20))




def exp_bbe_multiplier(k_rate: float, exp_pa: float) -> float:
    """Small bounded PA/contact adjustment used as a light sharpening layer."""
    try:
        k = float(k_rate or 0.0)
    except Exception:
        k = 0.0
    try:
        pa = float(exp_pa or 0.0)
    except Exception:
        pa = 0.0

    # More expected PA helps; high strikeout rates suppress a bit.
    pa_term = np.clip((pa - 4.1) / 1.2, -1.0, 1.0)
    k_term = np.clip((0.24 - k) / 0.12, -1.0, 1.0)
    mult = 1.0 + 0.03 * pa_term + 0.03 * k_term
    return float(np.clip(mult, 0.94, 1.06))


def recent_power_spike_multiplier(
    recent_barrel_rate: float,
    recent_hard_hit_rate: float,
    recent_ev: float,
) -> float:
    """Small recent-form spike multiplier; intentionally modest and stable."""
    try:
        rb = float(recent_barrel_rate or 0.0)
    except Exception:
        rb = 0.0
    try:
    ...
    signal = (
        0.55 * np.clip((rb - 0.08) / 0.10, -1.0, 1.0)
        + 0.30 * np.clip((rhh - 0.40) / 0.20, -1.0, 1.0)
        + 0.15 * np.clip((rev - 90.0) / 4.0, -1.0, 1.0)
    )
    mult = 1.0 + 0.04 * signal
    return float(np.clip(mult, 0.96, 1.06))


def temp_multiplier(temp_f: float | None) -> float:
    """Weather temp effect kept intentionally conservative."""
    ...
    try:
        ...
        return 1.0
    # Cold suppresses, warmth helps, but stay modest.
    delta = np.clip((temp - 72.0) / 18.0, -1.0, 1.0)
    return float(np.clip(1.0 + 0.035 * delta, 0.94, 1.05))


def wind_multiplier(wind_speed: float | None, wind_dir: str | None, out_to_cf_deg: float | None = None) -> float:
    """Simple direction-aware wind effect with conservative bounds."""
    try:
        ws = float(wind_speed or 0.0)
    except Exception:
        ws = 0.0
    wd = str(wind_dir or '').lower()
    if ws <= 0:
        return 1.0

    strength = np.clip(ws / 12.0, 0.0, 1.0)
    if 'out' in wd or 'blow out' in wd:
        mult = 1.0 + 0.05 * strength
    elif 'in' in wd or 'blow in' in wd:
        mult = 1.0 - 0.06 * strength
    elif 'cross' in wd or 'left to right' in wd or 'right to left' in wd:
        mult = 1.0 + 0.01 * strength
    else:
        mult = 1.0
    return float(np.clip(mult, 0.92, 1.06))
...
def _resolve_helper(fn_name: str, fallback):
    fn = globals().get(fn_name)
    return fn if callable(fn) else fallback


def _safe_sharpened_pitchtype_multiplier(pt_mult, dominant_pitch_usage, batter_vs_pitch_ratio):
    fallback = lambda pt_mult, dominant_pitch_usage, batter_vs_pitch_ratio: float(np.clip(pt_mult, 0.85, 1.20))
    fn = _resolve_helper("sharpened_pitchtype_multiplier", fallback)
    try:
        return float(fn(
            pt_mult=pt_mult,
        ...
        return float(fallback(pt_mult, dominant_pitch_usage, batter_vs_pitch_ratio))


def _safe_exp_bbe_multiplier(k_rate, exp_pa):
    fallback = lambda k_rate, exp_pa: 1.0
    fn = _resolve_helper("exp_bbe_multiplier", fallback)
    try:
        return float(fn(k_rate=k_rate, exp_pa=exp_pa))
    except Exception:
        return 1.0


def _safe_recent_power_spike_multiplier(recent_barrel_rate, recent_hard_hit_rate, recent_ev):
    fallback = lambda recent_barrel_rate, recent_hard_hit_rate, recent_ev: 1.0
    fn = _resolve_helper("recent_power_spike_multiplier", fallback)
    try:
        return float(fn(
...
def _first_non_null_value(df: pd.DataFrame, candidates, default=""):
...
def _used_column_name(df: pd.DataFrame, candidates):
    for col in candidates:
...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    for g in games:
        ...
        for side in ["away", "home"]:
            ...
            for hid in hitter_ids:
                ...
                pt_mult = _safe_sharpened_pitchtype_multiplier(
                    pt_mult=pt_mult,
                    dominant_pitch_usage=dominant_pitch_usage,
                    batter_vs_pitch_ratio=batter_vs_pitch_ratio,
                )
                ...
                # ---- Extra sharpening ----
                exp_bbe_mult = _safe_exp_bbe_multiplier(
                    k_rate=float(feat.iloc[0].get("k_rate", 0.0)),
                    exp_pa=exp_pa,
                )
                power_spike_mult = _safe_recent_power_spike_multiplier(
                    recent_barrel_rate=recent_barrel_rate,
                    recent_hard_hit_rate=recent_hard_hit_rate,
                    recent_ev=recent_ev,
                )
    ...
    cols = [
        ...
        "matchup_score_clean","matchup_label","p_hr_1plus_sim",
        "recent_damage_score","pitch_mix_fit_score","pitcher_hr_score","bullpen_score"
    ]
    cols = [c for c in cols if c in board.columns]

    date_str = str(board["date"].iloc[0]) if "date" in board.columns else "today"

    board[cols].to_csv(f"outputs/hr_board_full_slate_{date_str}.csv", index=False)

    top_n = int(globals().get("DEFAULT_TOPN", 75) or 75)
    board.head(top_n)[cols].to_csv(f"outputs/hr_board_top75_{date_str}.csv", index=False)
...
def _safe_sheet_slug(s: str) -> str:
    s = str(s or "game")
    s = re.sub(r"[^A-Za-z0-9]+", "_", s).strip("_")
    return s[:120] if s else "game"


def _first_valid_value(frame: pd.DataFrame, candidates, default=np.nan):
    for c in candidates:
...
def _describe_wind(direction, speed):
    d = str(direction or "").strip()
    s = _safe_float(speed, default=np.nan)
    if d and pd.notna(s):
        return f"{round(s,1)} mph {d}"
    if d:
        return d
    if pd.notna(s):
        return f"{round(s,1)} mph"
    return "NA"

def _summarize_pitcher_side(game_df: pd.DataFrame, pitcher_name: str) -> str:
    if not pitcher_name:
        return "No probable pitcher found"
    if "probable_pitcher_faced" in game_df.columns:
        pdf = game_df[game_df["probable_pitcher_faced"].astype(str) == str(pitcher_name)].copy()
    else:
        pdf = game_df.copy()
    if pdf.empty:
        pdf = game_df.copy()

    hr_score = _safe_float(pdf["pitcher_hr_score"].median(), default=np.nan) if "pitcher_hr_score" in pdf.columns else np.nan
    pitch = ""
    if "dominant_pitch" in pdf.columns and pdf["dominant_pitch"].dropna().any():
        pitch = str(pdf["dominant_pitch"].dropna().mode().iloc[0])

    tags = []
    if pd.notna(hr_score):
        if hr_score >= 8:
            tags.append("HR prone")
        elif hr_score <= 4:
            tags.append("suppresses HR")
        else:
            tags.append("neutral HR profile")
    if pitch:
        tags.append(f"{pitch}-leaning")
    if "pitch_mix_fit_score" in pdf.columns:
        fit = _safe_float(pdf["pitch_mix_fit_score"].median(), default=np.nan)
        if pd.notna(fit):
            if fit >= 65:
                tags.append("good pitch-fit for opposing bats")
            elif fit <= 40:
                tags.append("weak pitch-fit for opposing bats")
    return ", ".join(tags) if tags else "Limited pitcher context"

def _build_target_reason(row: pd.Series) -> str:
    bits = []
    prob = _safe_float(row.get("p_hr_pct"), default=np.nan)
    fit = _safe_float(row.get("pitch_mix_fit_score"), default=np.nan)
    hr_score = _safe_float(row.get("pitcher_hr_score"), default=np.nan)
    dmg = _safe_float(row.get("recent_damage_score"), default=np.nan)
    label = str(row.get("matchup_label", "") or "")

    if pd.notna(prob):
        if prob >= 16:
            bits.append("elite raw HR probability")
        elif prob >= 13:
            bits.append("strong raw HR probability")
        elif prob >= 10:
            bits.append("live secondary HR probability")

    if label:
        bits.append(f"matchup tag {label}")

    if pd.notna(fit):
        if fit >= 65:
            bits.append("strong pitch-fit")
        elif fit >= 50:
            bits.append("solid pitch-fit")

    if pd.notna(hr_score):
        if hr_score >= 8:
            bits.append("vulnerable opposing pitcher")
        elif hr_score <= 4:
            bits.append("tough opposing pitcher")

    if pd.notna(dmg):
        if dmg >= 60:
            bits.append("hot recent damage")
        elif dmg >= 40:
            bits.append("decent recent form")

    if not bits:
        return "best blend of probability and matchup in this game"
    return "; ".join(bits[:3])

def export_individual_game_sheets(board: pd.DataFrame, date_str: str) -> int:
    """Write one report-style CSV per game plus an index. Never raises."""
    try:
        if board is None or board.empty:
            print("[game_sheets] created 0 per-game CSV sheets.")
            return 0

        df = board.copy()
        if "venue" not in df.columns and "game_key" not in df.columns:
            print("[game_sheets] created 0 per-game CSV sheets (missing venue/game key).")
            return 0

        if "p_hr_1plus_sim" in df.columns:
            df["p_hr_pct"] = pd.to_numeric(df["p_hr_1plus_sim"], errors="coerce") * 100.0
        elif "p_hr_pct" not in df.columns:
            df["p_hr_pct"] = np.nan

        if "matchup_score_clean" not in df.columns:
            df["matchup_score_clean"] = np.nan

        out_dir = "outputs"
        nested_dir = os.path.join(out_dir, "game_sheets")
        os.makedirs(out_dir, exist_ok=True)
        os.makedirs(nested_dir, exist_ok=True)

        exported = []
        group_cols = ["game_key"] if "game_key" in df.columns else ["venue"]

        temp_candidates = ["weather_temp_f", "temp", "temperature", "temp_f", "weather_temp", "game_temp"]
        wind_speed_candidates = ["weather_wind_mph", "wind_speed", "wind_mph", "weather_wind_speed", "game_wind_speed"]
        wind_dir_candidates = ["wind_impact_label", "wind_dir", "wind_direction", "weather_wind_dir", "weather_wind_dir_deg", "game_wind_dir"]
        weather_mult_candidates = ["weather_mult", "weather_multiplier"]
        park_candidates = ["park_factor", "hr_park_factor", "park_hr_factor"]
        ...
        for gkey, g in df.groupby(group_cols, dropna=False):
            ...
            if "venue" in g.columns and g["venue"].notna().any():
                venue = str(g["venue"].dropna().iloc[0])
            else:
                venue = str(gkey)

            teams = [str(x) for x in g.get("team", pd.Series(dtype=str)).dropna().astype(str).unique().tolist()]
            teams_sorted = sorted(teams)
            matchup_display = " vs ".join(teams_sorted) if teams_sorted else str(venue)

            temp_val, temp_col = _first_valid_value(g, temp_candidates, default=np.nan)
            wind_speed_val, wind_speed_col = _first_valid_value(g, wind_speed_candidates, default=np.nan)
            wind_dir_val, wind_dir_col = _first_valid_value(g, wind_dir_candidates, default="")
            weather_mult_val, weather_mult_col = _first_valid_value(g, weather_mult_candidates, default=np.nan)
            park_val, park_col = _first_valid_value(g, park_candidates, default=np.nan)

            temp_num = _safe_float(temp_val, default=np.nan)
            wind_speed_num = _safe_float(wind_speed_val, default=np.nan)
            wind_desc = _describe_wind(wind_dir_val, wind_speed_val)

            pitcher_names = []
            if "probable_pitcher_faced" in g.columns:
                pitcher_names = [str(x) for x in g["probable_pitcher_faced"].dropna().astype(str).unique().tolist()]
            pitcher_names = pitcher_names[:2]

            game_score = 1.00
            if pd.notna(temp_num):
                if temp_num >= 82:
                    game_score += 0.08
                elif temp_num >= 72:
                    game_score += 0.04
                    ...
                    game_score -= 0.08
            if pd.notna(wind_speed_num):
                if "out" in str(wind_dir_val).lower():
                    game_score += 0.10 + (0.05 if wind_speed_num >= 10 else 0.0)
                elif "in" in str(wind_dir_val).lower():
                    game_score -= 0.10 + (0.05 if wind_speed_num >= 10 else 0.0)
                elif wind_speed_num >= 12:
                    game_score += 0.02

            pitcher_median = _safe_float(g["pitcher_hr_score"].median(), default=np.nan) if "pitcher_hr_score" in g.columns else np.nan
            if pd.notna(pitcher_median):
                if pitcher_median >= 8:
                    game_score += 0.08
                elif pitcher_median <= 4:
                    game_score -= 0.06

            weather_mult_num = _safe_float(weather_mult_val, default=np.nan)
            if pd.notna(weather_mult_num):
                if weather_mult_num >= 1.08:
                    game_score += 0.04
                elif weather_mult_num <= 0.94:
                    game_score -= 0.04

            game_score = round(game_score, 2)
            if game_score >= 1.12:
                game_tag = "ATTACK"
                decision = "Play 2 hitters"
            elif game_score <= 0.90:
                game_tag = "FADE"
                decision = "Avoid game"
            else:
                game_tag = "NEUTRAL"
            ...
            sort_cols = [c for c in ["p_hr_1plus_sim", "matchup_score_clean", "rank"] if c in g.columns]
            ...
            report_rows = []
            def add_row(section, key, value, **extra):
                row = {"section": section, "key": key, "value": value}
                row.update(extra)
                report_rows.append(row)

            add_row("GAME_INFO", "matchup", matchup_display)
            add_row("GAME_INFO", "venue", venue)
            add_row("GAME_INFO", "date", date_str)

            temp_out = temp_val if pd.notna(pd.to_numeric(temp_val, errors="coerce")) else (temp_val or "NA")
            add_row("ENVIRONMENT", "temp_f", temp_out)
            add_row("ENVIRONMENT", "wind", wind_desc)
            add_row("ENVIRONMENT", "park_factor", park_val if park_val != "" else "NA")
            add_row("ENVIRONMENT", "weather_multiplier", weather_mult_val if weather_mult_val != "" else "NA")
            add_row("ENVIRONMENT", "game_score", game_score)
            add_row("ENVIRONMENT", "game_tag", game_tag)

            env_summary = []
            if pd.notna(temp_num):
                if temp_num >= 80:
                    env_summary.append("warm hitting weather")
                elif temp_num <= 55:
                    env_summary.append("cold suppression")
            if "out" in str(wind_dir_val).lower():
                env_summary.append("wind helping carry")
            elif "in" in str(wind_dir_val).lower():
                env_summary.append("wind knocking balls down")
            park_num = _safe_float(park_val, default=np.nan)
            if pd.notna(park_num) and park_num >= 1.05:
                env_summary.append("park boost present")
            add_row("ENVIRONMENT", "summary", "; ".join(env_summary) if env_summary else "fairly neutral environment")

            if pitcher_names:
                for i, pname in enumerate(pitcher_names, start=1):
                    add_row("PITCHING", f"pitcher_{i}", pname)
                    add_row("PITCHING", f"pitcher_{i}_summary", _summarize_pitcher_side(g, pname))
            else:
                add_row("PITCHING", "summary", "No probable pitcher context available")

            for idx, (_, r) in enumerate(top3.iterrows(), start=1):
                prob_val = _safe_float(r.get("p_hr_pct"), default=np.nan)
                add_row(
                    "TOP_TARGET",
                    str(idx),
                    f"{r.get('player_name','')} | {round(prob_val,2) if pd.notna(prob_val) else 'NA'}% | rank {r.get('rank','')} | prob_rank {r.get('prob_rank','')}",
                    target_rank=idx,
                    player_name=r.get("player_name",""),
                    team=r.get("team",""),
                    hr_prob_pct=round(prob_val,2) if pd.notna(prob_val) else np.nan,
                    rank=r.get("rank", np.nan),
                    prob_rank=r.get("prob_rank", np.nan),
                    matchup_label=r.get("matchup_label",""),
                    matchup_score_clean=r.get("matchup_score_clean", np.nan),
                    probable_pitcher_faced=r.get("probable_pitcher_faced",""),
                    reason=_build_target_reason(r),
                )

            team_best = []
            if "team" in g.columns and "player_name" in g.columns:
                for team_name, tg in g.groupby("team", dropna=False):
                    sort_cols_t = [c for c in ["p_hr_1plus_sim","rank"] if c in tg.columns]
                    if sort_cols_t:
                        tg = tg.sort_values(sort_cols_t, ascending=[False, True][:len(sort_cols_t)])
                    if not tg.empty:
                        best = tg.iloc[0]
                        best_prob = _safe_float(best.get('p_hr_pct'), default=np.nan)
                        team_best.append(f"{team_name}: {best.get('player_name','')} ({round(best_prob,1) if pd.notna(best_prob) else 'NA'}%)")
            add_row("RECOMMENDATION", "play_type", decision)
            add_row("RECOMMENDATION", "best_per_team", " | ".join(team_best) if team_best else "NA")
            add_row("RECOMMENDATION", "notes", f"{game_tag} game. {decision}. Focus on top target(s) first.")

            add_row("EXPORT_DEBUG", "temp_col_used", temp_col or "NONE")
            add_row("EXPORT_DEBUG", "wind_speed_col_used", wind_speed_col or "NONE")
            add_row("EXPORT_DEBUG", "wind_dir_col_used", wind_dir_col or "NONE")
            add_row("EXPORT_DEBUG", "weather_mult_col_used", weather_mult_col or "NONE")
            add_row("EXPORT_DEBUG", "park_factor_col_used", park_col or "NONE")

            lineup_cols = [
                "player_name","team","probable_pitcher_faced","rank","prob_rank","p_hr_pct",
                "matchup_label","matchup_score_clean","pitcher_hr_score","pitch_mix_fit_score",
                "bullpen_score","recent_damage_score","dominant_pitch","dominant_pitch_usage",
                "batter_vs_pitch_ratio","bet_flag_reason","venue"
            ]
            for c in lineup_cols:
                if c not in g.columns:
                    g[c] = np.nan
            lineup = g[lineup_cols].copy()
            lineup.insert(0, "section", "ALL_HITTERS")
            lineup.insert(1, "key", lineup["player_name"])
            lineup.insert(2, "value", lineup.apply(lambda r: f"{r.get('team','')} | {round(_safe_float(r.get('p_hr_pct'), default=np.nan),2) if pd.notna(_safe_float(r.get('p_hr_pct'), default=np.nan)) else 'NA'}% | rank {r.get('rank','')} | {r.get('matchup_label','')}", axis=1))
            lineup["weather_temp_f"] = temp_out
            lineup["weather_wind"] = wind_desc
            lineup["game_score"] = game_score
            lineup["game_tag"] = game_tag
            lineup.rename(columns={"p_hr_pct": "hr_prob_pct"}, inplace=True)

            out_df = pd.concat([pd.DataFrame(report_rows), lineup], ignore_index=True, sort=False)

            matchup_slug = _safe_sheet_slug(f"{venue}_{matchup_display}_{date_str}")
            flat_path = os.path.join(out_dir, f"hr_game_sheet_{matchup_slug}.csv")
            nested_path = os.path.join(nested_dir, f"hr_game_sheet_{matchup_slug}.csv")
            out_df.to_csv(flat_path, index=False)
            out_df.to_csv(nested_path, index=False)

            exported.append({
                "game_sheet_file": os.path.basename(flat_path),
                "venue": venue,
                "teams": " | ".join(teams_sorted),
                "n_hitters": int(len(g)),
                "game_score": game_score,
                "game_tag": game_tag,
                "decision": decision,
                "top_target_1": top3.iloc[0]["player_name"] if len(top3) >= 1 else "",
                "top_target_2": top3.iloc[1]["player_name"] if len(top3) >= 2 else "",
                "top_target_3": top3.iloc[2]["player_name"] if len(top3) >= 3 else "",
            })

        if exported:
            pd.DataFrame(exported).to_csv(os.path.join(out_dir, f"hr_game_sheet_index_{date_str}.csv"), index=False)
        print(f"[game_sheets] created {len(exported)} per-game CSV sheets.")
        return len(exported)
    except Exception as e:
        print(f"[game_sheets] warning: {e}")
        return 0
...
def write_outputs(board: pd.DataFrame, date_str: str, top_n: int):
    ...
    full_path = f"outputs/hr_board_full_slate_{date_str}.csv"
    top_path = f"outputs/hr_board_top75_{date_str}.csv"

    board.to_csv(csv_path, index=False)
    board.to_csv(full_path, index=False)
    board.head(top_n).to_csv(top_path, index=False)
    ...
    export_individual_game_sheets(board, date_str)
