...
MODEL_VERSION = "v2_recent_form_daily_reactivity"
...
def compute_daily_reactivity_multiplier(
    pt_mult: float,
    ...
    """
    Increase day-to-day movement by giving more weight to matchup/environmental
    changes while keeping outputs bounded. Returns (multiplier, score).
    """
    # amplify true daily components around 1.0
    pitch_component = float(np.clip(1.0 + (pt_mult - 1.0) * 1.35, 0.85, 1.28))
    form_component = float(np.clip(1.0 + (recent_form_mult - 1.0) * 1.20, 0.90, 1.18))
    pitcher_component = float(np.clip(1.0 + (pitcher_hr_mult - 1.0) * 1.35, 0.86, 1.24))
    env_component = float(np.clip(1.0 + (env_mult - 1.0) * 1.20, 0.82, 1.28))
    bp_component = float(np.clip(1.0 + (bp_mult - 1.0) * 1.10, 0.90, 1.16))
    platoon_component = float(np.clip(1.0 + (platoon_mult - 1.0) * 1.15, 0.88, 1.18))

    mult = pitch_component * form_component * pitcher_component * env_component * bp_component * platoon_component
    mult = float(np.clip(mult, 0.78, 1.42))

    # diagnostic score centered at 100 = neutral
    score = 100.0 * mult
    return mult, float(np.clip(score, 78.0, 142.0))
    ...
    for g in games:
        ...
        for side in ["away", "home"]:
            ...
            for hid in hitter_ids:
                ...
                matchup_mult = float(np.clip(pt_mult * recent_form_mult * pitcher_hr_mult, 0.88, 1.26))
                daily_reactivity_mult, daily_reactivity_score = compute_daily_reactivity_multiplier(
                    pt_mult=pt_mult,
                ...
                p_pa_adj = float(np.clip(p_pa * matchup_mult * daily_reactivity_mult, 1e-6, 0.30))
