...
def ensure_date_aliases(df: pd.DataFrame | None, fallback_date: str | None = None) -> pd.DataFrame:
    """
    ...
    If no date-like column exists, uses `fallback_date` to create both `date`
    and `game_date` so downstream sort/merge code cannot die on KeyError.
    """
    if df is None:
        return pd.DataFrame()
    ...
    if "date" not in out.columns and fallback_date is not None:
        out["date"] = fallback_date
    if "game_date" not in out.columns and fallback_date is not None:
        out["game_date"] = fallback_date
...
def main():
    ...
    board = ensure_date_aliases(board, args.date)
