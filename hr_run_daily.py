        ...
        for side in ["away", "home"]:
            ...
            for hid in hitter_ids:
                ...
                daily_matchup_hr_score, matchup_tier = compute_hybrid_matchup_score(
                    matchup_mult=float(matchup_mult or 1.0),
                    pitchtype_mult=float(pt_mult or 1.0),
                    recent_form_mult=float(recent_form_mult or 1.0),
                    pitcher_hr_mult=float(pitcher_hr_mult or 1.0),
                    park_factor=float(park_contact_mult if "park_contact_mult" in locals() else 1.0),
                    weather_mult=float(w_mult if "w_mult" in locals() else 1.0),
                    exp_pa=float(exp_pa or 4.0),
                    batter_vs_pitch_ratio=float(batter_vs_pitch_ratio or 1.0),
                    dominant_pitch_usage=float(dominant_pitch_usage or 0.30),
                )
                rank_score = compute_rank_score(
                    p1=float(p1),
                    daily_matchup_hr_score=float(daily_matchup_hr_score),
                )
