...
def _sanitize_sheetish_name(value, max_len=80):
...
def _ensure_game_key(df: pd.DataFrame) -> pd.DataFrame:
    ...
    team = out["team"].astype(str) if "team" in out.columns else pd.Series(["UnknownTeam"] * len(out), index=out.index)
    venue = out["venue"].astype(str) if "venue" in out.columns else pd.Series(["UnknownVenue"] * len(out), index=out.index)
    pitcher = out["probable_pitcher_faced"].astype(str) if "probable_pitcher_faced" in out.columns else pd.Series(["UnknownPitcher"] * len(out), index=out.index)

    out["game_key"] = venue.fillna("UnknownVenue") + " | " + pitcher.fillna("UnknownPitcher")
    if "game" in out.columns:
        game_col = out["game"].astype(str).fillna("")
        out.loc[game_col.str.strip() != "", "game_key"] = game_col[game_col.str.strip() != ""]
    return out


def write_game_sheets(df: pd.DataFrame, date_str: str = "today", top_n: int = 75) -> None:
    """Write one CSV per game plus a compact top-N per-game folder.

    Avoids optional Excel dependencies so GitHub Actions won't fail when
    openpyxl/xlsxwriter are unavailable.
    """
    if df is None or df.empty:
        return

    out = _ensure_game_key(df)
    os.makedirs("outputs/game_sheets", exist_ok=True)
    os.makedirs("outputs/game_sheets_topn", exist_ok=True)

    sort_cols = [c for c in ["p_hr_1plus_sim", "matchup_score_clean", "recent_damage_score", "prob_rank"] if c in out.columns]
    ascending = []
    for c in sort_cols:
        ascending.append(True if c == "prob_rank" else False)
    ...
    for game_key, game_df in out.groupby("game_key", dropna=False):
        ...
        game_df["game_rank"] = np.arange(1, len(game_df) + 1)
        safe_name = _sanitize_sheetish_name(game_key)
        game_df.to_csv(f"outputs/game_sheets/{safe_name}_{date_str}.csv", index=False)
        game_df.head(int(top_n or 75)).to_csv(f"outputs/game_sheets_topn/{safe_name}_{date_str}_top{int(top_n or 75)}.csv", index=False)
...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
    ...
    # Export view: probability-first, with matchup/recent damage only as tiebreakers.
    ...
    board = _coalesce_date_column(board, target="date")
    date_str = str(board["date"].iloc[0]) if "date" in board.columns and len(board) else "today"
    ...
    # Per-game sheets for cleaner same-game filtering / analysis.
    game_sheet_cols = cols.copy()
    for extra_col in ["date", "game", "game_key"]:
        if extra_col in export_board.columns and extra_col not in game_sheet_cols:
            game_sheet_cols.append(extra_col)
    write_game_sheets(export_board[game_sheet_cols].copy(), date_str=date_str, top_n=top_n)
