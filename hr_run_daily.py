...
MODEL_VERSION = "v2_recent_form_hybrid_clean"
...
def compute_hybrid_matchup_score(
    ...
    """
    Lightweight daily matchup score built from existing v2 fields.
    No dependency on game_date or any extra cached tables.
    """
    def score(v, lo, hi):
        try:
            x = float(v)
    ...
    pitch_mix_fit = (
        0.45 * score(pitchtype_mult, 0.94, 1.18) +
        0.35 * score(batter_vs_pitch_ratio, 0.92, 1.22) +
        0.20 * score(dominant_pitch_usage, 0.22, 0.60)
    )
    pitcher_vuln = score(pitcher_hr_mult, 0.94, 1.10)
    environment = (
        0.35 * score(matchup_mult, 0.94, 1.18) +
        0.20 * score(park_factor, 0.92, 1.12) +
        0.25 * score(weather_mult, 0.88, 1.12) +
        0.20 * score(exp_pa, 3.6, 5.0)
    )
    recent = score(recent_form_mult, 0.96, 1.08)

    s = 0.34 * pitch_mix_fit + 0.26 * pitcher_vuln + 0.24 * environment + 0.16 * recent
...
def compute_rank_score(p1: float, daily_matchup_hr_score: float) -> float:
    import numpy as np
    return float(np.clip(0.60 * (float(daily_matchup_hr_score) / 100.0) + 0.40 * float(p1), 1e-6, 0.999))


def fails_viability_filter(row) -> bool:
    """
    Remove obvious non-bettable fringe names.
    """
    import pandas as pd
    try:
        exp_pa = float(row.get("exp_pa", 0) or 0)
    except Exception:
        exp_pa = 0.0
    try:
        p_hr_pa = float(row.get("p_hr_pa", 0) or 0)
    except Exception:
        p_hr_pa = 0.0

    odds = row.get("odds_open_1plus", None)
    no_odds = pd.isna(odds) or odds in ("", "NA", "N/A", None)

    if no_odds and exp_pa < 3.6:
        return True
    if exp_pa < 3.6 and p_hr_pa < 0.025:
        return True
    return False
...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
        ...
        for side in ["away", "home"]:
            ...
            for hid in hitter_ids:
                ...
                daily_matchup_hr_score, matchup_tier = compute_hybrid_matchup_score(
                    ...
                    park_factor=park_contact_mult if "park_contact_mult" in locals() else 1.0,
                    weather_mult=w_mult if "w_mult" in locals() else 1.0,
                    exp_pa=exp_pa,
                    batter_vs_pitch_ratio=batter_vs_pitch_ratio,
                    dominant_pitch_usage=dominant_pitch_usage,
                )
                rank_score = compute_rank_score(p1=p1, daily_matchup_hr_score=daily_matchup_hr_score)
    ...
    if not board.empty:
        board = board.loc[~board.apply(fails_viability_filter, axis=1)].copy()
