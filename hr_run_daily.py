...
def ensure_date_aliases(df: pd.DataFrame | None) -> pd.DataFrame:
    """
    Normalize common slate-date column names so downstream code can safely use
    either `date` or legacy `game_date`. Operates on a copy and is no-op safe.
    """
    if df is None:
        return pd.DataFrame()
    ...
    if "date" not in out.columns and canonical in out.columns:
        out["date"] = out[canonical]
    if "game_date" not in out.columns and canonical in out.columns:
        out["game_date"] = out[canonical]
...
PARK_SPECIFIC_WEATHER = {
    "Coors Field": {"base_mult": 1.10, "temp_sensitivity": 1.35, "wind_sensitivity": 1.35, "precip_sensitivity": 0.95, "cold_floor": 0.68, "hot_ceiling": 1.18},
    "Great American Ball Park": {"base_mult": 1.04, "temp_sensitivity": 1.15, "wind_sensitivity": 1.12, "precip_sensitivity": 1.00, "cold_floor": 0.72, "hot_ceiling": 1.15},
    "Yankee Stadium": {"base_mult": 1.03, "temp_sensitivity": 1.08, "wind_sensitivity": 1.18, "precip_sensitivity": 1.00, "cold_floor": 0.74, "hot_ceiling": 1.14},
    "Citizens Bank Park": {"base_mult": 1.03, "temp_sensitivity": 1.10, "wind_sensitivity": 1.12, "precip_sensitivity": 1.00, "cold_floor": 0.74, "hot_ceiling": 1.14},
    "Oracle Park": {"base_mult": 0.95, "temp_sensitivity": 0.92, "wind_sensitivity": 0.95, "precip_sensitivity": 1.05, "cold_floor": 0.70, "hot_ceiling": 1.08},
    "T-Mobile Park": {"base_mult": 0.96, "temp_sensitivity": 0.94, "wind_sensitivity": 0.98, "precip_sensitivity": 1.05, "cold_floor": 0.70, "hot_ceiling": 1.08},
    "loanDepot park": {"base_mult": 0.97, "temp_sensitivity": 0.95, "wind_sensitivity": 0.95, "precip_sensitivity": 1.00, "cold_floor": 0.72, "hot_ceiling": 1.08},
}


def _park_weather_profile(venue_name: str | None) -> dict:
    default = {
        "base_mult": 1.0,
        "temp_sensitivity": 1.0,
        "wind_sensitivity": 1.0,
        "precip_sensitivity": 1.0,
        "cold_floor": 0.72,
        "hot_ceiling": 1.14,
    }
    if not venue_name:
        return default
    for key, val in PARK_SPECIFIC_WEATHER.items():
        if str(venue_name).strip().lower() == key.lower():
            return dict(val)
    return default
...
def compute_weather_multiplier(
    ...
    profile = _park_weather_profile(venue_name)
    default_weather = {
        "temp_f": None,
        "wind_speed_mph": None,
        "wind_dir_deg": None,
        ...
        "temp_mult": 1.0,
        "wind_mult": 1.0,
        "park_weather_base_mult": round(float(profile.get("base_mult", 1.0)), 3),
        "park_weather_profile": str(venue_name or home_team or "neutral"),
    }
    if not loc or "lat" not in loc or "lon" not in loc:
        return float(profile.get("base_mult", 1.0)), default_weather
    ...
    temp_sens = float(profile.get("temp_sensitivity", 1.0))
    wind_sens = float(profile.get("wind_sensitivity", 1.0))
    precip_sens = float(profile.get("precip_sensitivity", 1.0))
    base_mult = float(profile.get("base_mult", 1.0))
    cold_floor = float(profile.get("cold_floor", 0.72))
    hot_ceiling = float(profile.get("hot_ceiling", 1.14))

    t_mult_raw = temp_multiplier(temp_f)
    t_mult = 1.0 + (t_mult_raw - 1.0) * temp_sens
    t_mult = float(np.clip(t_mult, cold_floor, hot_ceiling))

    w_mult_raw = wind_multiplier(wind_speed, wind_dir, out_to_cf_deg)
    w_mult = 1.0 + (w_mult_raw - 1.0) * wind_sens
    w_mult = float(np.clip(w_mult, 0.84, 1.18))

    final_mult = float(np.clip(base_mult * t_mult * w_mult, 0.58, 1.26))

    if temp_f is not None:
        if temp_f <= 38:
            final_mult *= (0.60 if str(venue_name) != "Coors Field" else 0.70)
        elif temp_f <= 45:
            final_mult *= (0.80 if str(venue_name) != "Coors Field" else 0.86)
        elif temp_f <= 50:
            final_mult *= (0.92 if str(venue_name) != "Coors Field" else 0.95)
        elif temp_f >= 88:
            final_mult *= (1.06 if str(venue_name) == "Coors Field" else 1.03)
        elif temp_f >= 80:
            final_mult *= (1.05 if str(venue_name) == "Coors Field" else 1.02)
        elif temp_f >= 72:
            final_mult *= (1.03 if str(venue_name) == "Coors Field" else 1.01)

    if snowfall_cm is not None and snowfall_cm > 0:
        final_mult *= max(0.60, 1.0 - 0.40 * precip_sens)
    elif precip_mm is not None and precip_mm >= 2.5:
        final_mult *= max(0.72, 1.0 - 0.25 * precip_sens)
    elif precip_mm is not None and precip_mm >= 0.8:
        final_mult *= max(0.88, 1.0 - 0.10 * precip_sens)

    if wind_speed is not None and wind_speed >= 8 and wind_dir is not None and out_to_cf_deg is not None:
        toward_deg = (float(wind_dir) + 180.0) % 360.0
        diff = abs(((toward_deg - float(out_to_cf_deg) + 180.0) % 360.0) - 180.0)
        outward_component = float(np.cos(np.deg2rad(diff)))
        if outward_component > 0:
            wind_bonus = 1.0 + min(0.07, 0.012 * float(wind_speed) * outward_component * wind_sens)
            final_mult *= wind_bonus
        elif outward_component < 0:
            wind_penalty = 1.0 - min(0.05, 0.010 * float(wind_speed) * abs(outward_component) * wind_sens)
            final_mult *= wind_penalty

    final_mult = float(np.clip(final_mult, 0.55, 1.30))
    weather.update({
        "temp_mult": round(t_mult, 3),
        "wind_mult": round(w_mult, 3),
        "out_to_cf_deg": out_to_cf_deg,
        "park_weather_base_mult": round(base_mult, 3),
        "park_weather_profile": str(venue_name or home_team or "neutral"),
    })
...
def compute_today_matchup_scores(
    ...
    # Tightened recent-damage signal: prioritize barrel quality and EV,
    # stabilize with baseline power, and de-emphasize soft recent-form noise.
    recent_damage_score = (
        0.38 * to_100_score(recent_barrel_rate, 0.03, 0.16)
        + 0.18 * to_100_score(recent_hard_hit_rate, 0.28, 0.60)
        + 0.22 * to_100_score(recent_ev, 87.5, 94.0)
        + 0.14 * to_100_score(barrel_rate, 0.03, 0.16)
        + 0.08 * to_100_score(avg_ev, 86.5, 93.5)
    )
    ...
    daily_matchup_hr_score = (0.70 * matchup_score + 0.30 * power_score) * float(power_gate_mult)
...
def build_board(date_str: str, n_sims: int, train_seasons: list[int], use_weather: bool):
        ...
        for side in ["away", "home"]:
            ...
            for hid in hitter_ids:
                ...
                matchup_scores = compute_today_matchup_scores(
                    ...
                    recent_hard_hit_rate=recent_hard_hit_rate,
                    recent_ev=recent_ev,
                    barrel_rate=float(feat.iloc[0].get("barrel_rate", 0.0) or 0.0),
                    avg_ev=float(feat.iloc[0].get("avg_ev", 0.0) or 0.0),
                )
                ...
                rows.append(
                    {
                        ...
                        "park_weather_base_mult": weather_info.get("park_weather_base_mult"),
                        "park_weather_profile": weather_info.get("park_weather_profile"),
    ...
    board["matchup_score_clean"] = (
        ...
        0.30 * batter_heat +
        0.26 * pitch_mix_fit +
        0.14 * environment_clean +
        0.08 * bullpen_score
    ).clip(0,100)

    q70 = board["matchup_score_clean"].quantile(0.70)
    q45 = board["matchup_score_clean"].quantile(0.45)
    q20 = board["matchup_score_clean"].quantile(0.20)
    board["matchup_label"] = np.where(
        board["matchup_score_clean"] >= q70, "🔥",
        np.where(
            board["matchup_score_clean"] >= q45, "👍",
            np.where(board["matchup_score_clean"] >= q20, "OK", "👎")
        )
    )

    def _component_bucket(series: pd.Series, high: float = 65.0, mid: float = 50.0) -> pd.Series:
        vals = pd.to_numeric(series, errors="coerce").fillna(0.0)
        return np.where(vals >= high, "HIGH", np.where(vals >= mid, "MED", "LOW"))

    board["pitcher_risk_bucket"] = _component_bucket(board.get("pitcher_vulnerability_score", 0.0), high=64.0, mid=50.0)
    board["pitch_match_bucket"] = _component_bucket(board.get("pitch_mix_fit_score", 0.0), high=64.0, mid=50.0)
    board["split_bucket"] = _component_bucket(board.get("split_advantage_score", 0.0), high=58.0, mid=48.0)
    board["opportunity_bucket"] = _component_bucket(board.get("opportunity_score", 0.0), high=62.0, mid=50.0)
    board["environment_bucket"] = _component_bucket(board.get("environment_score", 0.0), high=58.0, mid=48.0)
    board["recent_power_bucket"] = _component_bucket(board.get("recent_damage_score", 0.0), high=55.0, mid=40.0)
    board["bullpen_bucket"] = _component_bucket(board.get("bullpen_score", 0.0), high=58.0, mid=50.0)

    board["matchup_reason_primary"] = np.select(
        [
            pd.to_numeric(board.get("pitch_mix_fit_score", 0.0), errors="coerce").fillna(0.0) >= 64.0,
            pd.to_numeric(board.get("pitcher_vulnerability_score", 0.0), errors="coerce").fillna(0.0) >= 64.0,
            pd.to_numeric(board.get("recent_damage_score", 0.0), errors="coerce").fillna(0.0) >= 55.0,
            pd.to_numeric(board.get("environment_score", 0.0), errors="coerce").fillna(0.0) >= 58.0,
            pd.to_numeric(board.get("opportunity_score", 0.0), errors="coerce").fillna(0.0) >= 62.0,
        ],
        [
            "pitch_match",
            "pitcher_risk",
    ...
    # Probability-anchored blend so matchup helps sort without overruling core HR chance
    ...
    board["_final_score"] = 0.42 * board["_matchup_norm"] + 0.58 * board["_prob_norm"]
    board.loc[prob_vals >= 0.22, "_final_score"] += 0.06
    board.loc[(prob_vals >= 0.18) & (prob_vals < 0.22), "_final_score"] += 0.04
    board.loc[(prob_vals >= 0.15) & (prob_vals < 0.18), "_final_score"] += 0.02
    board.loc[prob_vals < 0.14, "_final_score"] -= 0.05
    board.loc[prob_vals < 0.12, "_final_score"] -= 0.05
    ...
    def _bucket_text(val, high_tokens=("HIGH", "YES"), med_tokens=("MEDIUM", "MID", "OK")):
        s = str(val).strip().upper()
        if s in high_tokens:
            return "HIGH" if "YES" not in high_tokens else "YES"
        if s in med_tokens:
            return "MED"
        if s in {"LOW", "NO", ""}:
            return "LOW" if s != "NO" else "NO"
        return s or "LOW"

    export_board = board.sort_values(["p_hr_1plus_sim", "p_hr_pa"], ascending=False).reset_index(drop=True).copy()
    if "rank" in export_board.columns:
        export_board = export_board.drop(columns=["rank"])
    export_board.insert(0, "rank", np.arange(1, len(export_board) + 1))
    export_board["probability"] = (pd.to_numeric(export_board["p_hr_1plus_sim"], errors="coerce") * 100.0).round(1)
    export_board["pitch_match_flag"] = export_board.get("pitch_match_bucket", "LOW").apply(lambda x: _bucket_text(x, high_tokens=("HIGH",), med_tokens=("MEDIUM", "MID", "OK")))
    export_board["pitcher_risk_flag"] = export_board.get("pitcher_risk_bucket", "LOW").apply(lambda x: _bucket_text(x, high_tokens=("HIGH",), med_tokens=("MEDIUM", "MID", "OK")))
    export_board["recent_power_flag"] = export_board.get("recent_power_bucket", "NO").apply(lambda x: "YES" if str(x).upper() in ("HIGH", "MED", "MEDIUM") else "NO")
    export_board["bullpen_flag"] = export_board.get("bullpen_bucket", "OK").apply(lambda x: "WEAK" if str(x).upper()=="HIGH" else ("AVG" if str(x).upper() in ("MED","MEDIUM") else "OK"))
    export_board["flags"] = export_board.apply(
        lambda r: f"MATCH:{r['pitch_match_flag']} | RISK:{r['pitcher_risk_flag']} | POWER:{r['recent_power_flag']} | BP:{r['bullpen_flag']}",
        axis=1,
    )
    export_board["display_line"] = export_board.apply(
        lambda r: f"{r['player_name']} | {r['probability']:.1f}% | {r.get('matchup_label', '')} | {r['flags']}",
        axis=1,
    )

    cols = [
        "rank", "player_name", "probability", "matchup_label",
        "pitch_match_flag", "pitcher_risk_flag", "recent_power_flag", "bullpen_flag",
        "flags", "display_line"
    ]

    date_str = str(export_board["date"].iloc[0]) if "date" in export_board.columns else "today"
...
def main():
    ...
    board = ensure_date_aliases(board)
    ...
    try:
        ...
        if str(e).strip("'") == "game_date":
            board = ensure_date_aliases(board)
            print(f"[DEBUG] Retrying market_clv.attach_clv after normalizing date columns: {board.columns.tolist()}")
            board = market_clv.attach_clv(board)
        else:
            raise
