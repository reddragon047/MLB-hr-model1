...
def force_sequential_ranks(board: pd.DataFrame) -> pd.DataFrame:
    if board is None or board.empty:
        return board
    sort_cols = [c for c in ["p_hr_1plus_sim", "matchup_score_clean", "p_hr_pa"] if c in board.columns]
...
def write_outputs(board: pd.DataFrame, date_str: str, top_n: int):
    ...
    # keep original familiar outputs
    board.to_csv(csv_path, index=False)
    board.to_csv("hr_board.csv", index=False)

    # helper files for existing workflow compatibility
    compact_cols = [c for c in [
        "rank", "player_name", "team", "p_hr_1plus_sim", "power_score",
        "matchup_score_clean", "matchup_label", "probable_pitcher_faced"
    ] if c in board.columns]
    compact = board.head(35)[compact_cols].copy()
    compact.to_csv("hr_board_top35_compact.csv", index=False)
    board.to_csv("hr_board_decision.csv", index=False)
    board[board["rank"] <= 15].to_csv("hr_board_tier1.csv", index=False)
    board[(board["rank"] >= 16) & (board["rank"] <= 35)].to_csv("hr_board_tier2.csv", index=False)
    board[(board["rank"] >= 36) & (board["rank"] <= 60)].to_csv("hr_board_tier3.csv", index=False)
