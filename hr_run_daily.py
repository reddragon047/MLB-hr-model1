...
def compute_weather_multiplier(loc: dict | None, home_team: str, game_datetime: str | None, date_str: str) -> tuple[float, dict]:
    ...
    out_mph, in_mph = _wind_out_in_components(wind_speed, wind_dir, out_to_cf_deg)
    park_override = 1.0
    team = (home_team or "").strip()

    if team == "Chicago Cubs":
        if temp_f is not None and temp_f >= 65 and out_mph >= 8:
            park_override *= 1.20 if out_mph < 12 else 1.25
        elif in_mph >= 10:
            park_override *= 0.70 if in_mph >= 14 else 0.75
        ...
        if out_mph >= 8:
            park_override *= 1.06 if out_mph < 12 else 1.10
        ...
        if in_mph >= 10:
            park_override *= 0.88
        elif temp_f is not None and temp_f <= 55:
            park_override *= 0.93
    ...
    final_mult = float(np.clip(final_mult * park_override, 0.55, 1.30))
    weather.update({
...
# Guaranteed top-level definition kept immediately above build_board to avoid runtime NameError
def sharpened_pitchtype_multiplier(pt_mult: float | None,
    ...
    try:
        pt = float(pt_mult if pt_mult is not None else 1.0)
    except Exception:
        pt = 1.0

    try:
        usage = float(dominant_pitch_usage if dominant_pitch_usage is not None else 0.0)
    except Exception:
        usage = 0.0

    try:
        ratio = float(batter_vs_pitch_ratio if batter_vs_pitch_ratio is not None else 1.0)
    except Exception:
        ratio = 1.0

    bonus = 1.0
    if usage >= 0.30 and ratio >= 1.15 and pt >= 1.0:
        bonus = 1.08
    elif usage >= 0.30 and ratio <= 0.90 and pt <= 1.0:
        bonus = 0.92
