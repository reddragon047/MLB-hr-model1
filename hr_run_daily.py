...
def compute_overall_bettable_score(
    p1: float,
    daily_matchup_hr_score: float,
    exp_pa: float,
    power_score: float,
    power_qualified: int,
    odds_open_1plus=None,
    odds_close_1plus=None,
) -> tuple[float, str]:
    """
    Full-slate visibility model:
    everyone stays on the board, but gets tagged as bettable / not bettable
    ...
    prob_score = scale(p1, 0.06, 0.26)
    matchup_score = float(daily_matchup_hr_score or 0.0)
    opp_score = scale(exp_pa, 3.4, 5.0)
    power_s = float(power_score or 0.0)
    qual_bonus = 8.0 if int(power_qualified or 0) == 1 else 0.0

    no_open = pd.isna(odds_open_1plus) or odds_open_1plus in ("", "NA", "N/A", None)
    no_close = pd.isna(odds_close_1plus) or odds_close_1plus in ("", "NA", "N/A", None)
    has_odds = 0.0 if (no_open and no_close) else 1.0
    odds_bonus = 6.0 if has_odds else -4.0
    ...
    for g in games:
        ...
        for side in ["away", "home"]:
            ...
            for hid in hitter_ids:
                ...
                overall_bettable_score, bettable_status = compute_overall_bettable_score(
                    p1=p1,
                    daily_matchup_hr_score=daily_matchup_hr_score if "daily_matchup_hr_score" in locals() else 50.0,
                    exp_pa=exp_pa,
                    power_score=power_score if "power_score" in locals() else 50.0,
                    power_qualified=power_qualified if "power_qualified" in locals() else 0,
                    odds_open_1plus=odds_open_1plus if "odds_open_1plus" in locals() else None,
                    odds_close_1plus=odds_close_1plus if "odds_close_1plus" in locals() else None,
                )
