...
def safe_read_csv(path: str, required_cols: list[str] | None = None) -> pd.DataFrame:
    """Read CSV defensively so one malformed row does not crash the full board build."""
    last_err = None
    for kwargs in (
        {},
        {"engine": "python"},
        ...
        try:
            ...
            if required_cols:
                ...
                missing = [c for c in required_cols if c.strip().lower() not in cols_norm]
                if missing:
...
def add_market_edge(board: pd.DataFrame) -> pd.DataFrame:
    ...
    def read_odds_file(fp: str):
        if not os.path.exists(fp):
            return None
        df = safe_read_csv(fp, required_cols=["player_name", "odds_1plus"])
        if df.empty:
            return None
