...
def required_edge_threshold_1plus(implied_open_prob: float) -> float:
    """
    Tiered edge requirement (probability-space).

    Input: implied_open_prob as a probability (0.10 = 10%).
    Output: required edge as a probability (0.02 = 2%).
    """
    try:
        p = float(implied_open_prob)
    except Exception:
        return float("nan")
...
def add_bet_flag_columns(board: pd.DataFrame) -> pd.DataFrame:
    """
    Adds:
      - edge_req_1plus: required edge based on implied open probability bucket
      - bet_flag_1plus: True if edge_open_1plus >= edge_req_1plus

    Does NOT filter rows; it's just a tag (you selected logging mode B).
    """
    ...
    if "implied_prob_open_1plus" in df.columns:
        implied = pd.to_numeric(df["implied_prob_open_1plus"], errors="coerce")
    else:
        implied = pd.Series(np.nan, index=df.index)

    if "edge_open_1plus" in df.columns:
        edge = pd.to_numeric(df["edge_open_1plus"], errors="coerce")
    else:
        edge = pd.Series(np.nan, index=df.index)

    df["edge_req_1plus"] = implied.map(required_edge_threshold_1plus)
    df["bet_flag_1plus"] = (edge >= df["edge_req_1plus"]) & edge.notna() & df["edge_req_1plus"].notna()
...
def append_performance_log(board: pd.DataFrame, run_date_str: str, log_path: str = PERF_LOG_PATH):
    ...
    # Tiered edge requirement + bet flag (does not filter; used for research mode B)
    df["edge_req_1plus"] = df["implied_open_prob"].map(required_edge_threshold_1plus)
    df["bet_flag_1plus"] = (df["edge_open_pct"] >= df["edge_req_1plus"]) & df["edge_open_pct"].notna() & df["edge_req_1plus"].notna()
    ...
    out = pd.DataFrame({
        ...
        "edge_req_1plus": df["edge_req_1plus"],
        "bet_flag_1plus": df["bet_flag_1plus"],
...
def main():
    ...
    board = add_bet_flag_columns(board)
