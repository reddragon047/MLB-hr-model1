...
def main():
    ...
    # For each N, take the top N players by p each day, then summarize hit rate.
    topn_rows = []
    if {'date','p','y'}.issubset(set(results.columns)):
        for n in [5, 10, 25]:
            top = (
            ...
            topn_rows.append({
                'N': n,
                'n_obs': int(len(top)),
                'days': int(top['date'].nunique()) if len(top) else 0,
                'hr_count': int(top['y'].sum()) if len(top) else 0,
                'hit_rate': float(top['y'].mean()) if len(top) else 0.0,
            })
        pd.DataFrame(topn_rows).to_csv('outputs/hr_topn_summary.csv', index=False)
    else:
        # If your columns differ, adjust here.
        pd.DataFrame([{'message': 'Missing columns for TopN summary', 'have_cols': ','.join(results.columns)}]).to_csv(
            'outputs/hr_topn_summary.csv', index=False
        )
