...
def statcast_safe(start_dt: str, end_dt: str, tries: int = 5):
    """Robust statcast pull with retry/backoff for occasional malformed CSV / network hiccups."""
    last_err = None
    for i in range(tries):
        try:
            ...
            last_err = e
        time.sleep(5 * (i + 1))
    raise RuntimeError(f"Statcast pull failed after {tries} tries: {last_err}")
